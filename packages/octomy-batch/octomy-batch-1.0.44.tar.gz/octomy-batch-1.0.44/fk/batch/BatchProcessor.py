...
class BatchProcessor:
    ...
    def _execute_safely(self, entrypoint, item):
        ...
        result = None
        try:
            # We cap runtime at TTL if set or DEFAULT_TTL as default
            timeout = item.get("ttl_seconds")
            if not timeout or timeout <= 0:
                timeout = self.config.get("batch-default-ttl", 30)
...
#            "last_failed": self.db.get_last_jobs(limit=20, status=["failed"]),
#            "last_pending": self.db.get_last_jobs(limit=20, status=["pending"]),
#            "last_in_progress": self.db.get_last_jobs(limit=20, status=["in-progress"]),
