...
class BigQueryConnector(SQLConnector):
    def __init__(self, dataset: str, project: str = "") -> None:
        ...
        self.project_id: str = getenv(f"BQ_{self.project}_PROJECT_ID")
    ...
    def _generate_credentials(self) -> None:
        ...
        creds: Dict = {
            ...
            "private_key": getenv(f"BQ_{self.project}_PRIVATE_KEY"),
...
class MongoDBSSHConnector(MongoDBConnector):
    ...
    def connection(self) -> MongoClient:
        ...
        try:
            ...
            ssh.start()
            mongo_params = dict(
                host=getenv(f"MONGO_{self.schema.upper()}_HOST"),
