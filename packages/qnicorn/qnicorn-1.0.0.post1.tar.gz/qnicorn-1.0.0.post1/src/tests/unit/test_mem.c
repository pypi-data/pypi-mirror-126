...
static void test_mem_protect()
{
    ...
    OK(qc_open(QC_ARCH_X86, QC_MODE_32, &qc));
    OK(qc_reg_write(qc, QC_X86_REG_EAX, &r_eax));
    OK(qc_reg_write(qc, QC_X86_REG_ESI, &r_esi));
    OK(qc_mem_map(qc, 0x1000, 0x1000, QC_PROT_READ | QC_PROT_EXEC));
    OK(qc_mem_map(qc, 0x2000, 0x1000, QC_PROT_READ));
    OK(qc_mem_protect(qc, 0x2000, 0x1000, UC_PROT_READ | UC_PROT_WRITE));
    OK(qc_mem_write(qc, 0x1000, code, sizeof(code)));

    OK(qc_emu_start(qc, 0x1000, 0x1000 + sizeof(code) - 1, 0, 1));
    OK(qc_mem_read(qc, 0x2000 + 4, &mem, 4));
