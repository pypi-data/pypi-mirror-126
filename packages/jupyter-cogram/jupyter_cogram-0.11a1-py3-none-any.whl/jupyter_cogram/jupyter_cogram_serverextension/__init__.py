...
cogram_server_path = os.environ.get("COGRAM_SERVER_PATH")

DEFAULT_CDN_PATHS = [
    {
        "metadata_url": "https://storage.googleapis.com/storage/v1/b/cogram-public/o/jupyter-cogram%2Fserver.py",
        "source_url": "https://storage.googleapis.com/cogram-public/jupyter-cogram/server.py",
    }
]
...
def get_cdn_urls() -> List[Dict]:
    url = "https://api.cogram.ai/info"
    # noinspection PyBroadException
    try:
        r = requests.get(url, timeout=timeout_in_seconds)
        r.raise_for_status()
        return r.json()["jupyter_cogram"]["python_sources"]
    except:
        return DEFAULT_CDN_PATHS


def import_cogram_server() -> Any:
    if cogram_server_path:
        p = Path(cogram_server_path).resolve()
        logger.debug(f"In development mode. Loading from local sources: '{p}'")
        if Path(cogram_server_path).is_file():
            ...
            logger.error(f"Local development file '{p}' not found.")
