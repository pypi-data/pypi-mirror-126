...
from algolink.runtime.interface import ExecutionError, Interface, InterfaceLoader
from algolink.runtime.utils import registering_type
from algolink.utils.classproperty import classproperty
...
class Server(_ServerBase):
    """
    Base class for AlgoLink servers
    """
...
class HTTPServerConfig(Config):
    ...
    port = Param('port', default='9001', parser=int)
...
class BaseHTTPServer(Server):
    """
    HTTP-based AlgoLink runtime server.
    ...
    Host to which server binds is configured via `ALGOLINK_HOST` environment variable:
    default is `0.0.0.0` which means any local or remote, for rejecting remote connections use `localhost` instead.

    Port to which server binds to is configured via `ALGOLINK_PORT` environment variable: default is 9000.
    """
    ...
    @staticmethod
    def _execute_method(interface: Interface, method: str, request_data, algolink_id: str):
        rlogger.debug('Got request for [%s]: %s', algolink_id, request_data)
        ...
        if isinstance(result, bytes):
            rlogger.debug('Got response for [%s]: <binary content>', algolink_id)
            return result
