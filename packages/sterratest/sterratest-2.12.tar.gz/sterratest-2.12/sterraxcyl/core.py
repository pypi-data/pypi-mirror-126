        ...
        for usrnm in progressbar:
            ...
            if __a1.status_code == 200 and __a1.text != {}:
                ...
                if __name__ == '__main__':
                    print(L['!']+f' ExportException: "https://www.instagram.com/{usrnm}/channel/?__a=1" responded {__a1.status_code};\n    Instagram detected a suspicious activity. Verify the existence of the {usrnm} account.')
                    while True:
                        progressbar.close()
                        keskonfé = input(L['?']+f' Do you prefer:\n    1 - Export what have been extracted,\n    2 - Stop the program.')
                        if keskonfé == '1':
                            return rtn
                        elif keskonfé == '2':
                            exit()
                    ...
                    outExcept(e=f'ExportException: "https://www.instagram.com/{usrnm}/channel/?__a=1" responded {__a1.status_code};\n    Instagram might have detected a suspicious activity.\n    Verify the existence of the {usrnm} account.\n    Verify the validity of your SessionID.')
        return rtn
        ...
        async def fetch(session, url):
            async with session.get(url, headers = {'User-Agent': self.useragent, 'Referer': f'https://www.instagram.com/'}, cookies=self.cookies, allow_redirects=False) as __a1:
                if __a1.status == 200 and await __a1.text() != {}:
                    ...
                    progressbar.close()
                    usrnm = url.replace('https://www.instagram.com/', '').replace('/channel/?__a=1', '')
                    outExcept(e=f'ExportException: "{url}" responded {__a1.status};\n    Verify the existence of the {usrnm} account, and also the validity of your SessionID.')
...
def main():
    ...
    lognn.add_argument('-lcrd', '--login-credentials', metavar=('U', 'P'), nargs=2)
    lognn.add_argument('-ssid', '--login-session-id', metavar='S')
    ...
    data_analysis.add_argument('--size', help='size of the most probable username list (will be by default the size of the followers/mutuals/following list filled in)', type=int)
...
'''À faire:
- exportation liste de probas
- probas d'intérêts;
    pas en mettant la liste en décroissant,
    mais en créant un second filtre de données,
    coéfficienté différement et prenant en compte les éléments des comptes pro,
    en évitant de rendre exclusif au --all-infos
- liste d'useragent pour aléatoires comme pour holehe
- informer que l'ecriture du fichier est en cours (longue liste avait bloqué sans donner de nouvelles)'''