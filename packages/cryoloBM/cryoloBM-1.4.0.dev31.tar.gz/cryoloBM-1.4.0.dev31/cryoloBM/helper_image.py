...
def set_and_update_boxdictionary_image_changed(controller:boxmanager_controller,index_tomo:int)->None:
    """
    ...
    :param controller: Boxmanager_controller obj
    :param index_tomo: index of tomo
    """
    ...
    if controller.model.box_dictionary.has_same_boxsize(boxsize=None, f=current_filename, n=index_tomo):
        controller.model.box_dictionary.update_boxsize_dictionary(new_boxsize=controller.model.boxsize,
                                                                  f=current_filename, n=index_tomo)
    ...
    if not controller.model.box_dictionary.has_same_visualization(visualization=controller.model.visualization,
        ...
        controller.model.box_dictionary.update_visualization_dictionary(
            new_visualization=controller.model.visualization,f=current_filename, n=index_tomo)
    ...
    if not controller.model.is_cbox_untraced and  controller.model.box_dictionary.has_same_boxsize(boxsize=controller.model.boxsize, f=current_filename, n=index_tomo):
        controller.model.rectangles = controller.model.box_dictionary.get_sketches(f=current_filename,
                                                        n=index_tomo, is_3D =controller.model.has_3d,  is_tracing = False)
    else:
        controller.model.rectangles = controller.model.box_dictionary.resized_and_get_sketches(
            new_boxsize=controller.model.boxsize, f=current_filename, n=index_tomo,use_estimated_size=controller.model.use_estimated_size)
    ...
    if controller.model.visualization in [constants.Visualization_cb.CIRCLE_SEGMENTED.value,constants.Visualization_cb.RECT_FILAMENT_SEGMENTED.value] and \
            (not controller.model.box_dictionary.has_same_box_distance(box_distance=controller.model.box_distance_filament, f=current_filename, n=index_tomo) or controller.model.has_3d):
        controller.model.rectangles =controller.model.box_dictionary.change_box_distance(new_box_distance=controller.model.box_distance_filament, f=current_filename, n=index_tomo)
    return
...
def event_image_changed(controller:boxmanager_controller, root_tree_item:helper_GUI.QtG.QTreeWidgetItem)->None:
    ...
    if controller.model.box_dictionary:
        ...
        set_and_update_boxdictionary_image_changed(controller=controller,index_tomo=controller.model.index_tomo)
...
def draw_all_patches(controller:boxmanager_controller)->None:
    ...
    # it happens when i change image after saving gif
    for box in controller.model.rectangles:
