...
def _to_record(d):
    """Convert a dictionary to a kvdb_pb2.Record.

    Args:
        d (Dict[str, Any]): the dictionary to be converted

    Returns:
        kvdb_pb2.Record: The return value. 
...
def _from_record(rec):
    """Convert a kvdb_pb2.Record to a dictionary.

    Args:
        rec (kvdb_pb2.Record): the record to be converted
    ...
    # Sanity check
    if type(rec) != Record:
        raise TypeError('expected a kvdb_pb2.Record')
    # Convert the values
    result = {}
    for k in rec.values:
...
def _to_record_iterator(iter):
    ...
    objects of type kvdb_pb2.Record.

    Args:
        iter (Iterable[Dict[str, Any]]): the iterator to be converted

    Returns:
        Iterable[kvdb_pb2.Record]: The return value. 
...
class _RecordIterator:
    """
    An iterator of objects of type kvdb_pb2.Record that wraps an
    underlying iterator of dictionaries.
    ...
    def __next__(self):
        return _to_record(next(self.itr))
...
def _from_number_of_records(nr):
    """Convert a kvdb_pb2.NumberOfRecords nr to an int."""
...
def _to_condition_and_replacement(condition, replacement):
    ...
    kvdb_pb2.ConditionAndReplacement.
...
def _to_order_by(x):
    """
    Converts the given sort order to a kvdb_pb2.OrderBy.
...
def _to_condition_template_order_and_limit(template, selector, order, limit):
    ...
    kvdb_pb2.ConditionTemplateOrderAndLimit.
...
def _from_record_iterator(itr):
    """
    Wraps an iterable of kvdb_pb2.Records as an iterator of key-value records.
    
    Args:
        itr (Iterable[kvdb_pb2.Record]):
...
def _to_index(s):
    """
    Converts the given string to a kvdb_pb2.Index.
...
def _from_index_list(S):
    """
    Converts the given kvdb_pb2.IndexList to a list of strings.

    Args:
        S (kvdb_pb2.IndexList):
