            ...
            with tf.device(get_device()):
                with self.name_scope:
                    ...
                    if self.transposed:
                        ...
                        self.weight =Parameter(tf.random.normal(shape=[*self.kernel_size,  self.input_filters // self.groups, self.input_filters], mean=0,  stddev=1) * 0.02, trainable=True, name='weight')

                    else:

                        # [filter_height, filter_width, in_channels, out_channels]`
                        self.weight =Parameter( data=tf.random.normal(shape=[*self.kernel_size, self.input_filters,  self.input_filters // self.groups], mean=0,    stddev=1) * 0.02, trainable=True, name='weight')
