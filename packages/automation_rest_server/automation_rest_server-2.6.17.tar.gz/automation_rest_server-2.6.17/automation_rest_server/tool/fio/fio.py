...
from tool.device.linux_nvme import NVME
...
class Fio(LinuxFio):
    ...
    @staticmethod
    def _get_dev_id(file_name):
        dev_id, namespace_id = None, None
        rets = re.findall(r"nvme(\d+)n(\d+)", file_name)
        if rets:
            dev_id = rets[0][0]
            namespace_id = rets[0][1]
        return dev_id, namespace_id

    def nvme_format(self, parameters):
        if "format" in parameters.keys():
            lbaf = parameters["lbaf"]
            if lbaf != "":
                dev_id, namespace_id = self._get_dev_id(parameters["fio"]["filename"])
                if dev_id is not None:
                    nvme = NVME(dev_id)
                    nvme.format(namespace_id, lbaf)
