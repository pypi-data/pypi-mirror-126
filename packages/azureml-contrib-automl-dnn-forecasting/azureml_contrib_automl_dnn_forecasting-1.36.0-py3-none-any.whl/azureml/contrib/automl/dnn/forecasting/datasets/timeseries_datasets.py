...
class TimeSeriesDataset(AbstractDNNTimeSeriesDataset):
    ...
    def get_train_test_split(self, validation_percent: float = 0.1):
        ...
        if len(test_data_grains) == 0 and self.cross_validation == 1:
            # not enough data for full test and train, take the one sample from a grain that has at least twice
            # horizon and remove last horizon from training data.
            grainitem_to_split = train_data_grains[0]
            for item in train_data_grains:
                if item.X_df.shape[0] >= 2 * self.horizon:
                    grainitem_to_split = item
            test_item = _DataGrainItem(grainitem_to_split.X_df, grainitem_to_split.y, 0, 1, offset=0)
            if grainitem_to_split.X_df.shape[0] > self.horizon:
                grainitem_to_split.X_df = grainitem_to_split.X_df.iloc[: -self.horizon, :]
                grainitem_to_split.y = grainitem_to_split.y[:, : -self.horizon]
                test_data_grains.append(test_item)
                test_size += 1
