...
from azureml.automl.core.shared.exceptions import InvalidOperationException, ValidationException
...
from ..metrics.metrics import compute_metric, get_target_values, save_metric, _get_worst_scores
...
class RunUpdateCallback(Callback):
    ...
    def _score_metrics(self, loss : float) -> None:
        ...
        if self.ds_valid.cross_validation:
            ...
            for i in eval_list:
                ...
                slice_scores = self._safe_compute_metrics(eval_set, y_pred)
    ...
    def _safe_compute_metrics(self, eval_set, y_pred):
        """Compute metric using the _EvalDataset and return worst score if there is an exception.
        ...
        try:
            ...
            self.logger.warning("Forecast Metric calculation resulted in error, reporting back worst scores")
