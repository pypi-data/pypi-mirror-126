...
__all__ = ['plf_2022', 'config', 'cache', 'tc', 'inputfile_path', 'sample_pop', 'convert_erfr_to_openfisca',
           'sample_pop', 'individus_to_foyer_fiscaux', 'sample_pop_foyer', 'create_simulation', 'CustomReform',
           'compute_variable', 'rounddown', 'compute_quantile', 'compute_reform', 'compute_all_simulation',
           'SimulationError', 'quantile_to_df']
...
from .aggregates_read import get_aggregate
...
def create_simulation(period, data, tbs):
    ...
    for colonne in data.columns:
        ...
        for PERIOD in range(int(period) - 2, int(period) + 1):
...
class CustomReform(Reform):
    ...
    def __init__(
        self, tbs: FranceTaxBenefitSystem, payload: ReformeSocioFiscale, period: str
    ) -> None:
...
def compute_reform(
    ...
    tax_benefit_system = CustomReform(
        tax_benefit_system, reformParameters, annee_de_calcul
    )
    #     logger.debug(
    ...
    for variable in reformParameters.output_variables:
        resultat = compute_variable(tax_benefit_system, annee_de_calcul, variable)
...
def compute_all_simulation(
    ...
    try:
        ...
        reform_base = ReformeSocioFiscale(
            base=reform.base,
            amendement={},  # On supprime l'amendement
        ...
        # Avec le PLF
        if reform.plf and config.get("PLF", fail_on_missing=False):
            with open(config.get("PLF")) as json_file:
                changement_plf = json.load(json_file)
            reform_plf = ReformeSocioFiscale(
                base=reform.base,
                plf=reform.plf,
                amendement=changement_plf,  # On donne le PLF
                output_variables=reform.output_variables,
                quantile_nb=reform.quantile_nb,
                quantile_base_variable=reform.quantile_base_variable,
                quantile_compare_variables=reform.quantile_compare_variables,
            )
            if reform.plf is not None:
    ...
    res = AllSimulationResult(
        result=result, errors=errors
    )  # if result else res=AllSimulationResult(errors=errors)
    return res
