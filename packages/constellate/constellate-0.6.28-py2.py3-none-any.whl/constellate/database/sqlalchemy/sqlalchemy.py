...
class SQLAlchemy:
    ...
    async def _setup_session_do_orm_execute(self, session=AsyncSession, options: Dict = None):
        ...
        def _switch_shard_engine(context: ORMExecuteState = None, shard_engine_id: str = None):
            logger.debug(f"Using engine with shard_id={shard_engine_id}")
            context.update_execution_options(_sa_shard_id=shard_engine_id)
        ...
        def switch_shard_engine(context: ORMExecuteState):
            if isinstance(context.session, ShardedSession):
                # Usage: Instruct session to use a particular engine whose shard_id is 'engine_shard_id_3' to execute select(...)
                ...
                for elem in context.user_defined_options:
                    ...
                    if found_shard_engine_option:
                        _switch_shard_engine(context=context, shard_engine_id=elem.payload)
                        return

                # Usage: Instruct session to use a particular engine whose shard_id is 'engine_shard_id_3' for all statement by default,
                #        unless overriden on an individual statement basis
                # Sample: async with self.session_scope(execution_options={'default_shard_id':SetShardEngineOption('engine_shard_id_3')}) as session:
                #           ...
                elem = context.execution_options.get("default_shard_id", None)
                found_shard_engine_option = isinstance(elem, SetShardEngineOption)
                if found_shard_engine_option:
                    _switch_shard_engine(context=context, shard_engine_id=elem.payload)
    ...
    async def __session_scope(
        ...
        try:
            self._setup_session_do_orm_execute(session=session)
            yield session
