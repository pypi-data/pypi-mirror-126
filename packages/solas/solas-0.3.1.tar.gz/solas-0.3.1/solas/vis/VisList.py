...
from solas.vislib.altair.AltairRenderer import AltairRenderer
...
class VisList:
    ...
    def sort(self, remove_invalid=True, descending=True, intent_cols=None):
        ...
        if solas.config.sort == "none":
            return
        elif solas.config.sort == "ascending":
            descending = False
        elif solas.config.sort == "descending":
            descending = True
    ...
    def _ipython_display_(self):
        ...
        recJSON = SolasDataFrame.rec_to_JSON([recommendation])
        self._widget = solaswidget.SolasWidget(
            currentVis={},
    ...
    def refresh_source(self, ldf):
        ...
        if ldf is not None:
            ...
            if len(self._input_lst) > 0:
                ...
                width_criteria = len(self._collection) > (solas.config.topk + 3)
                length_criteria = len(ldf) > solas.config.early_pruning_sample_start
                if solas.config.early_pruning and width_criteria and length_criteria:
                    # print("Apply approx to this VisList")
                    ldf._message.add_unique(
                        "Large search space detected: Solas is approximating the interestingness of recommended visualizations.",
                        priority=1,
                    )
                    approx = True
                solas.config.executor.execute(self._collection, ldf, approx=approx)
