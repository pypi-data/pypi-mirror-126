...
# from solas.implicit.tracker import CodeTracker
...
class Config:
    """
    Class for Solas configurations applied globally across entire session
    """
    ...
    def topk(self, k: Union[int, bool]):
        ...
        if isinstance(k, int) or isinstance(k, bool):
            ...
            warnings.warn(
                "Parameter to solas.config.topk must be an integer or a boolean.",
                stacklevel=2,
            )
    ...
    def sort(self, flag: Union[str]):
        ...
        if isinstance(flag, str) and flag in ["none", "ascending", "descending"]:
            ...
            warnings.warn(
                "Parameter to solas.config.sort must be one of the following: 'none', 'ascending', or 'descending'.",
                stacklevel=2,
            )
    ...
    def default_display(self):
        """
        Set the widget display to show Pandas by default or Solas by default
        Parameters
        ----------
        type : str
            Default display type, can take either the string `solas` or `pandas` (regardless of capitalization)
        """
    ...
    def default_display(self, type: str) -> None:
        ...
        """
        if type.lower() == "solas":
            self._default_display = "solas"
        elif type.lower() == "pandas":
            self._default_display = "pandas"
        else:
            warnings.warn(
                "Unsupported display type. Default display option should either be `solas` or `pandas`.",
                stacklevel=2,
            )
    ...
    def plotting_backend(self, type: str) -> None:
        ...
        if type.lower() == "vegalite" or type.lower() == "altair":
            ...
            warnings.warn(
                "Unsupported plotting backend. Solas currently only support 'altair', 'vegalite', or 'matplotlib'",
                stacklevel=2,
            )
    ...
    def plotting_scale(self, scale: float) -> None:
        """
        Set the scale factor for charts displayed in Solas.
        ----------
    ...
    def register_action(
        ...
        *args,
        ...
        """
        Registers the provided action globally in solas
    ...
    def remove_action(self, name: str = "") -> None:
        """
        Removes the provided action globally in solas
