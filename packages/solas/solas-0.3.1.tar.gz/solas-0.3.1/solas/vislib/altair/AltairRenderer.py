...
from solas.vislib.altair.BarChart import BarChart
from solas.vislib.altair.ScatterChart import ScatterChart
from solas.vislib.altair.LineChart import LineChart
from solas.vislib.altair.Histogram import Histogram
from solas.vislib.altair.Heatmap import Heatmap
from solas.vislib.altair.BoxPlot import BoxPlot
from solas.vislib.altair.Choropleth import Choropleth
    ...
    def create_vis(self, vis, standalone=True):
        ...
        if vis.approx:
            if vis.mark == "scatter" and vis._postbin:
                vis._mark = "heatmap"
                solas.config.executor.execute_2D_binning(vis)
            else:
                # Exactly recompute the selected vis (e.g., top k) to display
                solas.config.executor.execute([vis], vis._original_df, approx=False)
        ...
        if chart:
            if solas.config.plotting_style and (
                solas.config.plotting_backend == "vegalite" or solas.config.plotting_backend == "altair"
            ):
                chart.chart = solas.config.plotting_style(chart.chart)
            if self.output_type == "VegaLite":
                chart_dict = chart.chart.to_dict()
                # this is a bit of a work around because altair must take a pandas dataframe and we can only generate a solasDataFrame
                ...
                if solas.config.plotting_style:
                    chart.code += "\n".join(
                        inspect.getsource(solas.config.plotting_style).split("\n    ")[1:-1]
                    )
                chart.code += "\nchart"
