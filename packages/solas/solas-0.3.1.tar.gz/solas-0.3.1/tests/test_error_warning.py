...
def test_vis_private_properties(global_var):
    ...
    assert isinstance(vis.data, solas.core.frame.SolasDataFrame)
...
# Test DataFrame Properties give Solas Warning but not UserWarning
def test_solas_warnings(global_var):
    df = pd.DataFrame()
    df._ipython_display_()
    assert df._widget.message == f"<ul><li>Solas cannot operate on an empty DataFrame.</li></ul>"
    df = pd.DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]])
    df._ipython_display_()
    assert (
        df._widget.message
        == f"<ul><li>The DataFrame is too small to visualize. To generate visualizations in Solas, the DataFrame must contain at least 5 rows.</li></ul>"
    )
    df = pytest.car_df
    ...
    assert (
        ...
        == f"<ul><li>Solas does not currently support visualizations in a DataFrame with hierarchical indexes.\nPlease convert the DataFrame into a flat table via pandas.DataFrame.reset_index.</li></ul>"
    )
