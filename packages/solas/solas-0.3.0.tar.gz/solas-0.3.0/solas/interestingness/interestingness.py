...
from solas.interestingness.similarity import preprocess, euclidean_dist
...
def interestingness(vis: Vis, ldf: SolasDataFrame) -> int:
    """
    ...
    try:
        ...
        if solas.config.interestingness_fallback:
...
def deviation_from_overall(
    ...
    """
    if solas.config.executor.name == "PandasExecutor":
        if exclude_nan:
        ...
        v_size = len(vis.data)
    elif solas.config.executor.name == "SQLExecutor":
    ...
    solas.config.executor.execute([unfiltered_vis], ldf)
    if exclude_nan:
...
def unevenness(vis: Vis, ldf: SolasDataFrame, measure_lst: list, dimension_lst: list) -> int:
    """
