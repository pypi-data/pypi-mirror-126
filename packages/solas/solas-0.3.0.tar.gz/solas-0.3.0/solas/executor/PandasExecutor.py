...
from solas.utils.utils import check_import_solas_widget, check_if_id_like, is_numeric_nan_column
import warnings
import solas
...
class PandasExecutor(Executor):
    ...
    @staticmethod
    def execute_sampling(ldf: SolasDataFrame):
        """
        Compute and cache a sample for the overall dataframe

        - When # of rows exceeds solas.config.sampling_start, take 75% df as sample
        - When # of rows exceeds solas.config.sampling_cap, cap the df at {solas.config.sampling_cap} rows

        solas.config.sampling_start = 100k rows
        solas.config.sampling_cap = 1M rows
        ...
        if SAMPLE_FLAG and len(ldf) > SAMPLE_CAP:
            ...
            ldf._message.add_unique(
                f"Large dataframe detected: Solas is only visualizing a sample capped at {SAMPLE_CAP} rows.",
                priority=99,
            )
            ...
            ldf._message.add_unique(
                f"Large dataframe detected: Solas is visualizing a sample of {SAMPLE_FRAC}% of the dataframe ({len(ldf._sampled)} rows).",
                priority=99,
            )
        else:
            ldf._sampled = ldf
    ...
    @staticmethod
    def execute_approx_sample(ldf: SolasDataFrame):
        """
        ...
        if ldf._approx_sample is None:
            if len(ldf._sampled) > solas.config.early_pruning_sample_start:
                ldf._approx_sample = ldf._sampled.sample(
                    n=solas.config.early_pruning_sample_cap, random_state=1
                )
            else:
    ...
    @staticmethod
    def execute(vislist: VisList, ldf: SolasDataFrame, approx=False):
        """
        ...
        ldf : solas.core.frame
            SolasDataFrame with specified intent.
    ...
    def execute_aggregate(vis: Vis, isFiltered=True):
        ...
        if measure_attr != "":
            ...
            try:
                ...
                warnings.warn(
                    f"\nSolas detects that the attribute '{groupby_attr.attribute}' maybe contain mixed type."
    ...
    @staticmethod
    def execute_binning(ldf: SolasDataFrame, vis: Vis):
        """
    ...
    def execute_filter(vis: Vis) -> bool:
        ...
        ), "execute_filter assumes input vis.data is populated (if not, populate with SolasDataFrame values)"
    ...
    def execute_2D_binning(vis: Vis) -> None:
        ...
        with pd.option_context("mode.chained_assignment", None):
            ...
            vis._vis_data["xBin"] = pd.cut(vis._vis_data[x_attr], bins=solas.config.heatmap_bin_size)
            vis._vis_data["yBin"] = pd.cut(vis._vis_data[y_attr], bins=solas.config.heatmap_bin_size)
    ...
    #######################################################
    def compute_dataset_metadata(self, ldf: SolasDataFrame):
        ldf.history.freeze()
        ldf._data_type = {}
        self.compute_data_type(ldf)
        ldf.history.unfreeze()

    def compute_data_type(self, ldf: SolasDataFrame):
        ldf.history.freeze()
        ...
        if len(non_datetime_attrs) == 1:
            warn_msg += f"\nSolas detects that the attribute '{non_datetime_attrs[0]}' may be temporal.\n"
        elif len(non_datetime_attrs) > 1:
            warn_msg += f"\nSolas detects that attributes {non_datetime_attrs} may be temporal.\n"
    ...
    def compute_stats(self, ldf: SolasDataFrame):
        ldf.history.freeze()
