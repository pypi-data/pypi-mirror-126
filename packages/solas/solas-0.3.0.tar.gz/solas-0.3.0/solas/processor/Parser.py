...
class Parser:
    ...
    then generates the Solas internal specification through solas.Clause.
    """
    ...
    def parse(intent: List[Union[Clause, str]]) -> List[Clause]:
        ...
        ----------
        intent : List[Clause]
                Underspecified list of solas.Clause objects.

        Returns
        -------
        List[Clause]
                Parsed list of solas.Clause objects.
        """
        if type(intent) != list:
            raise TypeError(
                "Input intent must be a list consisting of string descriptions or solas.Clause objects."
                "\nSee more at: https://solas-api.readthedocs.io/en/latest/source/guide/intent.html"
            )
        import re

        new_context = []
        # checks for and converts users' string inputs into solas specifications
