...
def register_new_action(validator: bool = True):
    ...
    def random_categorical(ldf):
        intent = [solas.Clause("?", data_type="nominal")]
    ...
    if validator:
        solas.config.register_action("bars", random_categorical, contain_horsepower)
    else:
        solas.config.register_action("bars", random_categorical)
    return df
...
def test_invalid_function(global_var):
    ...
    with pytest.raises(ValueError, match="Action must be a callable"):
        solas.config.register_action("bars", "not a Callable")
...
def test_invalid_validator(global_var):
    ...
    with pytest.raises(ValueError, match="Display condition must be a callable"):
        solas.config.register_action("bars", random_categorical, "not a Callable")
...
def test_remove_action():
    ...
    solas.config.remove_action("bars")
    df._ipython_display_()
    assert (
        "bars" not in df.recommendation,
...
def test_remove_default_actions(global_var):
    ...
    solas.config.remove_action("distribution")
    df._ipython_display_()
    assert "Distribution" not in df.recommendation

    solas.config.remove_action("occurrence")
    df._ipython_display_()
    assert "Occurrence" not in df.recommendation

    solas.config.remove_action("temporal")
    df._ipython_display_()
    assert "Temporal" not in df.recommendation

    solas.config.remove_action("correlation")
    df._ipython_display_()
...
def test_matplotlib_set_default_plotting_style():
    ...
    df = pd.read_csv("solas/data/car.csv")
    solas.config.plotting_style = add_title
    df._ipython_display_()
...
def test_set_default_plotting_style():
    ...
    df = pd.read_csv("solas/data/car.csv")
    solas.config.plotting_style = change_color_make_transparent_add_title
    df._ipython_display_()
...
def test_sampling_flag_config():
    solas.config.heatmap = False
    solas.config.sampling = True
    solas.config.early_pruning = False
    import numpy as np

    N = int(1.1 * solas.config.sampling_cap)
    df = pd.DataFrame({"col1": np.random.rand(N), "col2": np.random.rand(N)})
    df.maintain_recs()
    assert len(df.recommendation["Correlation"][0].data) == solas.config.sampling_cap
    solas.config.sampling = True
    solas.config.heatmap = True
...
def test_sampling_parameters_config():
    ...
    solas.config.sampling_start = 50
    solas.config.sampling_cap = 100
    df = pd.read_csv("solas/data/car.csv")
    df._ipython_display_()
    assert df.recommendation["Correlation"][0].data.shape[0] == 100
    solas.config.sampling_cap = 30000
    solas.config.sampling_start = 10000


def test_heatmap_flag_config():
    solas.config.heatmap = True
    df = pd.read_csv("https://raw.githubusercontent.com/lux/solas-datasets/master/data/airbnb_nyc.csv")
    df._ipython_display_()
