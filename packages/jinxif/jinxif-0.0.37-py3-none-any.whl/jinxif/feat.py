...
def filter_cellpose_xy(
        ...
        s_tissue_dapi = 'DAPI1',  # bue 20211003: add to spawn, can be None
        i_tissue_nuc_diam = 30,
        i_tissue_area_thresh = 65536,  # bue 20211003: add to spawna
        # file system
        s_afsubdir = config.d_nconv['s_afsubdir'],  #'./SubtractedRegisteredImages/', or ./RegisteredImages
    ...
    input:
        ...
        i_tissue_nuc_diam: minimum nucleus diameter in pixel that was used for segementation.
    ...
    for s_slide in sorted(es_slide):
        ...
        if not (s_tissue_dapi is None):
            ...
            # for each slide pxscene
            for s_slidepxscene in sorted(df_img_thresh.slide_scene.unique()):

                # load nucleus basin label file
                for s_file_label in sorted(os.listdir(s_segpath)):
                    if s_file_label.startswith(s_slidepxscene):
                        print(f'search nucleus basin label file: {s_file_label} ...')
                        o_match_nuc = re.search(config.d_nconv['s_regex_tiff_celllabel_nuc'], s_file_label)
                        if not (o_match_nuc is None) and (int(o_match_nuc[config.d_nconv['di_regex_tiff_celllabel_nuc']['i_nuc_diam']]) == i_tissue_nuc_diam):
                            # load file
                            print(f'load: {s_file_label}!')
                            ai_label_nuc = io.imread(s_segpath + s_file_label)
                            print(f'nucleus basin label array shape: {ai_label_nuc.shape}')

                # load dapi round 1 treshold and image
                as_file_dapi = df_img_thresh.loc[(df_img_thresh.slide_scene == s_slidepxscene) & (df_img_thresh.marker == s_tissue_dapi), :].index.values
                if (len(as_file_dapi) != 1):
                    sys.exit(f'Error : @ jinxif.feat._detect_tissue_edege_distance : for slide_scene {s_slidepxscene} marker {s_tissue_dapi} more then one file found {as_file_dapi}.')
                s_file_dapi = as_file_dapi[0]
                print(f'load: {s_file_dapi}')
                s_path_dapi = s_format_afsubdir.format(s_afsubdir, s_slidepxscene)
                s_pathfile_dapi = s_path_dapi + s_file_dapi
                i_thresh_dapi = df_img_thresh.loc[s_file_dapi, 'threshold_li']
                ai_dapi = io.imread(s_pathfile_dapi)
                print(f'dapi image shape: {ai_dapi.shape}')

                # generate tissue mask
                ab_dapi = (ai_dapi > i_thresh_dapi)
                print(f'dapi image threshed shape: {ab_dapi.shape}')

                # generate tissue mask based on loaded dapi img dapi threshold and area threshold
                ...
                # get nucleus average distance to cell border
                df_distance_slidescene = pd.DataFrame(np.array([ai_label_nuc.flatten(), ai_dapi.flatten()]).T, columns=['cell_id', 'edge_distance'])
                df_distance_slidescene = df_distance_slidescene.groupby('cell_id').mean().reset_index()
                df_distance_slidescene['slide_scene'] = s_slidepxscene
                df_distance_slidescene['slide_scene_cell'] = df_distance_slidescene.slide_scene + '_cell' + df_distance_slidescene.cell_id.astype(str)
                ...
                s_ofile = config.d_nconv['s_format_tiff_tissueedgedistance'].format(s_slidepxscene, s_tissue_dapi, i_tissue_nuc_diam, i_tissue_area_thresh)
                print('write file:', s_ofile)
                io.imsave(s_segpath + s_ofile, ai_distances)

            # merge
            print('summ edge_distance pre-merge', df_distance_slide.edge_distance.sum())
            df_distance_slide.set_index('slide_scene_cell', inplace=True)
            df_xy_slide = pd.merge(
                df_xy_slide,
                df_distance_slide.edge_distance.to_frame(),
                left_index=True,
                right_index=True,
                how='left',
            )
...
def filter_features(
    ...
    filter_cellpose_xy(
        ...
        i_tissue_nuc_diam = i_tissue_nuc_diam,
...
def filter_features_spawn(
    ...
    for s_slide in sorted(es_slide):
        ...
        s_stream = s_stream.replace('peek_i_tissue_nuc_diam', str(i_tissue_nuc_diam))
