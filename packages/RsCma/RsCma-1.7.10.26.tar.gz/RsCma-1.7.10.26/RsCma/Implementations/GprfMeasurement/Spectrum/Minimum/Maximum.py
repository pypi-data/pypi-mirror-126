...
class Maximum:
	...
	def fetch(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.gprfMeasurement.spectrum.minimum.maximum.fetch() \n
		Queries the result traces calculated with the 'MinPeak' detector. The current, average, minimum and maximum traces can be
		retrieved. \n
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.gprfMeasurement.spectrum.minimum.maximum.read() \n
		Queries the result traces calculated with the 'MinPeak' detector. The current, average, minimum and maximum traces can be
		retrieved. \n
		Use RsCma.reliability.last_value to read the updated reliability indicator. \n
			:return: power: Comma-separated list of 1001 power values Unit: dBm"""
