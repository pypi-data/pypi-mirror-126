...
class Peak:
	...
	def set(self, enable: bool, limit: float) -> None:
		...
		Snippet: driver.configure.vse.measurement.limit.nxdn.ffError.peak.set(enable = False, limit = 1.0) \n
		...
		self._core.io.write(f'CONFigure:VSE:MEASurement<Instance>:LIMit:NXDN:FFERor:PEAK {param}'.rstrip())
	...
	def get(self) -> PeakStruct:
		"""SCPI: CONFigure:VSE:MEASurement<Instance>:LIMit:NXDN:FFERor:PEAK \n
		Snippet: value: PeakStruct = driver.configure.vse.measurement.limit.nxdn.ffError.peak.get() \n
		Enables/disables limit evaluation and sets the upper limit for the peak FSK frequency error. \n
			:return: structure: for return value, see the help for PeakStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:VSE:MEASurement<Instance>:LIMit:NXDN:FFERor:PEAK?', self.__class__.PeakStruct())
