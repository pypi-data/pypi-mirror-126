...
class Rms:
	...
	def set(self, enable: bool, limit: float) -> None:
		...
		Snippet: driver.configure.vse.measurement.limit.dmr.ffError.rms.set(enable = False, limit = 1.0) \n
		...
		self._core.io.write(f'CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:FFERor:RMS {param}'.rstrip())
	...
	def get(self) -> RmsStruct:
		"""SCPI: CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:FFERor:RMS \n
		Snippet: value: RmsStruct = driver.configure.vse.measurement.limit.dmr.ffError.rms.get() \n
		Configures an upper limit for the measured RMS value of the frequency deviation error. \n
			:return: structure: for return value, see the help for RmsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:FFERor:RMS?', self.__class__.RmsStruct())
