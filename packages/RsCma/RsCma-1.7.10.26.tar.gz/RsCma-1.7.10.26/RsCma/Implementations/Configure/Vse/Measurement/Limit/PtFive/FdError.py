...
class FdError:
	...
	def set(self, enable: bool, limit: float) -> None:
		...
		Snippet: driver.configure.vse.measurement.limit.ptFive.fdError.set(enable = False, limit = 1.0) \n
		...
		self._core.io.write(f'CONFigure:VSE:MEASurement<Instance>:LIMit:PTFive:FDERor {param}'.rstrip())
	...
	def get(self) -> FdErrorStruct:
		"""SCPI: CONFigure:VSE:MEASurement<Instance>:LIMit:PTFive:FDERor \n
		Snippet: value: FdErrorStruct = driver.configure.vse.measurement.limit.ptFive.fdError.get() \n
		No command help available \n
			:return: structure: for return value, see the help for FdErrorStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:VSE:MEASurement<Instance>:LIMit:PTFive:FDERor?', self.__class__.FdErrorStruct())
