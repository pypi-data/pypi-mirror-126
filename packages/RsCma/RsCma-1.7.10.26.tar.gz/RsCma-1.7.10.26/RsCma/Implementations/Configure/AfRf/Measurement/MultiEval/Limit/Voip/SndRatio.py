...
class SndRatio:
	...
	def set(self, enable: bool, lower: float, upper: float = None) -> None:
		...
		Snippet: driver.configure.afRf.measurement.multiEval.limit.voip.sndRatio.set(enable = False, lower = 1.0, upper = 1.0) \n
		...
		self._core.io.write(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:VOIP:SNDRatio {param}'.rstrip())
	...
	def get(self) -> SndRatioStruct:
		"""SCPI: CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:VOIP:SNDRatio \n
		Snippet: value: SndRatioStruct = driver.configure.afRf.measurement.multiEval.limit.voip.sndRatio.get() \n
		No command help available \n
			:return: structure: for return value, see the help for SndRatioStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:VOIP:SNDRatio?', self.__class__.SndRatioStruct())
