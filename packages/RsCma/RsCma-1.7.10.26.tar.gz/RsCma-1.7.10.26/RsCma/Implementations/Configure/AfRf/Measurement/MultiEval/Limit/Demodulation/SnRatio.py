...
class SnRatio:
	...
	def set(self, enable: bool, lower: float, upper: float = None) -> None:
		...
		Snippet: driver.configure.afRf.measurement.multiEval.limit.demodulation.snRatio.set(enable = False, lower = 1.0, upper = 1.0) \n
		Configures limits for all SNR results, measured via the RF input path. SNR results include S/N, (S+N) /N and (S+N+D) /N. \n
			...
			:param lower: Lower limit Range: 1 dB to 46 dB, Unit: dB
			:param upper: Upper limit Range: 1 dB to 46 dB, Unit: dB
		"""
		...
		self._core.io.write(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SNRatio {param}'.rstrip())
	...
	def get(self) -> SnRatioStruct:
		"""SCPI: CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SNRatio \n
		Snippet: value: SnRatioStruct = driver.configure.afRf.measurement.multiEval.limit.demodulation.snRatio.get() \n
		Configures limits for all SNR results, measured via the RF input path. SNR results include S/N, (S+N) /N and (S+N+D) /N. \n
			:return: structure: for return value, see the help for SnRatioStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SNRatio?', self.__class__.SnRatioStruct())
