...
class Net:
	"""Net commands group definition. 7 total commands, 2 Sub-groups, 5 group commands"""
	...
	def dns(self):
		...
		if not hasattr(self, '_dns'):
			from .Dns import Dns
			self._dns = Dns(self._core, self._cmd_group)
		return self._dns
	...
	def clone(self) -> 'Net':
		...
		new_group = Net(self._core, self._cmd_group.parent)
