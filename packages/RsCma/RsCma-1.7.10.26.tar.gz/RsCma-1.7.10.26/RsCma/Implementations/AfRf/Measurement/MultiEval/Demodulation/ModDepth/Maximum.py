...
class Maximum:
	...
	def fetch(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.demodulation.modDepth.maximum.fetch() \n
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:DEModulation:MDEPth:MAXimum?', self.__class__.ResultData())

	def read(self) -> ResultData:
		"""SCPI: READ:AFRF:MEASurement<Instance>:MEValuation:DEModulation:MDEPth:MAXimum \n
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.demodulation.modDepth.maximum.read() \n
		Queries the demodulation results for AM demodulation. A statistical evaluation of the modulation depth is returned. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'READ:AFRF:MEASurement<Instance>:MEValuation:DEModulation:MDEPth:MAXimum?', self.__class__.ResultData())
