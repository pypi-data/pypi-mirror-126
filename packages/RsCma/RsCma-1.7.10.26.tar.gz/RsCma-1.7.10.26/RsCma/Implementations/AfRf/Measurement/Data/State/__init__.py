...
class State:
	...
	def fetch(self) -> enums.ResourceState:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:DATA:STATe \n
		Snippet: value: enums.ResourceState = driver.afRf.measurement.data.state.fetch() \n
		Queries the main measurement state. \n
			:return: meas_state: OFF | RUN | RDY | PENDing | ADJusted | QUEued | ACTive | INValid"""
		response = self._core.io.query_str(f'FETCh:AFRF:MEASurement<Instance>:DATA:STATe?')
