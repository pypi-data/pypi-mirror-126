...
class Absolute:
	...
	def fetch(self, trace: enums.Statistic, function: enums.MarkerFunction = None, channel=repcap.Channel.Default, marker=repcap.Marker.Nr1) -> FetchStruct:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:DEModulation<nr>:MARKer<mnr>:ABSolute \n
		Snippet: value: FetchStruct = driver.afRf.measurement.multiEval.fft.demodulation.marker.absolute.fetch(trace = enums.Statistic.AVERage, function = enums.MarkerFunction.MAX, channel = repcap.Channel.Default, marker = repcap.Marker.Nr1) \n
		Queries the absolute coordinates of marker number <mnr>. Select the trace to be evaluated. Optionally, you can perform a
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:DEModulation{channel_cmd_val}:MARKer{marker_cmd_val}:ABSolute? {param}'.rstrip(), self.__class__.FetchStruct())
