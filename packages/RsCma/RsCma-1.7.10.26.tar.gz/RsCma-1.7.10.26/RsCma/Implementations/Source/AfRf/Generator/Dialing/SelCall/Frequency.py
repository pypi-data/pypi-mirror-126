...
class Frequency:
	...
	def reset(self) -> None:
		...
		Snippet: driver.source.afRf.generator.dialing.selCall.frequency.reset() \n
		No command help available \n
		"""
		self._core.io.write(f'SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency:RESet')

	def reset_with_opc(self, opc_timeout_ms: int = -1) -> None:
		"""SCPI: SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency:RESet \n
		Snippet: driver.source.afRf.generator.dialing.selCall.frequency.reset_with_opc() \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency:RESet', opc_timeout_ms)
	...
	def set(self, standard: enums.SelCallStandard, frequency: List[float]) -> None:
		...
		Snippet: driver.source.afRf.generator.dialing.selCall.frequency.set(standard = enums.SelCallStandard.CCIR, frequency = [1.1, 2.2, 3.3]) \n
		Configures the user-defined tone table for a SelCall standard. To enable the table, see method RsCma.Source.AfRf.
		Generator.Dialing.SelCall.UserDefined.enable. \n
		...
		self._core.io.write(f'SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency {param}'.rstrip())

	def get(self, standard: enums.SelCallStandard) -> List[float]:
		"""SCPI: SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency \n
		Snippet: value: List[float] = driver.source.afRf.generator.dialing.selCall.frequency.get(standard = enums.SelCallStandard.CCIR) \n
		Configures the user-defined tone table for a SelCall standard. To enable the table, see method RsCma.Source.AfRf.
		Generator.Dialing.SelCall.UserDefined.enable. \n
			:param standard: CCIR | EEA | EIA | ZVEI1 | ZVEI2 | ZVEI3 | DZVei | PZVei Selects the SelCall standard
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'SOURce:AFRF:GENerator<Instance>:DIALing:SELCall:FREQuency? {param}')
		return response
