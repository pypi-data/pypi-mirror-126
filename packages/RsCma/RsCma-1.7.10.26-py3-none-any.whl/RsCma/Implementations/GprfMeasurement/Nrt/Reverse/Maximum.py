...
class Maximum:
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.gprfMeasurement.nrt.reverse.maximum.calculate() \n
		...
		return self._core.io.query_struct(f'CALCulate:GPRF:MEASurement<Instance>:NRT:REVerse:MAXimum?', self.__class__.CalculateStruct())
	...
	def fetch(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.gprfMeasurement.nrt.reverse.maximum.fetch() \n
		Return the measurement results for the reverse direction.
			INTRO_CMD_HELP: The meaning of the result <Power> depends on the value set via the command method RsCma.Configure.GprfMeasurement.Nrt.Forward.Value.enable: \n
			- FPWR or PEP: <Power> is the reverse power.
			- CFAC or CCDF: <Power> is the forward power.
		...
		return self._core.io.query_struct(f'FETCh:GPRF:MEASurement<Instance>:NRT:REVerse:MAXimum?', self.__class__.ResultData())

	def read(self) -> ResultData:
		"""SCPI: READ:GPRF:MEASurement<Instance>:NRT:REVerse:MAXimum \n
		Snippet: value: ResultData = driver.gprfMeasurement.nrt.reverse.maximum.read() \n
		Return the measurement results for the reverse direction.
			INTRO_CMD_HELP: The meaning of the result <Power> depends on the value set via the command method RsCma.Configure.GprfMeasurement.Nrt.Forward.Value.enable: \n
			- FPWR or PEP: <Power> is the reverse power.
			- CFAC or CCDF: <Power> is the forward power.
		CALCulate commands return error indicators instead of measurement values. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'READ:GPRF:MEASurement<Instance>:NRT:REVerse:MAXimum?', self.__class__.ResultData())
