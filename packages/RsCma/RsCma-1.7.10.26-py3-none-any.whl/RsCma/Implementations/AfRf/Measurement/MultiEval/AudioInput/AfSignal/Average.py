...
class Average:
	...
	def fetch(self, audioInput=repcap.AudioInput.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.audioInput.afSignal.average.fetch(audioInput = repcap.AudioInput.Default) \n
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:AIN{audioInput_cmd_val}:AFSignal:AVERage?', self.__class__.ResultData())
	...
	def read(self, audioInput=repcap.AudioInput.Default) -> ResultData:
		...
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.audioInput.afSignal.average.read(audioInput = repcap.AudioInput.Default) \n
		Queries the AF frequency and level results measured for an AF input path. \n
			:param audioInput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'AudioInput')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		audioInput_cmd_val = self._cmd_group.get_repcap_cmd_value(audioInput, repcap.AudioInput)
		return self._core.io.query_struct(f'READ:AFRF:MEASurement<Instance>:MEValuation:AIN{audioInput_cmd_val}:AFSignal:AVERage?', self.__class__.ResultData())
