...
class Relative:
	...
	def fetch(self, trace: enums.Statistic, function: enums.MarkerFunction = None, audioInput=repcap.AudioInput.Default, markerOther=repcap.MarkerOther.Nr2) -> FetchStruct:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:AIN<nr>:MARKer<mnr>:RELative \n
		Snippet: value: FetchStruct = driver.afRf.measurement.multiEval.fft.audioInput.marker.relative.fetch(trace = enums.Statistic.AVERage, function = enums.MarkerFunction.MAX, audioInput = repcap.AudioInput.Default, markerOther = repcap.MarkerOther.Nr2) \n
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:AIN{audioInput_cmd_val}:MARKer{markerOther_cmd_val}:RELative? {param}'.rstrip(), self.__class__.FetchStruct())
