...
class Average:
	...
	def calculate(self, audioInput=repcap.AudioInput.Default) -> CalculateStruct:
		"""SCPI: CALCulate:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN<Nr>:AVERage \n
		Snippet: value: CalculateStruct = driver.afRf.measurement.multiEval.signalQuality.audioInput.average.calculate(audioInput = repcap.AudioInput.Default) \n
		Queries the signal quality results measured for an AF input path. CALCulate commands return error indicators instead of
		measurement values. \n
		...
		return self._core.io.query_struct(f'CALCulate:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN{audioInput_cmd_val}:AVERage?', self.__class__.CalculateStruct())
	...
	def fetch(self, audioInput=repcap.AudioInput.Default) -> ResultData:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN<Nr>:AVERage \n
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.signalQuality.audioInput.average.fetch(audioInput = repcap.AudioInput.Default) \n
		Queries the signal quality results measured for an AF input path. CALCulate commands return error indicators instead of
		measurement values. \n
			:param audioInput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'AudioInput')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		audioInput_cmd_val = self._cmd_group.get_repcap_cmd_value(audioInput, repcap.AudioInput)
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN{audioInput_cmd_val}:AVERage?', self.__class__.ResultData())

	def read(self, audioInput=repcap.AudioInput.Default) -> ResultData:
		"""SCPI: READ:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN<Nr>:AVERage \n
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.signalQuality.audioInput.average.read(audioInput = repcap.AudioInput.Default) \n
		Queries the signal quality results measured for an AF input path. CALCulate commands return error indicators instead of
		measurement values. \n
			:param audioInput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'AudioInput')
			:return: structure: for return value, see the help for ResultData structure arguments."""
		audioInput_cmd_val = self._cmd_group.get_repcap_cmd_value(audioInput, repcap.AudioInput)
		return self._core.io.query_struct(f'READ:AFRF:MEASurement<Instance>:MEValuation:SQUality:AIN{audioInput_cmd_val}:AVERage?', self.__class__.ResultData())
