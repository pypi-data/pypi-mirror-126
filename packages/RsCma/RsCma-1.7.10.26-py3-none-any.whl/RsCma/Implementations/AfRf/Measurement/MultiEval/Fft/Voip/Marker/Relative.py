...
class Relative:
	...
	def fetch(self, trace: enums.Statistic, function: enums.MarkerFunction = None, markerOther=repcap.MarkerOther.Nr2) -> FetchStruct:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:VOIP:MARKer<mnr>:RELative \n
		Snippet: value: FetchStruct = driver.afRf.measurement.multiEval.fft.voip.marker.relative.fetch(trace = enums.Statistic.AVERage, function = enums.MarkerFunction.MAX, markerOther = repcap.MarkerOther.Nr2) \n
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:VOIP:MARKer{markerOther_cmd_val}:RELative? {param}'.rstrip(), self.__class__.FetchStruct())
