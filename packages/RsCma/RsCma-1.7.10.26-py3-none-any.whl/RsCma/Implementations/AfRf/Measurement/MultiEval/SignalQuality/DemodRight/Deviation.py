...
class Deviation:
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.afRf.measurement.multiEval.signalQuality.demodRight.deviation.calculate() \n
		...
		return self._core.io.query_struct(f'CALCulate:AFRF:MEASurement<Instance>:MEValuation:SQUality:DEMRight:DEViation?', self.__class__.CalculateStruct())
	...
	def fetch(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.signalQuality.demodRight.deviation.fetch() \n
		Queries the signal quality results measured for the right demodulator channel. The commands are only relevant for FM
		stereo. The results are related to the right audio channel. CALCulate commands return error indicators instead of
		measurement values. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:SQUality:DEMRight:DEViation?', self.__class__.ResultData())

	def read(self) -> ResultData:
		"""SCPI: READ:AFRF:MEASurement<Instance>:MEValuation:SQUality:DEMRight:DEViation \n
		Snippet: value: ResultData = driver.afRf.measurement.multiEval.signalQuality.demodRight.deviation.read() \n
		Queries the signal quality results measured for the right demodulator channel. The commands are only relevant for FM
		stereo. The results are related to the right audio channel. CALCulate commands return error indicators instead of
		measurement values. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'READ:AFRF:MEASurement<Instance>:MEValuation:SQUality:DEMRight:DEViation?', self.__class__.ResultData())
