...
class Deviation:
	...
	def fetch(self) -> float:
		...
		Snippet: value: float = driver.afRf.measurement.multiEval.demodLeft.afSignal.deviation.fetch() \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:DEMLeft:AFSignal:DEViation?', suppressed)
		return Conversions.str_to_float(response)

	def read(self) -> float:
		"""SCPI: READ:AFRF:MEASurement<Instance>:MEValuation:DEMLeft:AFSignal:DEViation \n
		Snippet: value: float = driver.afRf.measurement.multiEval.demodLeft.afSignal.deviation.read() \n
		Queries the AF frequency results for the left demodulator channel. For FM stereo, these results are related to the left
		audio channel. \n
		Use RsCma.reliability.last_value to read the updated reliability indicator. \n
			:return: frequency: Frequency of the AF signal Unit: Hz"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:AFRF:MEASurement<Instance>:MEValuation:DEMLeft:AFSignal:DEViation?', suppressed)
		return Conversions.str_to_float(response)
