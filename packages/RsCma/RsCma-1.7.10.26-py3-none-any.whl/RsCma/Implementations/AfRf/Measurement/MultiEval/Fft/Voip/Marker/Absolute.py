...
class Absolute:
	...
	def fetch(self, trace: enums.Statistic, function: enums.MarkerFunction = None, marker=repcap.Marker.Nr1) -> FetchStruct:
		"""SCPI: FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:VOIP:MARKer<mnr>:ABSolute \n
		Snippet: value: FetchStruct = driver.afRf.measurement.multiEval.fft.voip.marker.absolute.fetch(trace = enums.Statistic.AVERage, function = enums.MarkerFunction.MAX, marker = repcap.Marker.Nr1) \n
		...
		return self._core.io.query_struct(f'FETCh:AFRF:MEASurement<Instance>:MEValuation:FFT:VOIP:MARKer{marker_cmd_val}:ABSolute? {param}'.rstrip(), self.__class__.FetchStruct())
