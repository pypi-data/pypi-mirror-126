...
class Sinad:
	...
	def set(self, enable: bool, lower: float, upper: float = None) -> None:
		...
		Snippet: driver.configure.afRf.measurement.multiEval.limit.demodulation.sinad.set(enable = False, lower = 1.0, upper = 1.0) \n
		...
		self._core.io.write(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SINad {param}'.rstrip())
	...
	def get(self) -> SinadStruct:
		"""SCPI: CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SINad \n
		Snippet: value: SinadStruct = driver.configure.afRf.measurement.multiEval.limit.demodulation.sinad.get() \n
		Configures limits for the SINAD results, measured via the RF input path. \n
			:return: structure: for return value, see the help for SinadStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:AFRF:MEASurement<Instance>:MEValuation:LIMit:DEModulation:SINad?', self.__class__.SinadStruct())
