...
class Rms:
	...
	def set(self, enable: bool, limit: float) -> None:
		...
		Snippet: driver.configure.vse.measurement.limit.dmr.merror.rms.set(enable = False, limit = 1.0) \n
		...
		self._core.io.write(f'CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:MERRor:RMS {param}'.rstrip())
	...
	def get(self) -> RmsStruct:
		"""SCPI: CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:MERRor:RMS \n
		Snippet: value: RmsStruct = driver.configure.vse.measurement.limit.dmr.merror.rms.get() \n
		Configures an upper limit for the measured RMS value of the magnitude error. \n
			:return: structure: for return value, see the help for RmsStruct structure arguments."""
		return self._core.io.query_struct(f'CONFigure:VSE:MEASurement<Instance>:LIMit:DMR:MERRor:RMS?', self.__class__.RmsStruct())
