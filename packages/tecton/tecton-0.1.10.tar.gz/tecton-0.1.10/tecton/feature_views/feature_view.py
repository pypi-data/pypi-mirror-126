...
class MaterializedFeatureView(FeatureDefinition):
    ...
    def run(
        ...
        **mock_inputs: DataFrame,
        ...
        """
        if aggregate_tiles and not self._args.HasField("temporal_aggregate_args"):
            raise FV_UNSUPPORTED_ARG("aggregate_tiles")
        ...
        if aggregate_tiles:
            return construct_partial_time_aggregation_df(
                ...
                time_aggregation=self._construct_trailing_time_window_aggregation(),
                version=None,  # `version` is unused, since convert_to_epoch=False.
    ...
    def _construct_trailing_time_window_aggregation(self):
        ...
        for feature_aggregation in temporal_agg_args.aggregations:
            aggregation.features.append(self._create_partial_aggregate_feature(feature_aggregation))
        return aggregation

    @classmethod
    def _create_partial_aggregate_feature(cls, feature_aggregation: FeatureAggregationProto) -> AggregateFeature:
        """Build a simple AggregateFeature with the fields required for partial time aggregation."""
        feature = AggregateFeature()
        feature.input_feature_name = feature_aggregation.column
        feature.function = AggregationFunction.Value(f"AGGREGATION_FUNCTION_{feature_aggregation.function.upper()}")
