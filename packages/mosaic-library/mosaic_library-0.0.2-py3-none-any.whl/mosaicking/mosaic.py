...
def main():
    ...
    try:  # External try to handle any unexpected errors
        ...
        while not utils.evaluate_stopping(reader, args):
            ...
            # Get the ROI of the area containing the new mask
            rect = cv2.boundingRect(image_mask)
            # Crop the image mask
            image_mask = image_mask[rect[1]:rect[1]+rect[3], rect[0]:rect[0]+rect[2]]
            # Crop the image to the same mask
            img = img[rect[1]:rect[1] + rect[3], rect[0]:rect[0] + rect[2]]
            # Re-position the src_points to the ROI
            #src_pts = src_pts - (rect[0], rect[1])
            # for p, k in zip(src_pts, kp):
            #     k.pt = p.squeeze()
            ...
            #mosaic_img = preprocessing.crop_to_valid_area(mosaic_img)
            #mosaic_mask = preprocessing.crop_to_valid_area(mosaic_mask)
