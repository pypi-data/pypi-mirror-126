...
class InteractionClient:
    ...
    def buttonClick(self, custom_id=None):
        def Rdecorator(func):
            ...
            if custom_id:
                self.add_listener("button",custom_id, func)
            else:
                self.add_listener("button",func.__name__, func)
    ...
    def selectClick(self, custom_id=None):
        def Rdecorator(func):
            ...
            if custom_id:
                self.add_listener("menu", custom_id, func)
            else:
                self.add_listener("menu", func.__name__, func)
    ...
    def userCommand(self, name=None):
        def Rdecorator(func):
            ...
            if name:
                self.add_listener("user", name, func)
            else:
                self.add_listener("user", func.__name__, func)
    ...
    def msgCommand(self, name=None):
        def Rdecorator(func):
            ...
            if name:
                self.add_listener("msg", name, func)
            else:
                self.add_listener("msg", func.__name__, func)
    ...
    def slashCommand(self, name=None):
        def Rdecorator(func):
            ...
            if name:
                self.add_listener("slash", name, func)
            else:
                self.add_listener("slash", func.__name__, func)
    ...
    # 리스너 추가 
    def add_listener(self, type, event, func):
        if type == "button":
    ...
    # User Command 추가 요청
    async def add_userCommand(self, name):
        route = Route("POST", f"/applications/{self.appId}/guilds/829256103791427624/commands")
        route.url = f"https://discord.com/api/v9/applications/{self.appId}/guilds/829256103791427624/commands"
    ...
    # Msg Command 추가 요청
    async def add_msgCommand(self, name):
        route = Route("POST", f"/applications/{self.appId}/guilds/829256103791427624/commands")
        route.url = f"https://discord.com/api/v9/applications/{self.appId}/guilds/829256103791427624/commands"
    ...
    async def remove_Command(self, id):
        route = Route("DELETE", f"/applications/{self.appId}/guilds/829256103791427624/commands/{id}")
        route.url = f"https://discord.com/api/v9/applications/{self.appId}/guilds/829256103791427624/commands/{id}"

        await self.bot.http.request(route)

    async def add_slashCommand(self, slash: SlashCommand):
        route = Route("POST", f"/applications/{self.appId}/guilds/829256103791427624/commands")
        route.url = f"https://discord.com/api/v9/applications/{self.appId}/guilds/829256103791427624/commands"
