...
Monotonic WOE Encoding Function

Missing Value Convention:

All numeric missing values set to:
-99XXXX where XXXX is a customized enum
default: -990000

All categorical missing values set to:
NA_XXXX where XXXX is a customized tag
default: NA
...
class WOE(TransformerMixin):
    ...
    def fit(self, df_xtrain, df_ytrain, **kwargs):
        ...
        :param df_xtrain: training features
        :param df_ytrain: training label
        :param kwargs:
               feature_list
               categorical_features
               missing_values
               missing_logic: high_risk, low risk

        :return: fitted WOE model
        """

        # get params
        self.feature_list = self.__get_feature_list(df_xtrain, **kwargs)
        ...
        df_train['label'] = df_ytrain.copy(deep=True)
        ...
        # fit each categorical or numerical feature
        lst_bin = list()
        for c in tqdm(self.feature_list):

            start = time.time()
            if c in self.categorical_features:
                df_bin = self.__fit_categorical(df_train, c)
            else:
                df_bin = self.__fit_numerical(df_train, c)
            finish = time.time()

            df_bin['time'] = finish - start
            lst_bin.append(df_bin)

        # combine the fitted bins
    ...
    def transform(self, df_xtest, **kwargs):
        ...
        feature_list = self.__get_feature_list(df_trans, **kwargs)
        ...
        for c in tqdm(feature_list):
            ...
            if c in self.categorical_features:
                # seperate into missing and data parts
                ...
                # set default woe value according to missing logic
                if method == 'woe':
                ...
                # set default categorical missing value: NA without missing logic
                df_trans.loc[:, c] = df_trans.loc[:, c].fillna(Config.CAT_MISSING)

                # transform the bin/woe value according to method = bin or woe
            ...
            # transform numerical feature
                ...
                # set default numerical missing value: -990000 without missing logic, woe accuracy to 4 digits
                df_trans.loc[:, c] = df_trans.loc[:, c].fillna(Config.NUM_MISSING)
                df_trans.loc[:, c] = df_trans.loc[:, c].apply(lambda x: round(x, 4))

                # transform the bin/woe value according to method = bin or woe
                for idx, row in df_data.iterrows():
                    mask = (float(row['min']) < df_trans[c]) & (df_trans[c] <= float(row['max']))
                    df_trans_res.loc[mask, c] = row[method]
    ...
    def __fit_numerical(self, df, c):
        # split into missing and data
        df_missing, df_data = self.__split_data_by_missing(df, c)

        # stat on missing part
        stat_missing = self.__stat_feature_by_good_and_bad(df_missing, c, 'label')

        # stat on data part
        df_data.loc[:, c] = df_data.loc[:, c].apply(lambda x: round(x, 4))
        stat_data = self.__stat_feature_by_good_and_bad(df_data, c, 'label')

        # combine missing and data in to a bin
        df_bin = self.__merge_missing_and_data_bin(stat_missing, stat_data, c)

        return df_bin

    def __fit_categorical(self, df, c):
        # split into missing and data
        df_missing, df_data = self.__split_data_by_missing(df, c)
    ...
    @classmethod
    def __get_feature_list(cls, df, **kwargs):
    ...
    def __split_data_by_missing(self, df, c):
        df_missing = df.loc[df[c].apply(lambda x: self.__is_missing(x))]
        df_data = df.loc[df[c].apply(lambda x: not self.__is_missing(x))]

        if c in self.categorical_features:
            df_missing.loc[:, c] = df_missing.loc[:, c].fillna(Config.CAT_MISSING)
        else:
            df_missing.loc[:, c] = df_missing.loc[:, c].fillna(Config.NUM_MISSING)

        return df_missing, df_data

    def __is_missing(self, x):
    ...
    @classmethod
    def __stat_feature_by_good_and_bad(cls, df, c, label):
        """
        get stat dataframe from a feature and label with following format:
        ----------------
        var bin good bad
        age 18  20   5
        age 20  30   2
    ...
    def __merge_missing_and_data_bin(self, stat_missing, stat_data, c):
        """
        get stat dataframe with following format:
        ----------------
        var    type           bin      min     max       woe     iv      total  ratio  bad  bad_rate
        gender cat_missing    NA       NA      NA        0.0000  0.0000  10     0.05   1    0.1
        gender cat_data       Male     Male    Male      0.1234  0.1234  10     0.05   1    0.1
        age    num_missing    -990000  -990000 -990000   0.0000  0.0000  10     0.05   1    0.1
        age    num_data       [18, 30) 18      30        0.1234  0.1234  10     0.05   1    0.1
        ----------------
        """
        ...
        # stat categorical feature
        if c in self.categorical_features:
            # stat when exists missing part
            if len(stat_missing) > 0:
                good_missing = stat_missing['good'].sum()
                bad_missing = stat_missing['bad'].sum()

                bin_missing = self.__stat_and_merge_bin_list(stat_missing, stat_missing.index.tolist(),
                                                             'cat_missing', good_data, bad_data)
            # stat when does not exists missing part
            else:
                good_missing = 0
                bad_missing = 0
                bin_missing = pd.DataFrame([{'var': c, 'type': 'cat_missing', 'bin': Config.CAT_MISSING,
                                             'min': Config.CAT_MISSING, 'max': Config.CAT_MISSING,
                                             'woe': -np.inf, 'iv': 0,
                                             'total': 0, 'ratio': 0, 'bad': 0, 'bad_rate': 0
                                             }])
            # stat categorical data part
            bin_data = self.__stat_and_merge_bin_list(stat_data, stat_data.index.tolist(),
                                                      'cat_normal', good_missing, bad_missing)
        # stat numerical feature
        else:
            # stat when exists missing part
            if len(stat_missing) > 0:
                good_missing = stat_missing['good'].sum()
                ...
                bin_missing = self.__stat_and_merge_bin_list(stat_missing, stat_missing.index.tolist(),
                                                             'num_missing', good_data, bad_data)
            # stat when does not exists missing part
            else:
                good_missing = 0
                bad_missing = 0
                bin_missing = pd.DataFrame([{'var': c, 'type': 'num_missing', 'bin': Config.NUM_MISSING,
                                             'min': Config.NUM_MISSING, 'max': Config.NUM_MISSING,
                                             'woe': -np.inf, 'iv': 0,
                                             'total': 0, 'ratio': 0, 'bad': 0, 'bad_rate': 0
                                             }])

            # stat numerical data part
            total = good_data + bad_data + good_missing + bad_missing
            knots = self.init_knots(stat_data, total, self.nbins)
            knots = self.__combine_bins(stat_data, self.nbins, knots)
            bin_data = self.__stat_and_merge_bin_list(stat_data, knots,
                                                      'num_normal', good_missing, bad_missing)

        # combine missing and data
        ...
        # add woe max and min for missing value filling
    ...
    @classmethod
    def __stat_and_merge_bin_list(cls, stat, knots, bin_type, good, bad):
        var = stat['var'][0]
        total_good = stat['good'].sum() + good
        total_bad = stat['bad'].sum() + bad

        lst_df, lst_bin, lst_min, lst_max = list(), list(), list(), list()
        # combine bin when it is categorical bin or missing bin
        ...
        # combine bin when it is numerical bin
        else:
            if len(knots) == 2:
        ...
        # merge bin list
        df_bin = cls.__merge_bin_list(var, bin_type, lst_df, lst_bin, lst_min, lst_max, total_good, total_bad)

        return df_bin

    @classmethod
    def __merge_bin_list(cls, var, bin_type, lst_df, lst_bin, lst_min, lst_max, total_good, total_bad):
        """
        get stat dataframe with following format:
        ----------------
        var    type           bin      min     max       woe     iv      total  ratio  bad  bad_rate
        ...
        lst_rate = list(map(lambda x: float(sum(x['bad'])) / (sum(x['good']) + sum(x['bad'])), lst_df))
        ...
        if bin_type == 'cat_missing':
            if Config.CAT_MISSING not in lst_bin:
                print(f'add edge case: None in {var}')
                lst_bin.append(Config.CAT_MISSING)
                lst_min.append(Config.CAT_MISSING)
                lst_max.append(Config.CAT_MISSING)
                lst_woe.append(0)
                lst_iv.append(0)
                lst_total.append(0)
                lst_ratio.append(0)
            ...
            if f'{float(Config.NUM_MISSING)}' not in lst_bin and f'{Config.NUM_MISSING}' not in lst_bin and Config.NUM_MISSING not in lst_bin:
                print(f'add edge case: None in {var}')
                lst_bin.append(Config.NUM_MISSING)
                lst_min.append(Config.NUM_MISSING)
                lst_max.append(Config.NUM_MISSING)
        ...
        df_bin = pd.DataFrame({'var': var, 'type': bin_type, 'bin': lst_bin,
                               'min': lst_min, 'max': lst_max,
                               'woe': lst_woe, 'iv': lst_iv,
                               'total': lst_total, 'ratio': lst_ratio, 'bad': lst_bad, 'bad_rate': lst_rate})
        return df_bin

    def init_knots(self, stat_bin, total, max_iter):
        knots = self.__get_best_knots_helper(stat_bin, total, max_iter, 0, len(stat_bin), 0)
        knots = list(filter(lambda x: x is not None, knots))
        knots.sort()

        return knots

    def __combine_bins(self, stat, max_nbins, knots):
        max_nbins = min(max_nbins, len(knots) + 1)
        if max_nbins == 1:
            return [0, len(stat) - 1]
        for cur_nbins in sorted(range(2, max_nbins + 1), reverse=True):
            new_knots = self.__combine_helper(stat, cur_nbins, knots)
    ...
    def __combine_helper(self, stat, nbins, knots):
        lst_knots = list(combinations(knots, nbins - 1))

        knots_list = list(map(lambda x: sorted(x + (0, len(stat) - 1)), lst_knots))
        lst_iv = list(map(lambda x: self.__eval_iv_mono(stat, x), knots_list))
    ...
    @classmethod
    def __eval_iv_mono(cls, stat, knots):
        lst_df = []
        for i in range(1, len(knots)):
            if i == 1:
                lst_df.append(stat.loc[knots[i - 1]:knots[i]])
    ...
    def __get_best_knots_helper(self, stat_bin, total, max_iter, left, right, cur_iter):
        ...
        best_knot = self.__get_best_knot_by_ks(stat_bin, total, left, right)

        if best_knot is not None:
            left_knots = self.__get_best_knots_helper(stat_bin, total, max_iter, left, best_knot, cur_iter + 1)
            right_knots = self.__get_best_knots_helper(stat_bin, total, max_iter, best_knot + 1, right, cur_iter + 1)
        else:
            left_knots = []
            right_knots = []
        return left_knots + [best_knot] + right_knots

    def __get_best_knot_by_ks(self, stat_bin, total, left, right):
        stat = stat_bin.loc[left:right]
        total_cur = stat['good'].sum() + stat['bad'].sum()
    ...
    def __adjust_woe(self, bin_info):
        ...
        mask = ((bin_info['ratio'] < self.min_bin_rate) | (bin_info['total'] < self.min_bin_size)) & \
               (bin_info['woe'] != -np.inf) & (bin_info['type'].isin(['num_normal', 'cat_normal']))
        bin_info.loc[mask, 'woe'] = 0

        mask = (bin_info['bad'] < self.min_missing_bad_cnt) & (bin_info['woe'] != -np.inf) & (
        ...
        for c in self.feature_list:
            if c in self.categorical_features:
                ...
                if self.missing_logic[c] == 'high_risk':
                    val = bin_info.loc[(bin_info['var'] == c) & (bin_info['type'] == 'cat_normal'), 'woe'].min()
                elif self.missing_logic[c] == 'low_risk':
                    val = bin_info.loc[(bin_info['var'] == c) & (bin_info['type'] == 'cat_normal'), 'woe'].max()
                else:
                    val = 0
                ...
                if self.missing_logic[c] == 'high_risk':
                    val = bin_info.loc[(bin_info['var'] == c) & (bin_info['type'] == 'num_normal'), 'woe'].min()
                elif self.missing_logic[c] == 'low_risk':
                    val = bin_info.loc[(bin_info['var'] == c) & (bin_info['type'] == 'num_normal'), 'woe'].max()
                else:
                    val = 0
                bin_info.loc[mask, 'woe'] = val
    ...
    def update(self, sr_xtrain, sr_ytrain, ws, **kwargs):
        """
        ...
        origin_feature_missing_logic = self.missing_logic[sr_xtrain.name]
        feature_missing_logic = kwargs.get('missing_logic', origin_feature_missing_logic)
        if feature_missing_logic is not None:
            self.missing_logic[sr_xtrain.name] = feature_missing_logic

        df_train = pd.DataFrame({sr_xtrain.name: sr_xtrain, 'label': sr_ytrain})

        df_missing, df_data = self.__split_data_by_missing(df_train, sr_xtrain.name)
        stat_missing = self.__stat_feature_by_good_and_bad(df_missing, sr_xtrain.name, 'label')

        bin_info = self.bin_info
        bin_rest = bin_info[bin_info['var'] != sr_xtrain.name]
        bin_update = self.__refit_numerical_feature(df_train, sr_xtrain.name, ws)
        ...
        if len(stat_missing) > 0:
            ...
            bin_missing = pd.DataFrame([{'var': sr_xtrain.name, 'type': 'num_missing', 'bin': Config.NUM_MISSING,
                                         'min': Config.NUM_MISSING, 'max': Config.NUM_MISSING,
                                         'woe': -np.inf, 'iv': 0,
                                         'total': 0, 'ratio': 0, 'bad': 0, 'bad_rate': 0
                                         }])
    ...
    def __refit_numerical_feature(self, df, c, ws):
        df_missing, df_data = self.__split_data_by_missing(df, c)
        ...
        stat_data = df_data.groupby('bin').agg(good=('label', lambda x: x.count() - x.sum()),
                                               bad=('label', lambda x: x.sum())).reset_index()
        stat_data['var'] = c

        print(stat_missing)
        print(stat_data)

        df_bin = self.__update_missing_and_data_bin(stat_missing, stat_data)
        return df_bin

    def __update_missing_and_data_bin(self, stat_missing, stat_data):
        ...
        bin_data = self.__stat_bin_update(stat_data,
                                          'num_normal', good_missing, bad_missing)
    ...
    def update_by_insert_value(self, sr_xtrain, sr_ytrain, val, **kwargs):
        """
        ...
        bin_list = self.export()[sr_xtrain.name]['data']
        bin_list.append(val)
        bin_list = sorted(set(bin_list))
        self.update(sr_xtrain, sr_ytrain, bin_list, **kwargs)
        return self
