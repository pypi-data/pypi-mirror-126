...
def get_datetime(
    ...
    *,
    sub: t.Union[tuple[str, str], list[tuple[str, str]]] = None,
    fmt: t.Union[str, list[str]] = None,
    ...
    def adjust_timezone(dt_obj: datetime):
    ...
    if isinstance(value, (int, float)):
        return adjust_timezone(datetime.fromtimestamp(value, tz=tz))
    if isinstance(value, datetime):
        return adjust_timezone(value)
    # apply substitutions if specified
    sub = sub or []
    if len(sub) == 2 and isinstance(sub[0], str):
        sub = [t.cast(tuple[str, str], sub)]
    for pattern, repl in t.cast(list[tuple[str, str]], sub):
        value = re.sub(pattern, repl, value)
    # parse with dateutil parser when no format is specified
    if not fmt:
        default = datetime(1970, 1, 1, 0, 0, 0)
        return adjust_timezone(dt_parser.parse(value, default=default))
    # parse with strptime when formats are specified, use the first match
    fmt = [fmt] if isinstance(fmt, str) else (fmt or [])
    for f in fmt:
        try:
            return adjust_timezone(datetime.strptime(value, f))
        except ValueError:
            pass
    raise ValueError(f"{value!r} doesn't match any of the given formats: {fmt!r}")
