...
def get_price_ak_cn(ticker,fromdate,todate,adjust='none'):
    ...
    result,start,end=check_period(fromdate,todate)
    if not result:
        print("  #Error(get_price_ak_cn): invalid date period from",fromdate,'to',todate)
        return None   
    ...
    if adjust not in adjustlist:
        print("  #Error(get_price_ak_cn): adjust only supports",adjustlist)
        return None          
