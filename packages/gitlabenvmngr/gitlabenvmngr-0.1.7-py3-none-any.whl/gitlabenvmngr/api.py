...
class Project:
    ...
    def copy_env(self, from_env, to_env):
        for variable in self.__variables:
            ...
            if env_value != None:
                self.create_variable(var_name = variable, var_env = to_env, var_value = env_value)
    ...
    def delete_env(self, env):
        ...
        with open(f"{self.__project_name.replace(' ', '')}_{env}.deleted.env", "w") as env_file:
            for variable in self.__variables:
                ...
                if env_value != None:
                    env_file.write(f"{self.__variables[variable].name}={env_value}\n")
                    print(f"Deleting: {self.__variables[variable].name} on {env}")
                    self.__variables[variable].delete_env(env)
        self.__variables = {}
