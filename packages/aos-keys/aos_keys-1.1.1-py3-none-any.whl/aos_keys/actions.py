...
def __check_can_create(output_directory: str) -> bool:
    """Check if directory can be created"""
    Path(output_directory).mkdir(parents=True, exist_ok=True)
    can_create = True
    console = Console()
    for file_name in (_OEM_FILE_NAME, _SP_FILE_NAME):
        file = Path(output_directory) / file_name
        if file.exists():
            can_create = False
            console.print(f'File [{file}] exists. Cannot proceed!', style="bold red")
...
def new_token_user(domain: str, output_directory: str, auth_token: str, user_type: UserType, create_ecc_key):
    console = Console()
    Path(output_directory).mkdir(parents=True, exist_ok=True)
    if not __check_can_create(output_directory=output_directory):
        sys.exit(1)
