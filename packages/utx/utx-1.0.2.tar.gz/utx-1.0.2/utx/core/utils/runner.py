...
from airtest_selenium.exceptions import AirtestSeleniumException
...
def run_web_air(driver, home_url, case, log_path, case_path, base_dir, static_dir=''):
    """

    :param driver:
    :param home_url:首页URL
    :param case:
    :param log_path:
    :param case_path:
    :param base_dir:
    ...
    if f.endswith(".air"):
        ...
        try:
            ...
            rpt = report.LogToHtml(script, run_log, script_name=f.replace(".air", ".py"), static_root=static_dir,
                                   plugins=['airtest_selenium.report', 'poco.utils.airtest.report'])
            rpt.report("log_template.html", output_file=output_file, base_dir=base_dir)
            result = {"name": airName.replace('.air', ''), "result": rpt.test_result}
