...
rds.DatabaseInstance(self, "InstanceWithCustomizedSecret",
    ...
    credentials=rds.Credentials.from_generated_secret("postgres",
        ...
        exclude_characters="!&*^#@()",
        replica_regions=[ReplicaRegion(region="eu-west-1"), ReplicaRegion(region="eu-west-2")]
    )
)
```
...
instance.add_rotation_single_user(
    vpc_subnets=SubnetSelection(subnet_type=ec2.SubnetType.PRIVATE_WITH_NAT),  # Place rotation Lambda in private subnets
