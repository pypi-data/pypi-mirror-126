...
def test_load_superwise_login_user_password():
    ...
    try:
        ...
        print(config.get("CLIENT_NAME"))
        sw = Superwise(email=email, password=password, client_name=config.get("CLIENT_NAME"))
    except:
        ok = False
    assert ok is True
...
def test_load_superwise_object_login():
    ok = True
    try:
        sw = Superwise(client_name=config["CLIENT_NAME"], client_id=config["CLIENT_ID"], secret=config["SECRET"])
    except:
        ok = False
    assert ok is True
...
def test_invalid_token():
    ok = False
    try:
        sw = Superwise(client_name="test", client_id="test", secret="test_fail")
