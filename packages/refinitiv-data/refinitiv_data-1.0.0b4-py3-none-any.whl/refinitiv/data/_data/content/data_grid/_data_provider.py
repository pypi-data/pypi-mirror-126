...
from .data_grid_type import use_field_names_in_headers_arg_parser, DataGridType
...
def convert_json_rdp_to_pandas(
    ...
    name_title = "title" if use_title else "name"
...
def convert_json_udf_to_pandas(
    ...
    data = np.array([list(map(get_value, row)) for row in json_data["data"]])
    if len(data):
...
class DataGridDataProvider(DataProvider):
    def _monkey_patch_args(self, session: "Session", data_grid_type: "DataGridType"):
        if not isinstance(data_grid_type, DataGridType):
            raise ValueError("not valid type argument data_grid_type")

        if session.type == SessionType.PLATFORM and data_grid_type == DataGridType.UDF:
            session.warning(
                f"UDF DataGrid service cannot be used with platform sessions. "
                f"The \"/apis/data/datagrid/underlying-platform = '{data_grid_type.value}'\" "
        ...
        data_grid_provider = data_grid_type_by_data_grid_data_provider.get(
            data_grid_type
        )
        self.request = data_grid_provider.request
        self.response = data_grid_provider.response
        self.validator = data_grid_provider.validator

    async def get_data_async(self, *args, **kwargs):
        session: "Session" = args[0]
        data_grid_type: "DataGridType" = kwargs.get("data_grid_type")

        self._monkey_patch_args(session, data_grid_type)
        return await super().get_data_async(*args, **kwargs)


data_grid_data_provider = DataGridDataProvider()

_data_grid_rdp_data_provider = DataProvider(
    request=DataGridRDPRequestFactory(),
    response=DataGridRDPResponseFactory(),
    validator=DataGridRDPContentValidator(),
)

_data_grid_udf_data_provider = DataProvider(
...
data_grid_type_by_data_grid_data_provider = {
    DataGridType.RDP: _data_grid_rdp_data_provider,
    DataGridType.UDF: _data_grid_udf_data_provider,
}
