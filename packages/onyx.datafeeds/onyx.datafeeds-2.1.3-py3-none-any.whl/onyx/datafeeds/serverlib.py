...
# -----------------------------------------------------------------------------
def process_response(resp, compress):
    dtype = resp["type"]
    if dtype == "Curve":
    ...
    if compress:
        # --- serialize to json string, convert to bytes, compress and use
        #     base64 to get back a ascii string
        payload = json.dumps(resp["payload"], separators=(",", ":")) + "\n"
        payload = zlib.compress(bytes(payload, "utf-8"))
        resp["payload"] = base64.encode(payload).decode("ascii")
...
class DataServer(tornado.tcpserver.TCPServer):
    ...
    async def handle_stream(self, stream, address):
        ...
        compress = request.pop("compress", False)
        ...
        resp = process_response(resp, compress)
