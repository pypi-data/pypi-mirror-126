...
def get_price(code: str or list,
    ...
    if frequency in ['d1', 'd1_raw', 'd1_post']:
        ...
        for s, e in zip(years[:-1], years[1:]):
            y_sql = f"select * from {region}_{frequency}.{region}_st_{frequency} where ({code_sql}) and (time <= '{s}-01-01 00:00:00' and time <'{e}-01-01 00:00:00');"
            sql_list.append(y_sql)
        sql_list.append(f"select * from {region}_{frequency}.{region}_st_{frequency} where ({code_sql}) and (time <= '{years[-1]}-01-01 00:00:00' and time <= '{str(end_date)}');")
    else:
        sql_list = [f"select * from {region}_{frequency}.{region}_st_{frequency} where ({code_sql}) and (time between '{str(start_date)}' and '{str(end_date)}');"]
