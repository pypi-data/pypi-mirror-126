...
def run_periodic_iterate(
    ...
    query = f"""
    CALL apoc.periodic.iterate("{cypherIterate}","{cypherAction}",{{{",".join(config_map)}}}) 
    """
    log.debug(query)
    res = graph.run(query)
    ...
    if res_data["failedOperations"][0] != 0 and not _ignore_erros:
        ...
        raise Neo4jPeriodicIterateError(
            f'Error on {res_data["failedOperations"][0]} of {res_data["total"][0]} operations. ErrorMessages:\n{":".join(errors)}'
        )
    return res
