...
class Observations:
    ...
    def read(self, record_type, record_id): 
        ...
        file_name = record_id.replace('___', '/') + '.json'
        file_full_path = file_path + '/' + file_name

        # Error handling if no file
        ...
        if not self._observations_on_file.get(record_type):
            self._observations_on_file[record_type] = {}
        self._observations_on_file[record_type][record_id] = data
    ...
    def write(self, record_type, record_id):
        ...
        # Skip save if same as loaded
        if self._observations_on_file.get(record_type, {}).get(record_id, None) == data:
            return
