...
def get_default_root_dir():
    ...
    return os.path.join(sys.prefix, 'share', 'JSBSim')
...
cdef class FGFDMExec(FGJSBBase):
    ...
    def __cinit__(self, root_dir, FGPropertyManager pm_root=None):
        ...
        if root_dir:
            ...
            if os.path.isdir(default_root_dir):
                self.set_root_dir(default_root_dir)
            else:
                raise IOError("Can't find the default root directory: {0}".format(default_root_dir))
