...
def create_save_file(path, use_lstm_pred=False):
    with open(path, 'w') as file:
        ...
        writer_parameter = ['Sentence', 'POS Tagging', 'NER', 'LSTM'] if use_lstm_pred else ['Sentence', 'POS Tagging', 'NER']
...
def save_predict(path, sequence, pred, lstm=None):
    with open(path, 'a') as file:
        writer = csv.writer(file, delimiter='|', lineterminator='\n')
        if lstm is not None:
            writer_parameter = zip([' '.join(k) for k in sequence[0]], [' '.join(k) for k in sequence[1]],
                                   [' '.join(k) for k in pred], [' '.join(k) for k in lstm])
        else:
            writer_parameter = zip([' '.join(k) for k in sequence[0]], [' '.join(k) for k in sequence[1]],
