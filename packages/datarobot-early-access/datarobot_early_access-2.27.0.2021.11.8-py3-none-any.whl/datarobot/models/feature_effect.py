...
class FeatureEffectsMulticlass(APIObject):
    ...
    def create(
        ...
        top_n_classes : int or None
            Number of top classes (ranked by class record counts) used to calculate Feature Effects.
        classes : list or None
            The list of classes to use to calculate feature effects.
        ...
        """
        if not ((classes is None) ^ (top_n_classes is None)):
            raise ValueError("Either 'classes' or 'top_n_classes' must be provided, but not both.")
        ...
        if top_n_classes:
            payload["topNClasses"] = top_n_classes
        if classes:
