...
class BlockMatchStorage:
    match_positions: list
    match_positions_shift: list
    group_sizes: list
    total_size: int
...
class BlockMatchStorageCtype(ctypes.Structure):
    """
...
ARGTYPES_THR = [ctypes.POINTER(ctypes.c_float),  # sourceImage
                ...
                ctypes.POINTER(BlockMatchStorageCtype)
                ]
...
def get_blockmatch_storage(blockmatches: Union[BlockMatchStorage, bool], stack_size: int, block_count: int):
    if isinstance(blockmatches, BlockMatchStorage):
        storage = BlockMatchStorageCtype()
        storage.status = ctypes.c_int(1)
        storage.maxStackSize = stack_size
        storage.stackSizes = conv_to_array(np.array(blockmatches.group_sizes), ctypes.c_int)

        block_positions = []
        shift_positions = []
        for i in range(0, blockmatches.total_size):
            block_positions.append(create_vec3i_arr(blockmatches.match_positions[i]))
            shift_positions.append(create_vec3i_arr(blockmatches.match_positions_shift[i]))

        storage.blockPositions = conv_to_array(block_positions, Vec3i)
        storage.shiftedPositions = conv_to_array(shift_positions, Vec3i)
        storage.currentStack = ctypes.c_int(0)
        return storage
    elif blockmatches:
        storage = BlockMatchStorageCtype()
        ...
        block_positions = []
        shift_positions = []
        for i in range(0, block_count * stack_size):
            block_positions.append(create_vec3i_arr([0, 0, 0]))
            shift_positions.append(create_vec3i_arr([0, 0, 0]))

        storage.blockPositions = conv_to_array(block_positions, Vec3i)
        storage.shiftedPositions = conv_to_array(shift_positions, Vec3i)
        storage.stackSizes = conv_to_array(np.zeros(block_count, dtype=ctypes.c_int), ctypes.c_int)
        storage.maxStackSize = stack_size
        return storage
    else:
        return None
...
def get_params_ht(pro: profiles.BM4DProfile, z_shape: tuple, psd_shape: tuple, z_max: float, refilter: bool,
    ...
    """
    simple_shape = (pro.nf[0] * pro.nf[1] * pro.nf[2] == pro.nf[0] ** 3) or (pro.nf[0] * pro.nf[1] == pro.nf[0] ** 2 and pro.nf[2] == 1)
    params = Parameters()
    ...
    params.splitBlockExtraction = create_vec3b(pro.split_block_extraction[0], pro.split_block_extraction[1], pro.split_block_extraction[2])
...
def get_params_wie(pro: profiles.BM4DProfile, z_shape: tuple, psd_shape: tuple, z_max: float, refilter: bool,
    ...
    params.muSqDC = params.muSq
    params.gamma = ctypes.c_float(pro.gamma)
    params.searchWindowSize = create_vec3i(pro.search_window_wiener[0], pro.search_window_wiener[1], pro.search_window_wiener[2])
    params.imageSize = create_vec3i(z_shape[1], z_shape[0], z_shape[2])
    params.psdSizeOrig = create_vec3i(psd_shape[1], psd_shape[0], psd_shape[2])
...
def parse_blockmatches_output(matchtables: BlockMatchStorageCtype) -> BlockMatchStorage:
    bm_out = BlockMatchStorage()
    bm_out.match_positions = []
    bm_out.match_positions_shift = []
    bm_out.total_size = matchtables.currentStack * matchtables.maxStackSize
    bm_out.group_sizes = []
    for i in range(0, matchtables.currentStack):
        bm_out.group_sizes.append(matchtables.stackSizes[i])
    for i in range(0, bm_out.total_size):
        bm_out.match_positions.append(
            [matchtables.blockPositions[i].x, matchtables.blockPositions[i].y, matchtables.blockPositions[i].z])
        bm_out.match_positions_shift.append(
            [matchtables.shiftedPositions[i].x, matchtables.shiftedPositions[i].y, matchtables.shiftedPositions[i].z])
...
def bm4d_ht(z: np.ndarray, psd: np.ndarray, pro: profiles.BM4DProfile,
    ...
    stack_storage_size = (z.shape[0] + 1) * (z.shape[1] + 1) * (z.shape[2] + 1) // \
                         (pro.step_ht[0] * pro.step_ht[1] * pro.step_ht[2])
    ...
    func_ht(c_z, c_psd, params, transforms, c_est, ctypes.byref(matchtables) if matchtables is not None else matchtables)
    ...
    if isinstance(blockmatches, bool) and blockmatches:
        bm_out = parse_blockmatches_output(matchtables)
...
def bm4d_ht_complex(z: np.ndarray, psd: np.ndarray, pro: profiles.BM4DProfile,
    ...
    func_ht_complex(c_z, c_psd, params, transforms, c_est, ctypes.byref(matchtables) if matchtables is not None else matchtables)
...
def bm4d_wie(z: np.ndarray, psd: np.ndarray, pro: profiles.BM4DProfile,
    ...
    stack_storage_size = (z.shape[0] + 1) * (z.shape[1] + 1) * (z.shape[2] + 1) // \
                         (pro.step_wiener[0] * pro.step_wiener[1] * pro.step_wiener[2])
    matchtables = get_blockmatch_storage(blockmatches, pro.max_stack_size_wiener, stack_storage_size)

    func_wie(c_z, c_psd, params, transforms, c_ref, c_est, ctypes.byref(matchtables) if matchtables is not None else matchtables)
...
def bm4d_wie_complex(z: np.ndarray, psd: np.ndarray, pro: profiles.BM4DProfile,
    ...
    params = get_params_wie(pro, z_shape, psd_shape, np.max(ref) - np.min(ref), refilter, qshifts)
    transforms = get_transforms_complex(t_forward, t_inverse, hadper_trans_single_den, inverse_hadper_trans_single_den, wwin3d)
    params.cutPSD = ctypes.c_bool(False) # Only guaranteed symmetric with real transforms
    ...
    if pro.adjust_complex_params:
        adj_mu = lambda_convert(np.sqrt(params.muSq)) ** 2
        if not np.isreal(z).all():
            params.muSqDC = adj_mu
        params.muSq = adj_mu

    func_wie_complex(c_z, c_psd, params, transforms, c_ref, c_est, ctypes.byref(matchtables) if matchtables is not None else matchtables)
