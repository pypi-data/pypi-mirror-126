...
def can_use_manylinux1_build():
    return platform.system() == 'Linux' and platform.libc_ver()[0] == 'glibc'


def can_use_osx_build():
    return platform.system() == 'Darwin' and platform.mac_ver().split('.')[0] == '11'


# Add the toolset as a dependency where we have a binary available (currently only Linux and OSX 11 with x86)
CHTOOLSET = 'clickhouse-toolset==0.12.dev0'
try:
    if platform.machine() == 'x86_64' and sys.version_info.major == 3 and 6 <= sys.version_info.minor <= 10:
        if can_use_manylinux1_build() or can_use_osx_build():
            install_requires.append(CHTOOLSET)
except:
    pass
