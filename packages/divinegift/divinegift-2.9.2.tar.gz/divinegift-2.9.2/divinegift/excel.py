...
class ExcelX(Excel):
    ...
    def create_simple_excel(self, filename, data: List[Union[Dict, List]], column_width: Optional[List] = None,
        ...
        if header:
            self.add_row(start_row, header, style_list=header_style)
            start_row += 1

        if any(isinstance(e, dict) for e in data):
            self.add_list_rows(start_row, 1, [list(x.values()) for x in data], style_list)
        else:
            self.add_list_rows(start_row, 1, data, style_list)
        self.save_file()
