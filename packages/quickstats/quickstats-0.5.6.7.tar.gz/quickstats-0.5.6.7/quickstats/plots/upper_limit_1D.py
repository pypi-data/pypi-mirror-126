...
class UpperLimit1D(AbstractPlot):
    ...
    def draw(self, logx:bool=False, xlabel:Optional[str]=None, markersize:float=50.,
             draw_observed:bool=True, draw_stat:bool=False):
        ...
        for i, category in enumerate(self.category_df):
            ...
            if draw_observed:
                ...
                ax.text(text_pos['observed'], i + 0.5, "{:.0f}".format(observed_limit), 
            ...
            if draw_stat:
                stat_limit = df['stat']
                ax.text(text_pos['stat'], i + 0.5, "({:.0f})".format(stat_limit), 
                        horizontalalignment='center', 
                        verticalalignment='center',
                        transform=transform,
                        **self.styles['text'])
            ...
            ax.text(text_pos['expected'], i + 0.5, "{:.0f}".format(expected_limit), 
                    horizontalalignment='center', 
                    verticalalignment='center',
                    transform=transform,
                    **self.styles['text'])
