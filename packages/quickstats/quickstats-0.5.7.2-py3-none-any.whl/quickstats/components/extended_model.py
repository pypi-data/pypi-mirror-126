    ...
    def print_summary(self, items:Optional[List]=None, suppress_print:bool=False, detailed:bool=True,
        ...
        for pstr, pset in zip(param_strs, param_sets):
            ...
            if include_patterns is not None:
                pset = [p for pattern in include_patterns for p in pset if fnmatch.fnmatch(p.GetName(), pattern)]
            if exclude_patterns is not None:
                pset = [p for pattern in exclude_patterns for p in pset if not fnmatch.fnmatch(p.GetName(), pattern)]
