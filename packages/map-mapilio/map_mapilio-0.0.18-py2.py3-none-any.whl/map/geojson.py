...
class Geojson:
    ...
    @staticmethod
    def export(points: dict, file_id: str):
        """
        Export geo json format
        """
        Geojson.common_json.features = points
        with open(os.path.join('Exports', file_id, 'detected_points.json'), 'w') as f:
            json.dump(Geojson.common_json, f, cls=helper.JSON_ENCODER_FIX)
