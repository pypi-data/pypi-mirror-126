...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        hashed_video_id_length : int, optional
            ...
            The value can be 4 (recommended) to 32. A higher value would mean faster response times, but it's just a few miliseconds
    ...
    def get_skip_segments(
        ...
        *,
        ...
        try:
            ...
            raise InvalidJSONException("The server returned invalid JSON", response) from exc
        else:
        ...
        *,
        ...
        body = {
            ...
            "segments": [
                {
                    "segment": [
                        s.total_seconds() if isinstance(s, timedelta) else s for s in [segment.start, segment.end]
                    ],
    ...
    def get_top_users(self, sort_type: SortType) -> List[TopUser]:
        ...
        params = {"sortType": sort_type.value if isinstance(sort_type, SortType) else sort_type}
    ...
    def get_segment_info(
        ...
        if segment:
            segments.extend([segment])
        parameters = {"UUID": [segment.uuid if isinstance(segment, Segment) else str(segment) for segment in segments]}
    ...
    @cache(max_entries=300)  # 5 minutes
    def search_for_user(self, user_name: str, exact: bool = False) -> List[SearchedUser]:
        ...
        try:
            ...
            if code != 200:
                ...
                if code == 404:
                    raise NotFoundException("Not Found", response)
                if code == 400:
                    raise BadRequest("Bad Request (Your inputs are wrong/impossible)", response)
                else:
                    raise UnexpectedException("Unexpected response from server", response)
