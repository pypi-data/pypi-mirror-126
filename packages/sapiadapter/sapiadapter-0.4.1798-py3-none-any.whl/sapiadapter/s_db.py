...
def meta_checkupdefault( ) :
    mn = "_default_5800b137d"
    t = meta_read( mn )
    if not t :
        meta_setdefault( )
...
def meta_setdefault( mv_in = False ) :
    # _default_5800b137d
...
def files_get_defaultdata( ) :
    f = files_read( "./local_sapiserver_default_5800b137d.json" )
    if not f :
        return( False )
    d = json.loads( f[ "fc" ] )
    return( d )

def files_set_defaultdata( data ) :

    if( tablerow_exists( "files" , "where fp='./local_sapiserver_default_5800b137d.json'" ) ) :
        files_delete("./local_sapiserver_default_5800b137d.json")


    flag = files_dict2json( "./local_sapiserver_default_5800b137d.json" , data )
    if not flag :
        s_log.write("False files_set_defaultdata")
        return(False)
...
def files_write_defaultdata( ) :
    with open( "./local_sapiserver_default_5800b137d.json" , "w" , 1 ) as f :
        f.write( files_getcontent( "./local_sapiserver_default_5800b137d.json" ) )
