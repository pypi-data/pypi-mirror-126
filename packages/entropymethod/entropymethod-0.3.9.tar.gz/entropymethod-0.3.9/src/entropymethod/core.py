...
def draw_errorbar_plot(k,scores,sd, add_title=None, kticks=None, yticks=None, filename=None):
	...
	plt.plot(k,scores,c='tab:blue',linewidth=2,linestyle='--',alpha=1)
...
def compute_best_k(k,score,score_sd,mode='greatest',out='single'):
	...
	# establish which of the candidate values is a local minimum
	local_minima_bool = np.zeros(len(k)).astype(bool)
	for k_idx, k_val in enumerate(k):

		# compute local sd
		if (k_idx == 0):
			local_sd = np.max([score_sd[k_idx],score_sd[k_idx+1]])
			comp_score = score_sd[k_idx+1]
		elif (k_idx == len(k)-1):
			local_sd = np.max([score_sd[k_idx-1],score_sd[k_idx]])
			comp_score = score_sd[k_idx-1]
		elif (k_idx > 0) and (k_idx < len(k)-1):
			local_sd = np.max([score_sd[k_idx-1],score_sd[k_idx],score_sd[k_idx+1]])
			comp_score = np.min([score_sd[k_idx-1],score_sd[k_idx+1]])

		local_sd = score_sd[k_idx]

		# check if k_val is a local minimum 
		upper_bound = score[k_idx]+local_sd
		if (upper_bound <= comp_score):
			local_minima_bool[k_idx] = True

	# estimate k as the greatest local min
	if np.any(local_minima_bool):
		k_local_min = np.max(k_arr[local_minima_bool])
	else:
		k_local_min = max(k_set)
		print('no local min, buddy :-(')
...
def process_scores(k_list,scores,scores_sd,out,verbose=False, tabspace='', 
	...
	if (best_k_mode == 'greatest'):
		...
		k_estimate, k_set = compute_best_k(k_list,scores,scores_sd,
											mode='smallest', out='set')
	elif (best_k_mode == 'local'):
		k_estimate, k_set = compute_best_k(k_list,scores,scores_sd,
											mode='local', out='set')
	else:
