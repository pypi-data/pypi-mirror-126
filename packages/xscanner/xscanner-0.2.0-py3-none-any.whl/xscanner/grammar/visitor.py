...
class VisitorImp(ParseTreeVisitor):
    ...
    def visitAssignExpression(self, ctx: RuleParser.AssignExpressionContext):
        ...
        if x := ctx.zipExpression():
            arg0 = x.INTERPRETED_STRING_LIT(0).getText()[1:-1]
            arg1 = x.INTERPRETED_STRING_LIT(1).getText()[1:-1]
            result = self.data.match_file(arg0, arg1)
        elif x := ctx.zipHexExpression():
            arg0 = x.INTERPRETED_STRING_LIT().getText()[1:-1]
            arg1 = x.HEX_STRING_LIT().getText()[1:-1]
            result = self.data.match_file_hex(arg0, arg1)
        elif x := ctx.INTERPRETED_STRING_LIT():
