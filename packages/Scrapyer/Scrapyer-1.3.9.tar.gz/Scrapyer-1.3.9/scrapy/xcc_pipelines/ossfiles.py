...
class OSSFilesStore:
    ...
    def del_dirty_page(self,content,according_to,check_num):
        ...
        def inva_page_index():
            ...
            for check_page,extract_text in enumerate(check_gener(pdf)):
                if eval(if_words_spell):
                    fund_index = len(pdf.pages) - check_num + check_page
        ...
        def return_result_body():
            ...
            file_content.close()
            # file_name = 'out00.pdf'
            # file_content = open(f'{file_name}', 'wb')
            # pdfFileWriter.write(file_content)
            return body
        pdf.close()
...
class OssFilesPipeline(MediaPipeline):
    ...
    def __init__(self, store_uri, file_pipe_config, download_func=None, settings=None):
        ...
        # if not settings.get('FILES_OSS_FILE'):
        #     raise NotConfigured("At settings.py missing 'FILES_OSS_FILE'")
    ...
    def get_media_requests(self, item, info):
        ...
        if not hasattr(item,'_cookie'):
            item.__setattr__('_cookie','')
        ...
        if self.file_pipe_config.get(self.__class__.__name__)["RESOURCE_CLASSNAME"].lower() in item.__class__.__name__.lower():
            ...
            def request_add_params(urls):
                for u in urls:
                    u = Request(u)
                    if item._header:
                        u.headers = item._header
                        u.headers.setdefault('Referer',item._refer)
                    if item._cookie:
                        # 如果设置自定义ITEM_COOKIE优先替换到header
                        u.headers.setdefault('Cookie',item._cookie)
                    yield u
