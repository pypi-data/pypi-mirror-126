...
class TextRankSummarizerFormatter(FormatterBase, ProcessorBase, ABC):
    ...
    def _check_sentences(self, nlp, document: Document):
        doc: Doc = None
        if not document.sentences:
            doc = nlp(document.text)
        else:
            textrank = nlp.get_pipe("textrank")
            parser = nlp.get_pipe("parser")
            with nlp.select_pipes(disable=["parser", "textrank"]):
                doc = nlp(document.text)
                sent_map = RangeMap()
    ...
    def _summarize(self, document: Document, parameters: FormatterParameters) -> str:
        ...
        doc = self._check_sentences(nlp, document)
