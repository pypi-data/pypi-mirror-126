...
class NoFillNoFuture:
    ...
    def check_token_recur(self, temp_token: list, token_y: int, token_x: int, mapping_y: int, mapping_x: int,
        ...
        if sum([l_token.count(True) for l_token in temp_token]) == 0:
            self.coordinate_mapping_y = mapping_y - token_y
            self.coordinate_mapping_x = mapping_x - token_x
            self.coordinate_list.append((mapping_y - token_y, mapping_x - token_x))
            self.evaluate()
            return True
    ...
    def evaluate(self):
        evaluation_points = 0
        if self.p_num == 1:
            evaluation_points += self.coordinate_mapping_x ** 2
            evaluation_points += self.coordinate_mapping_y ** 2
        else:
            evaluation_points += (self.mapping.x - self.coordinate_mapping_x) ** 2
            evaluation_points += (self.mapping.y - self.coordinate_mapping_y) ** 2
        self.evaluated_list.append(evaluation_points)
    ...
    def init_before_search(self):
        ...
        self.coordinate_mapping_x = 0
        self.token.num = 0
        self.evaluated_list = [0]
        self.coordinate_list = [(0, 0)]
