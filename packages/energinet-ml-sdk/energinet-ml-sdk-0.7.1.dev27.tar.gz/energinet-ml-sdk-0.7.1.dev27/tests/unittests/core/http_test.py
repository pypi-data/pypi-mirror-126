...
from energinetml.core.http import create_app
...
def client(controller):
    with patch(
        ...
        app = create_app(model=model, trained_model=Mock(), model_version="123")
...
def client_with_opentelemetry(controller):
    with patch(
        ...
        app = create_app(model=model, trained_model=Mock(), model_version=MODEL_VERSION)
