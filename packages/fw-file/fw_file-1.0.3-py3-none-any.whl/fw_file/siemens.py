...
def load_dat(file: t.BinaryIO) -> t.Dict[str, t.Any]:
    ...
    for line_bytes in file:
        try:
            ...
            continue
        if match := re.match(DAT_FIELD_RE, line):
            key, value = match.groups()
            fields[key] = parse_yaml_value(value)
        elif match := re.match(DAT_ARRAY_RE, line):
            array_key = match.group("key")
        elif array_key and (match := re.match(DAT_VALUE_RE, line)):
            fields[array_key] = parse_yaml_value(match.group("value"))
            array_key = None
