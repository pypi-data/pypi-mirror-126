...
class Player():
    ...
    def holds_all_winners_in_suit(self, suit: Suit, trick: Trick=None) -> bool:
        ...
        our_length = len(self.seats_unplayed_cards[long_player][suit.name])
        our_cards = self.seats_unplayed_cards[long_player][suit.name]
        ...
        for index, card in enumerate(our_cards):
            if index + 1 >= min(opponents_length, our_length):
                return True
