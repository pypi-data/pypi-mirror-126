...
class SceneMark:
    ...
    node_id : string
        Env variable assigned to the Node through the Developer Portal. The Node ID is the unique identifier of the Node.
    event_type : string
        Env variable assigned to the Node through the Developer Portal.
        The main thing this Node is 'interested in'. Can take a range of
    ...
    analysis_description : string, default None
        Env variable assigned to the Node through the Developer Portal.
        Used to describe what the analysis is about, what it is 'doing'.
    analysis_id : string, default None
        Env variable assigned to the Node through the Developer Portal.
    ...
    def save_request(self, request_type : str, name : str):
        ...
        Returns
        -------
        Saved json file of the (state of the) SceneMark or NodeSequencer Address
        """
    ...
    def get_id_from_uri(self, uri : str):
        ...
        Returns
        -------
        * SceneDataID, or "No match" if there is no match.
        """
    ...
    def generate_detected_object_item(
        ...
        custom_item_type : string,
            Allows specifying the custom NICEItemType
            Non-mandatory, defaults to None
        item_id ; string,
            Indicating an ID on the object, e.g. the person's name
            Non-mandatory, defaults to None
        item_type_count : int,
            Counting the amount of the stated NICEItemType
            Non-mandatory, defaults to None
        probability : float,
            Indicating the confidence on the item
            Non-mandatory, defaults to None
        attributes : list,
            Containing attribute items
            Non-mandatory, defaults to None
    ...
    def add_analysis_list_item(
        ...
        processing_status : string, taking one the following values:
            ...
            "Failed",\n
            "Error"
        custom_event_type : string, set if the above processing status
            equals CustomAnalysis
        total_item_count : int, total amount of items detected in the scene
        error_message : string, used to propagate errors
        detected_objects : list of objects, generated
