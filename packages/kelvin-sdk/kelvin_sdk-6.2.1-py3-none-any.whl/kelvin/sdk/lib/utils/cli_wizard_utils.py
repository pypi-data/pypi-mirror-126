...
from kelvin.sdk.lib.configs.internal.cli_wizard_configs import AppCreationWizardConfigs
...
def start_app_creation_wizard() -> ProjectCreationParametersObject:
    """Creates the application creation object from a prompted wizard

    Returns
    -------
    ProjectCreationParametersObject:
        an object with all the necessary variables for the creation of an application
    ...
    starting_tree = ProjectCreationParametersObject.compatibility_tree()

    if app_type and app_type not in ["docker"]:
        starting_tree = starting_tree.get(ProjectType(app_type), {})
    ...
    return_obj = ProjectCreationParametersObject(
