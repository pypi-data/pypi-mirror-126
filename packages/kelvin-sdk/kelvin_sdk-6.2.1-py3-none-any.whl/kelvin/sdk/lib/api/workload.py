...
from kelvin.sdk.lib.models.generic import GenericObject, KPath, NameWithVersion
from kelvin.sdk.lib.models.ksdk_workload_deployment import WorkloadDeploymentRequest, WorkloadTemplateData
...
def workload_list(
    ...
    try:
        ...
        if app_name:
            app_name_with_version = NameWithVersion(app_name)
...
def workload_deploy(workload_deployment_request: WorkloadDeploymentRequest) -> OperationResponse:
    ...
    try:
        ...
        if not workload_deployment_request.app_name:
            app_configuration = KelvinAppConfiguration(**loaded_app_config_object)
            workload_deployment_request.app_name = app_configuration.info.app_name_with_version

        app_name_with_version = NameWithVersion(name=workload_deployment_request.app_name)
...
def workload_update(
    workload_name: str, app_config: str, workload_title: Optional[str] = None, app_version: Optional[str] = None
) -> OperationResponse:
    """
    ...
    try:
        ...
        if not workload.app_name:
            raise ValueError("Invalid workload name")

        app_name_with_version = NameWithVersion(name=workload.app_name, version=app_version)
        workload_deployment_request: WorkloadDeploymentRequest = WorkloadDeploymentRequest(
            node_name=workload.acp_name,
            app_name=app_name_with_version.full_name,
...
def _deploy_bulk_workload(
    ...
    colour = logger_utils.LOG_COLOR == LogColor.COLORED
    ...
    for i, (info, app_config) in enumerate(zip(data, app_configs)):
        ...
        with NamedTemporaryFile("wt") as temp_file:
            ...
            app_name_with_version = NameWithVersion(name=info.app_name, version=info.app_version)
            workload_deployment_request = WorkloadDeploymentRequest(
