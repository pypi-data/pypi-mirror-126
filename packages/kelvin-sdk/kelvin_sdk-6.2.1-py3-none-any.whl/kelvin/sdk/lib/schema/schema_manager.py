...
from kelvin.sdk.lib.configs.internal.schema_manager_configs import SchemaManagerConfigs
...
def build_kelvin_app_block(project_creation_parameters_object: ProjectCreationParametersObject) -> dict:
    ...
    if project_creation_parameters_object.app_flavour is ApplicationFlavour.pubsub:
        ...
        kelvin_broker_container = DockerImageNameDetails(
            docker_image_name=broker_name,
            registry_url=session_manager.get_current_session_metadata().sdk.components.kelvin_broker,
        )
        kelvin_block.update(
            {
                "outputs": [  # type: ignore
...
def _get_schema_version_file_path(schema_version: str) -> KPath:
    ...
    return KPath(schema_storage_path / f"{schema_version}.json").expanduser().resolve()
