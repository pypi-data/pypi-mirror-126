...
from kelvin.sdk.lib.models.apps.common import EnvironmentVar, NameDNS, Port, Version
from kelvin.sdk.lib.models.apps.docker_app import DockerAppType
from kelvin.sdk.lib.models.apps.flow_app import FlowAppType
...
class App(KSDKModel):
    ...
    bridge: Optional[BridgeAppType] = None
    flow: Optional[FlowAppType] = None

    @property
    def app_type_configuration(self) -> Optional[KelvinAppType]:
...
class PortMapping(KSDKModel):
    name: NameDNS = Field(..., description="Port name.", title="Port Name")
...
class Environment(KSDKModel):
    node_name: Optional[NameDNS] = Field("unknown-node", description="The name of the node", title="node Name")
    workload_name: Optional[NameDNS] = Field(
        "unknown-workload", description="The name of the Workload, unique to the node", title="Workload Name"
    )
