...
class ProjectCreationParametersObject(KSDKModel):
    ...
    def assess_parameters_validity(self) -> bool:
        ...
        if self.app_type in [ProjectType.kelvin, ProjectType.bridge]:
...
class KelvinAppBuildingObject(ProjectBuildingObject):
    ...
    def get_dockerfile_run_command(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        if app.app_type_configuration and app.app_type_configuration.language:
            if app.app_type_configuration.language.python:
                python_entry_point = app.app_type_configuration.language.python.entry_point
                default_docker_cmd = f'["--configuration", "/opt/kelvin/app/{self.app_config_file_path.name}"]'

                if project_type == ProjectType.kelvin:
                    app_flavour = app.app_type_configuration.language.python.get_flavour()
