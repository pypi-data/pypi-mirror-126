...
class Markovitz(object):
    ...
    def exam_search(self, exam_shares):
        ...
        if True in list(df_exam.min() == 0):
            print(df_exam)
            raise ValueError("存在某只股票购买手数为0！！！")
    ...
    def weights_tests(self, stocks=['药明康德', '比亚迪', '山西汾酒', '五粮液', '贵州茅台'], number=5):
        ...
        for i in tqdm(list(itertools.combinations(stocks, number))):
            ...
            std = np.std(np.array(list(result_dict.values()))+1)
            lists.append(
                [result_dict, std, sharpe, np.min(list(result_dict.values()))])
            if False not in list(result > 0.02)[:-1]:
                print('--'*30)
                print(result.map(lambda x: round(x, 3)))
                print('--'*30)
        return pd.DataFrame(lists, columns=['weights', 'std', 'sharpe', 'min weight'])


if __name__ == '__main__':

    mk = Markovitz(names=['迎驾贡酒', '明微电子', '健民集团', '西藏珠峰', '西部超导'],  # 股票组合
    ...
    ow = mk.buy()
    print(ow)
    # mk.weights_tests(stocks=stocks, number=5).to_excel(
