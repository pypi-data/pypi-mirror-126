...
class Markovitz(object):
    ...
    def calculate(self):
        ...
        for name in self.names:
            data_cov[name] = list(data[data['name'] == name]['pctChg'])
    ...
    def weights_test(self, stocks=['药明康德', '比亚迪', '山西汾酒', '五粮液', '贵州茅台'], number=5):
        
        self.names = stocks
        self.lens = len(self.names)
        global StockData
        self.data = self.datas.loc[self.names]
        self.data = self.data.reset_index(drop=True)
        return self.optimal_weight()

    def weights_tests(self, stocks=['药明康德', '比亚迪', '山西汾酒', '五粮液', '贵州茅台'], number=5):
        lists=[]
        for i in tqdm(list(itertools.combinations(stocks,number))):
            result = self.weights_test(list(i), number)
            result_dict = result.to_dict()
            sharpe = result_dict['sharpe']
            del result_dict['sharpe']
            lists.append([result_dict,sharpe,np.min(list(result_dict.values()))])
            if False not in list(result > 0.02)[:-1]:
                print('--'*30)
                print(result.map(lambda x: round(x, 3)))
                print('--'*30)
        return pd.DataFrame(lists,columns=['weights','sharpe','min weight'])
                


if __name__ == '__main__':
    stocks = [
        '国投中鲁', '品渥食品', '德利股份', '有友食品', '熊猫乳品',
        '仟源医药', '大理药业', '开开实业', '赛隆药业', '双成药业', #'易明医药',
        '禾盛新材', '金海高科', '奇精机械', '朗迪集团', '春光科技', #'毅昌股份', '长虹华意',
        '信雅达', '智莱科技', '博彦科技', '新智认知', '唐源电气', #'佳华科技', '大华股份',
        '宁波富邦', '精艺股份', '丽岛新材', '翔鹭钨业', '众源新材',# '罗平锌电',
        '久量股份', '朝阳科技', '利通电子', '力鼎光电', '亚世光电', #'惠威科技', '澳弘电子'
    ]
    mk = Markovitz(names=stocks,  # 股票组合
                   start_date='2021-05-02',  # 开始日期
                   end_date='2021-11-02',  # 结束日期
                   no_risk_rate=0.023467,  # 无风险利率
                   funds=10000000  # 最大资金限制
                   )
    mk.weights_tests(stocks=stocks, number=5).to_excel('result.xlsx',index=False)
