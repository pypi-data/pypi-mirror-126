...
class Cron(object):
    ...
    def __init__(self, settings, dbs):
        ...
        if 'wd.login' in settings and 'wd.password' in settings:
            self.auth = (settings['wd.login'], settings['wd.password'])
        else:
            self.auth = None
    ...
    def get(self):
        ...
        for self.srv in self.dbs.query(Server).filter_by(
            ...
            try:
                if self.auth:
                    rda = requests.get(self.srv.url, verify = False, auth = self.auth)
                else:
