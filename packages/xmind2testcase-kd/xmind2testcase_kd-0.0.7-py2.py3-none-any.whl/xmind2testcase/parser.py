...
def xmind_to_testsuites(xmind_content_dict):
    ...
    for sheet in xmind_content_dict:
        ...
        if sub_topics:
            # 递归，去掉所有空的子主题以及配置文件中说明需要忽略字段的子主题
...
def sheet_to_suite(root_topic):
    ...
    suite.details = root_topic['note']  # 备注==前置条件
    suite.link = root_topic['link']
    suite.sub_suites = []
...
def parse_testsuite(suite_dict):
    ...
    testsuite.link = suite_dict['link']
...
def parse_a_testcase(case_dict, parent):
    ...
    testcase.name = gen_testcase_title(topics)  # 链接用例名

    preconditions = gen_testcase_preconditions(topics)  # 前置条件
    testcase.preconditions = preconditions if preconditions else '无'

    summary = gen_testcase_summary(topics)  # 批注
...
def get_execution_type(topics):
    ...
    for item in labels[::-1]:
        if item.lower() in ['接口', '接口测试', 'implement']:
            exe_type = 5
            break
        if item.lower() in ['性能', '性能测试', 'performance']:
            exe_type = 4
            break
        if item.lower() in ['功能', '功能测试', 'function']:
            exe_type = 3
            break
...
def get_link(topics):
    links = [topic['link'] for topic in topics]
    links = filter_empty_or_ignore_element(links)
    return links[-1] if links and len(links) > 0 else ''
