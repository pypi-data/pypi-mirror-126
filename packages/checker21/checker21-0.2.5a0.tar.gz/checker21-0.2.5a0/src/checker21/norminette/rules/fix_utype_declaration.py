...
from norminette.rules.check_utype_declaration import CheckUtypeDeclaration, utypes


class FixUtypeDeclaration(CheckUtypeDeclaration):
		...
		if contain_full_def == False:
			...
			if context.check_token(i - 1, ["MULT", "BWISE_AND", "LPARENTHESIS"]) is True:
				...
				while (
					...
					i -= 1
			context.add_var_decl_align(context.peek_token(i))
			...
			if context.scope.vars_alignment == 0:
				...
				if current_indent > context.scope.vars_alignment:
					context.scope.vars_alignment = current_indent
