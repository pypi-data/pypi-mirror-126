...
def check(files, show_ok):
    ...
    for source_file in files:
        ...
        for item in iter:
            ...
            start_pos = item.start() + per_log.index(',')+1
            # print(text[start_pos:item.end()])

            end_pos, num_arg = parse(text, start_pos)
            if end_pos != item.end():
                per_log = text[item.start():end_pos]
            
            num_brace = count_num_brac(per_log)
