...
@click.option('--quiet', '-q', required=False, help='Do not print the introductory and copyright messages. These messages are also suppressed in batch mode.')
@click.option('--args', required=False, help='Arguments passed to debugger')
@click.argument('elf_file', type=click.Path(exists=True))
def debug(ctx, gateway, interpreter, verbose, tty, quiet, args, elf_file):
    """
    ...
    resp = session.remote_debug(gateway, archive, args)
