...
class GnistaDataPoint:
    ...
    def set_data_point_data(self, data_frame: Union[DataFrame, float], unit: Union[str, None]):
        token = self.connection.get_access_token()
        client = AuthenticatedClient(base_url=self.connection.datapoint_base_url, token=token)
        ...
        if isinstance(data_frame, DataFrame):
            ...
            update_request = UpdateTimeSeriesRequest(values=values, time_series_id=str(uuid.uuid4()), unit=unit)
        ...
        if isinstance(data_frame, float):
            constant_request = UpdateConstantDataRequest(value=data_frame, unit=unit)
            if self.connection.id_token:
                data_point_update_constant_data.sync_detailed(
                    client=client, data_point_id=self.data_point_id, json_body=constant_request
                )
            else:
