...
class MultiVariableIteration:
	def __init__(self, A, x0, b, power=-6, N=100, norm_type="l_infinity"):
		...
		if np.sum(x0.shape) - np.sum(x0.shape[0]) > 1: raise IndexError(f"ERROR! {bad_x0(sym_x0)}")
		if np.sum(b.shape) - np.sum(b.shape[0])> 1: raise IndexError(f"ERROR! {bad_b(sym_b)}")
