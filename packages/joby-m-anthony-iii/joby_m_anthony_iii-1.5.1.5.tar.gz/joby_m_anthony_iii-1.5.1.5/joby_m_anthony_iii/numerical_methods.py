...
class norms:
	def __init__(self, x, x0=[]):
		...
		if not(x0.shape[0] == 0 or len(x) == len(x0)):
...
# eigenvalue solvers
class DirectSolver:
	def __init__(self, A, x, power, max_iter=100):
		"""
		Parameters
		----------
		self : object
		...
		self.x : array
			Aggregate collection of approximated solutions to system of equations.
	...
	def power_method(self):
		...
		"""
		A, x, tol, N = self.A, self.x, 10**self.power, self.max_iter
		mu = [norms.l_infinity(x)]
		x = x/mu[-1]
		k, eigenvectors, errors = 1, [x], [1]
		while errors[-1] > tol and k <= N:
			y = np.matmul(A, x)
			for yi in y:
				if np.abs(yi) == norms.l_infinity(y):
					yp = float(yi)
			mu.append(yp)
			eigenvectors.append(y/yp)
			errors.append(norms.l_infinity(x, eigenvectors[-1]))
			x = eigenvectors[-1]
			k += 1
		...
		return pd.DataFrame(data={"Iterations": self.iterations, "Mu": self.mu, "Lambdas": self.eigenvectors, "Errors": self.errors})

	def inverse_power_method(self, q):
		"""Approximate dominant eigenvalue and associated eigenvector of matrix, `A` given some non-zero vector, `x`.
	...
	def qr_algorithm(self):
		...
		self.errors : tuple
			Collection of yielded norms.
		"""
		A, tol, N = self.A, 10**self.power, self.max_iter
		k, eigenvectors, errors = 0, [np.diag(A)], [1]
		while errors[-1] > tol and k <= N:
	...
	def steepest_descent(self, b):
		"""Approximate solution vector, `x` given matrix, `A` initial guess vector, `x0`, and vector, `b`.
		...
		self.errors : tuple
			Collection of yielded norms.
		"""
		A, x0, tol, N = self.A, self.x, 10**self.power, self.max_iter
		self.b = np.array(b)
		k, eigenvectors, errors = 1, [x0], [1]
		while errors[-1] > tol and k <= N:
			r = b - A*x0
			rt = np.transpose(r)
			alpha = float((rt*r)/(rt*A*r))
			x1 = x0 + alpha*r
			eigenvectors.append(x1)
			errors.append(norms(x1, x0).l_infinity())
			x0 = x1
			k += 1
		self.iterations = tuple(range(k))
		self.eigenvectors = tuple(eigenvectors)
	...
	def conjugate_gradient(self, b, C=None):
		"""Approximate solution vector, `x` given matrix, `A` initial guess vector, `x0`, and vector, `b`.
		...
		"""
		A, x0, tol, N = self.A, self.x, 10**self.power, self.max_iter
		self.b, self.C = np.array(b), C
		if isinstance(C, (None)): Minv = 1
		else: Minv = np.linalg.inv(C*np.transpose(C))
		r0 = b - A*x0
		# v0 = r0
		v0 = Minv*r0
		k, eigenvectors, errors = 1, [x0], [1]
		while errors[-1] > tol and k <= N:
			r0t = np.transpose(r0)
			v0t = np.transpose(v0)
			# alpha = float((r0t*r0)/(v0t*A*v0))
			alpha = float((r0t*Minv*r0)/(v0t*A*v0))
			x1 = x0 + alpha*v0
			eigenvectors.append(x1)
			errors.append(norms(x1, x0).l_infinity())
			r1 = r0 - alpha*A*v0
			r1t = np.transpose(r1)
			# s1 = float((r1t*r1)/(r0t*r0))
			s1 = float((r1t*Minv*r1)/(r0t*Minv*r0))
			# v0 = r1 + s1*v0
			x0, r0, v0 = x1, r1, Minv*r1 + s1*v0
			k += 1
		self.iterations = tuple(range(k))
		self.eigenvectors = tuple(eigenvectors)
		self.errors = tuple(errors)
...
class ode:
	...
	def runge_kutta(f, t0, w0, tn, N):
		...
		"""
		sym_function = "f"
		bad_f = "Input, " + sym_function + " was not an expression."
