...
def get_datasets_list(api_inputs: ApiInputs, include_path: bool = False, path_prefix="*"):
    ...
    if(path_prefix != "*"):
        url = f"{api_inputs.api_projects_endpoint}/{api_inputs.api_project_id}/data-ingestion/datasets?pathprefix={path_prefix.replace('/', '|')}&includepath={str(include_path)}"
    else:
        url = f"{api_inputs.api_projects_endpoint}/{api_inputs.api_project_id}/data-ingestion/datasets?includepath={str(include_path)}"
