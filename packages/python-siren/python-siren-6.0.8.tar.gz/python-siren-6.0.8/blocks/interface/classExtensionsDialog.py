...
class ExtensionsDialog(PreferencesDialog):
    ...
    def __init__(self, parent, pref_handle, conf_filter):
        PreferencesDialog.__init__(self, parent, pref_handle, conf_filter)
    ...
    def iterateSections(self, conf_filter):
        tsecs = self.pref_handle.getPreferencesManager().getTopSections(conf_filter)
        available_exts = sorted(self.pref_handle.getData().getAvailableExtensionKeys())
        params_exts = {}
        for tsec in tsecs:
            for sec in tsec.get("subsections", []):
                params_exts[sec.get("name")] = sec
        return [params_exts.get(sec, {"name": sec, "empty": True}) for sec in available_exts]
