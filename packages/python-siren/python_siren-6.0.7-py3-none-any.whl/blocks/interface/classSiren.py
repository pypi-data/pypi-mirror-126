...
from ..work.classWorkInactive import GUIBoss
...
from ..mine.classData import RowE, Data
from ..mine.classQuery import Literal, Query
from ..mine.classCol import DataError, ColM
from ..mine.classRedescription import Redescription
from ..mine.classProps import findFile
...
class Siren(GUIBoss):
    ...
    rffiles = [('fields_rdefs_basic.txt', 'mine')]
    ...
    def __init__(self):
        ...
        self.plant.setUpCall([self.doUpdates, self.resetLoggerComm])
        self.refresh()
    ...
    def OnQuit(self, event):
        if self.plant.getWP().isActive():
            if self.plant.getWP().isDistributed() and self.plant.getWP().nbWorking() > 0:
                collectLater = self.quitReturnLaterDialog(clientid=self.plant.getWP().getHid())
                if collectLater == 0:
                    return
    ...
    def quitReturnLaterDialog(self, test=None, clientid=0):
        reponse = -1
        msg = WorkClient.questionReturnLater(clientid)
        dlg = wx.MessageDialog(self.toolFrame, msg, style=wx.YES_NO | wx.CANCEL | wx.NO_DEFAULT | wx.ICON_EXCLAMATION, caption='Computations in progress')
        tt = dlg.ShowModal()
        if tt == wx.ID_CANCEL:
            reponse = 0
        if tt == wx.ID_YES:
    ...
    def expand(self, params={}):
        if params is None:
            params = {}
        self.progress_bar.Show()
        rt = self.plant.getWP().addWorker(self, params)
        if rt is not None:  # something went wrong
            updates = {}
            self.processInComm(rt, updates=updates)
    ...
    def project(self, proj=None, vid=None):
        ...
        if proj is not None and vid is not None:
            ...
            if out == 0:
                ...
                if wid is None:  # otherwise, already ongoing, wait for results
                    rt = self.plant.getWP().addWorker(self, proj, {"vid": vid})
                    if rt is not None:  # something went wrong
                        updates = {}
                        self.processInComm(rt, updates=updates)
                        self.doUpdates(updates)  # To update the worker stoppers
    ...
    def getWP(self):
        return self.plant.getWP()

    def rescheduleCheck(self):
        if self.plant.getWP().nbWorking() > 0:
            # TODO CHECK!
            if self.call_check is None:
                self.call_check = wx.CallLater(self.dw.getPreference('results_delay')*1000, self.checkResults)
            else:
                self.call_check.Restart(self.dw.getPreference('results_delay')*1000)
        else:
            self.call_check = None

    def checkResults(self, once=False, menu=False):
        # print("Check results\tnb working", self.plant.getWP().infoStr(), self.plant.getWP().nbWorking())
        in_comm = self.plant.getWP().checkInComm()
        updates = {}
        self.processInComm(in_comm, updates=updates)
        if menu:
            updates["menu"] = True
        if not once:
    ...
    def readyProj(self, vid, proj):
        if proj is None:
            self.viewsm.deleteView(vid, freeing=True)
            return
