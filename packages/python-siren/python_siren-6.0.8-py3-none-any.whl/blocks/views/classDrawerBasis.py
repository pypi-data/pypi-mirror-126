...
from ..mine.classSParts import SSetts
from ..mine.classRedescription import Redescription
from ..mine.classQuery import SYM, Query
from ..mine.classData import Data
...
class DrawerEntitiesTD(DrawerBasis):
    ...
    def plotMapperHist(self, axe, vec, vec_dets, mapper, nb_bins, corners, draw_settings):
        ...
        axe.tick_params(direction="inout", left=False, right=True,
                        labelleft=False, labelright=True, labelsize=self.view.getFontSizeProp())
        return (x0, x1, y0, y1)
    ...
    def plotDotsSimple(self, axe, dots_draw, draw_indices, draw_settings):
        ...
        # pdb.set_trace()  # why does it get here? proj fails to see it's not ready...
        # x = self.getCoords(0, draw_indices[kindices == 0])
        # y = self.getCoords(1, draw_indices[kindices == 0])
        # c = dots_draw["fc_dots"][draw_indices[kindices == 0], :]
    ...
    def drawEntity(self, idp, fc, ec, sz=1, zo=4, dsetts={}):
        ...
        if self.drawPoly():
            ...
            return [self.axe.scatter([x], [y], c=numpy.array([fc]), edgecolors=ec, s=5*sz, marker=dsetts["shape"], zorder=zo)]
