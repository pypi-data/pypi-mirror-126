...
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = None, socketDebugging = False):
        ...
        self.apis = "https://service.narvii.com/api/v1"
    ...
    def parse_apis_headers(self, data: str = None):
        if not data:
    ...
    async def login(self, email: str, password: str):
        ...
        **Returns**
        ...
        async with self.session.post(f"{self.api}/g/s/auth/login", headers=self.parse_headers(data=data), data=data) as response:
            if json.loads(await response.text())["api:statuscode"] != 0: return exceptions.CheckException(json.loads(await response.text()))
            else:
                self.authenticated = True
        ...
        if doNotDisturb is not None:
            if doNotDisturb:
                ...
                async with self.session.post(f"{self.api}/g/s/chat/thread/{chatId}/member/{self.userId}/alert", headers=self.parse_headers(data=data), data=data) as response:
                    if json.loads(await response.text())["api:statuscode"] != 0: res.append(exceptions.CheckException(json.loads(await response.text())))
