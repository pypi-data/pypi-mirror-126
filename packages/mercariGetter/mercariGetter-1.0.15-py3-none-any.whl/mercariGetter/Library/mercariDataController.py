...
class mercariDataSearch:
    ...
    def getWebDriver(self, config:mercariConfig):
        ...
        # WebDriverを返す
        return webdriver.Chrome(executable_path=config.WebDriverPath, options=options)
    ...
    def getMercariDataToPandas(self, searchConditionJsonString:str, config:mercariConfig):
        ...
        # ヘッドレスでブラウザを起動
        driver:WebDriver = self.getWebDriver(config)
