...
def service_validate(data: Dict[str, Any], general_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def is_valid_empty_job(job_name: str) -> bool:
        if not job_name in commands:
            return False
        job : Dict[str, Any] = commands[job_name]
        result : bool = Schema(empty_job_schema).validate(job)
        return result
                ...
                Or(
                    ...
                    {  # Task converted to a Circleci job; can be referenced via "ServiceName:TaskName"
                        str:
                        {
                            ...
                            Optional("requires", default=[]): [And(str, is_requires_id)],
                            Optional("not_changed_job", default="empty_job"): And(str, is_valid_empty_job)
                        }
                    }
                )
            }
...
def general_config_validate(data: Dict[str, Any]) -> Dict[str, Any]:
    general_config_schema = Schema({
        ...
        Optional("commands"): {
            ...
            And("empty_job", error="You have to provide a command for 'empty_job'"): empty_job_schema,
