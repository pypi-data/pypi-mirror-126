...
def check_two_excel_diff(old_file_name, new_file_name, out_file_name=None, out_dir='./data'):
    ...
    save_diff_to_excel(out_file_name, df_changed, df_removed, df_added, subset)
...
def check_two_df_diff(old, new, subset=None, id_key='ID', out_file_name=None, out_dir='./data'):
    if not subset:
        subset = old.columns
    # print(subset)
    ...
    if out_file_name is None:
        time_stemp = str(datetime.now())
        time_stemp = time_stemp.replace(':', '_')
        out_file_name = '{}/check_two_df_diff_{}.xlsx'.format(out_dir, time_stemp)
    save_diff_to_excel(out_file_name, df_changed, df_removed, df_added, subset)
