...
class IsClickedMixin:
    """
    Миксин для пометки полей, как кликабельные (непереносимые)
    """

    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        context_data.update({'is_clicked': self.is_clicked})
        return context_data
    ...
    def is_clicked(self):
        ...
        """
        return 'is_clicked' in self.request.GET
...
class ReportDBTablesTreeListView(IsClickedMixin, DBTablesBaseTreeListView):
    """
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().for_report(self.parent)
        if self.is_clicked:
...
class TableFieldsListView(IsClickedMixin, ParentMixin, BaseTreeListView):
    """
...
class ReportVirtualFieldCreateView(ParentMixin, ReportFieldMixin, CreateAjaxView):
    """
    ...
    form_class = ReportVirtualFieldCreateForm
