...
function report_designer() {
    ...
    $(document).on('focus click', '.js-expression-content[contenteditable="true"]', function (e) {
        ...
        let element = sel.anchorNode.nodeName === '#text' ? sel.anchorNode.parentNode : sel.anchorNode;
        $('[contenteditable="true"]').removeAttr('data-position');
        $(element).attr('data-position', sel.anchorOffset);
    });
    ...
    $(document).on('click', '.js-tree-function-btn', function (e) {
         ...
         */
        // Поле выражения
        let expression_field = $('.js-rd-create-expression-area');

        // Создание элемента выражения
        let template =
            $('<div>', {'class': 'expression js-expression', 'data-name': $(this).data('name'), 'data-display-name': $(this).data('display-name')})
                .append(
                    $('<span>', {'class': 'title js-expression-title', 'contenteditable': 'false'}).text($(this).data('display-name'))
                        .append($('<span>', {'class': 'delete js-expression-delete'})),
                    $('<span>', {'class': 'lbkt', 'contenteditable': 'false'}),
                    $('<span>', {'class': 'content js-expression-content', 'contenteditable': 'true'}),
                    $('<span>', {'class': 'rbkt', 'contenteditable': 'false'}),
                );
        ...
        let left_part = content.contents().slice(0, position);
        let right_part = content.contents().slice(position + 1, contents.length);
        ...
        function tree(branches) {
            ...
            branches.each(function (index, value) {
                ...
                let obj = {
                    ...
                    'branches': expressions.length ? tree(expressions) : [],
                    'value': expressions.length ? null : expression_content.text(),
                }
                data.push(obj);
            });
            return data;
