...
@method_decorator(login_required, name='dispatch')
class ReportGeneratedTableView(DynamicContentTableBaseView):
    """
    ...
    ajax_content_name = 'generated_report'
    ...
    def report(self):
        ...
        Отчет
        """
        return get_object_or_404(Report, pk=self.request.GET.get('report'))
    ...
    def generated_report(self):
        ...
        """
        return ReportDataHandler(self.report).run()

    def get_queryset(self):
        if self.is_base_type:
            return pandas.DataFrame(dict.fromkeys([], []))
        return self.generated_report.report
    ...
    def init_table(self, queryset, **kwargs):
        ...
        if not table_class:
            return
        table = table_class(dataframe=None, groups=None)
        table.create_table()
        return table
