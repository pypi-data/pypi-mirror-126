...
class Analysis:
	"""Analysis commands group definition. 8 total commands, 4 Sub-groups, 0 group commands"""
	...
	def efficiency(self):
		...
		if not hasattr(self, '_efficiency'):
			...
			self._efficiency = Efficiency(self._core, self._cmd_group)
	...
	def prRate(self):
		...
		if not hasattr(self, '_prRate'):
			from .PrRate import PrRate
			self._prRate = PrRate(self._core, self._cmd_group)
		return self._prRate
	...
	def clone(self) -> 'Analysis':
		...
		new_group = Analysis(self._core, self._cmd_group.parent)
