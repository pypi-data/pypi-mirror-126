...
class Level:
	...
	def alinearize(self):
		...
		if not hasattr(self, '_alinearize'):
			from .Alinearize import Alinearize
			self._alinearize = Alinearize(self._core, self._cmd_group)
	...
	def amplifier(self):
		...
		if not hasattr(self, '_amplifier'):
			...
			self._amplifier = Amplifier(self._core, self._cmd_group)
	...
	def detAtt(self):
		...
		if not hasattr(self, '_detAtt'):
			from .DetAtt import DetAtt
			self._detAtt = DetAtt(self._core, self._cmd_group)
		return self._detAtt
	...
	def dlinearize(self):
		...
		if not hasattr(self, '_dlinearize'):
			from .Dlinearize import Dlinearize
			self._dlinearize = Dlinearize(self._core, self._cmd_group)
	...
	def swAmplifier(self):
		...
		if not hasattr(self, '_swAmplifier'):
			from .SwAmplifier import SwAmplifier
			self._swAmplifier = SwAmplifier(self._core, self._cmd_group)
