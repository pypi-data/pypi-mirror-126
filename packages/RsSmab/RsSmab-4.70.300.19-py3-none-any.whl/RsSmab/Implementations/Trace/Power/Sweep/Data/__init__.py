...
class Data:
	...
	def ysValue(self):
		...
		if not hasattr(self, '_ysValue'):
			from .YsValue import YsValue
			self._ysValue = YsValue(self._core, self._cmd_group)
	...
	def yvalues(self):
		...
		if not hasattr(self, '_yvalues'):
			from .Yvalues import Yvalues
			self._yvalues = Yvalues(self._core, self._cmd_group)
