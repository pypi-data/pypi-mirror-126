...
class Frequency:
	...
	def get_manual(self) -> float:
		...
		Sets the frequency and triggers a sweep step manually if SWEep:MODE MAN. \n
			:return: manual: float You can select any frequency within the setting range, where: STARt is set with FREQuency:STARt STOP is set with FREQuency:STOP OFFSet is set with FREQuency:OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: Hz
		"""
	...
	def set_manual(self, manual: float) -> None:
		...
		Sets the frequency and triggers a sweep step manually if SWEep:MODE MAN. \n
			:param manual: float You can select any frequency within the setting range, where: STARt is set with FREQuency:STARt STOP is set with FREQuency:STOP OFFSet is set with FREQuency:OFFSet Range: (STARt + OFFSet) to (STOP + OFFSet) , Unit: Hz
		"""
	...
	def get_mode(self) -> enums.FreqMode:
		...
		further frequency settings. \n
			:return: mode: CW| FIXed | SWEep| LIST CW|FIXed Sets the fixed frequency mode. CW and FIXed are synonyms. The instrument operates at a defined frequency, set with command [:​SOURcehw]:​FREQuency[:​CW|FIXed]. SWEep Sets sweep mode. The instrument processes frequency (and level) settings in defined sweep steps. Set the range and current frequency with the commands: FREQuency:STARt and FREQuency:STOP, FREQuency:CENTer, FREQuency:SPAN, FREQuency:MANual LIST Sets list mode. The instrument processes frequency and level settings by means of values loaded from a list. To configure list mode settings, use the commands of the 'SOURce:LIST Subsystem'.
		"""
