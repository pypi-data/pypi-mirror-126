...
class BrowserWorkspace(object):
    ...
    def read(self, spec=None, show=False, keep=None):
        ...
        if len(list_of_saves) > 1 and \
           os.path.getsize(list_of_saves[0]) == os.path.getsize(list_of_saves[1]):
            # remove the older one that is the same
            os.remove(list_of_saves.pop(0))
            Path(list_of_saves[0]).touch(exist_ok=True)
        nlos = []
