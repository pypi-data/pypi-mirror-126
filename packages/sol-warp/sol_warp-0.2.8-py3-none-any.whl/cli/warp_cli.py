...
def main():
    try:
        ...
        raise Exception("Please use a python venv, see https://github.com/NethermindEth/warp#installation-gear for detials")
    if platform.system() == "Linux":
        kudu_pkg_dir = os.path.join(
            pkg_resources.get_distribution("sol-warp").location, "bin/linux/kudu"
        )
    elif platform.system() == "Darwin":
        if float(platform.mac_ver()[0]) < 11:
            kudu_pkg_dir = os.path.join(
                pkg_resources.get_distribution("sol-warp").location, "bin/macos/10/kudu"
            )
        elif float(platform.mac_ver()[0]) >= 11:
            kudu_pkg_dir = os.path.join(
