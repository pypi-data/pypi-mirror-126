...
@LastEditTime: 2021-11-02 11:09:32
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def prepare(self):
        ...
        # 防攻击校验
        is_attack_request, error_message = self.check_attack_request(self.request_params)
    ...
    def check_continue_request(self, request_param_dict, handler_name, app_id, object_id, expire=100):
        """
    ...
    def check_attack_request(self, request_param_dict):
        """
        :description: 攻击请求校验
        :param request_param_dict: 请求参数字典
        :return:满足条件直接拦截
        ...
        if is_filter_attack == True and request_param_dict:
            for key in request_param_dict.keys():
                if StringHelper.is_contain_sql(request_param_dict[key]) or StringHelper.is_attack(request_param_dict[key]):
    ...
    def prepare(self):
        ...
        try:
            ...
            if dict_param["open_id"] and "/client/" in self.request.uri:
                is_continue_request, error_message = self.check_continue_request(self.request_params, self.__class__.__name__, dict_param["app_id"], dict_param["open_id"])
