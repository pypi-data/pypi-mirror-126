...
from openapi_client.models import influxdb_experiment_plot_file
...
class VesslRun(object):
    ...
    def _get_experiment_from_environment(self):
        ...
        self._experiment = read_experiment_by_id(experiment_id)
        ...
        if metric_payload:
            if self._mode == MODE_NOT_STARTED:
                self.api.experiment_metrics_update_api(
                    self._experiment.id,
                    experiment_metrics_update_api_payload=ExperimentMetricsUpdateAPIPayload(
                        metrics=self._user_metric_collector.build_metric_payload(
                            metric_payload
                        )
                    ),
                )
                return self._user_metric_collector.step
            return self._user_metric_collector.log_metric(metric_payload)
    ...
    def _update_images(self, row: Dict[str, List[Image]]):
        ...
        if files:
            plot_files = [
                influxdb_experiment_plot_file.InfluxdbExperimentPlotFile(
                    step=None,
                    path=file.path,
        ...
        for f in plot_files:
            ...
            if self._mode == MODE_NOT_STARTED:
                self.api.experiment_metrics_update_api(
                    self._experiment.id,
                    experiment_metrics_update_api_payload=ExperimentMetricsUpdateAPIPayload(
                        metrics=self._user_metric_collector.build_image_payload(payload)
                    ),
                )
                continue
            self._user_metric_collector.log_image(payload)
