...
It was actually build only on linux, so you should rewrite this for windows.
...
class Screen:
    ...
    def __init__(self, id: str, saveLog: bool = True):
        """
        ...
        saveLog : BOOLEAN <optional>
            If you want to save logs, required if you use "on_stdout" event.
            Defaults to None.

        """
        self.logFilePath = self.pid = self.date = self.state = None
        if saveLog:
            self.logFilePath = f"/tmp/{id}"
        self.id: str = f"{MARK}{id}{MARK}{saveLog}{MARK}"
        self.events = {"stdout": __foo, "close": __foo}
    ...
    def on(self, call: callable):
        ...
        """
        if not call.__name__ in self.events:
            raise TypeError(f"Event '{call.__name__}' isn't available.")
        self.events[call.__name__] = call
    ...
    def setStdout(self):
        ...
        Call "on_stdout" event for every latest log from screen session's logFilePath.
        Infinite loop until his pid is None.

        Event
        ----------
        on_stdout (line : STRING)
        ...
        def _stdWatcher():
            ...
            while self.pid:
                ...
                if not line or not "\n" in line:
                    sleep(0.1)
                    continue
                Thread(target=self.events["on_stdout"], args=(
                    line,)).start()
...
def getAll() -> list:
    ...
    for line in os.popen("screen -ls").readlines()[1:-1]:
        if "£" in line:
            [pid, id, saveLog, date, state] = re.findall(
                r"(?<=\t).*?(?=\.)|(?<=\@\&\$\£).*?(?=\@\&\$\£)|(?<=\().*?(?=\))", line)
            screen = Screen(id, eval(saveLog))
            screen.pid = int(pid)
            screen.date = date
            screen.state = state
