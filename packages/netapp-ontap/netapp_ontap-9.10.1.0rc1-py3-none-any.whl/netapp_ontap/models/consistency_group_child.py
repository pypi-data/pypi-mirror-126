...
class ConsistencyGroupChildSchema(ResourceSchema):
...
An NVMe namespace must be mapped to an NVMe subsystem to grant access to the subsystem's hosts. Hosts can then access the NVMe namespace and perform I/O using the NVMe over Fabrics protocol. """
    ...
    restore_to = fields.Nested("netapp_ontap.models.consistency_group_restore_to.ConsistencyGroupRestoreToSchema", unknown=EXCLUDE, data_key="restore_to")
...
The total number of volumes across all child consistency groups contained in a consistency group is constrained by the same limit. """
    ...
    """links,luns,name,parent_consistency_group.links,parent_consistency_group.name,parent_consistency_group.uuid,provisioning_options,qos,snapshot_policy.links,snapshot_policy.name,snapshot_policy.uuid,space,svm.links,svm.name,svm.uuid,tiering,uuid,volumes,"""
    ...
    """luns,name,parent_consistency_group.links,parent_consistency_group.name,parent_consistency_group.uuid,provisioning_options,qos,snapshot_policy.name,snapshot_policy.uuid,space,svm.name,svm.uuid,tiering,volumes,"""
