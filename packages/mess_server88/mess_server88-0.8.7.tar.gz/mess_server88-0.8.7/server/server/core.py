...
import binascii
import os
from packets.client.common.descriptors import Port
from packets.client.common.utils import send_message, get_message
from packets.client.common.decos import login_required
...
class MessageProcessor(threading.Thread):
    ...
    def process_client_message(self, message, client):
        ...
        # Если клиент выходит
        elif ACTION in message and message[ACTION] == EXIT and CLIENT_NAME in message \
                and self.names[message[CLIENT_NAME]] == client:
            self.remove_client(client)
        ...
        elif ACTION in message and message[ACTION] == ADD_CONTACT and CLIENT_NAME in message and USER in message \
                and self.names[message[USER]] == client:
            self.database.add_contact(message[USER], message[CLIENT_NAME])
            try:
                send_message(client, RESPONSE_200)
            except OSError:
                self.remove_client(client)
        ...
        elif ACTION in message and message[ACTION] == REMOVE_CONTACT and CLIENT_NAME in message and USER in message \
                and self.names[message[USER]] == client:
            self.database.remove_contact(message[USER], message[CLIENT_NAME])
            try:
                send_message(client, RESPONSE_200)
            except OSError:
                self.remove_client(client)
        ...
        elif ACTION in message and message[ACTION] == USERS_REQUEST and CLIENT_NAME in message \
                and self.names[message[CLIENT_NAME]] == client:
            response = RESPONSE_202
        ...
        elif ACTION in message and message[ACTION] == PUBLIC_KEY_REQUEST and CLIENT_NAME in message:
            response = RESPONSE_511
            response[DATA] = self.database.get_pubkey(message[CLIENT_NAME])
    ...
    def autorize_user(self, message, sock):
        ...
        if message[USER][CLIENT_NAME] in self.names.keys():
            response = RESPONSE_400
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
        elif not self.database.check_user(message[USER][CLIENT_NAME]):
            response = RESPONSE_400
            ...
            hash = hmac.new(self.database.get_hash(message[USER][CLIENT_NAME]), random_str, 'MD5')
            digest = hash.digest()
            ...
            if RESPONSE in ans and ans[RESPONSE] == 511 and hmac.compare_digest(
                ...
                self.names[message[USER][CLIENT_NAME]] = sock
                client_ip, client_port = sock.getpeername()
                try:
                    send_message(sock, RESPONSE_200)
                except OSError:
                    self.remove_client(message[USER][CLIENT_NAME])
                # добавляем пользователя в список активных и если у него изменился открытый ключ
                # сохраняем новый
                self.database.user_login(
                    message[USER][CLIENT_NAME],
                    client_ip,
                    client_port,
