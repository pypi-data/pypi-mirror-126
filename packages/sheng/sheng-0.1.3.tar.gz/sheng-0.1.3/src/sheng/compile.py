...
def exec(data, debug=False):
    ...
    builtins = {
        name: obj for name, obj in inspect.getmembers(sys.modules['src.sheng.builtins'])
    }
    for identifier, builtin_name in BUILTIN_NAMES.items():
        name = Name(identifier)
        function_pointer = builtins[builtin_name]
        arguments = ArgumentSequence(
            [Name(arg) for arg in list(function_pointer.__code__.co_varnames)]
        )
        functiondef = BuiltinFunctionDef(name, arguments, function_pointer)
