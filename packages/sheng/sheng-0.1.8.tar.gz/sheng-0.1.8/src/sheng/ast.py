...
class ASTUtil(object):
    ...
    def evaluate_block(symtable, sequence):
...
class If(Statement):
    ...
    def __init__(self, expression, statements, 
        ...
        self.statement_elif = statement_elif
    ...
    def evaluate(self, symtable):
        if (self.expression.evaluate(symtable)):
            ...
            for statement_elif in self.statement_elif:
                ...
                elif (statement_elif.get_block_else()):
                    return ASTUtil.evaluate_block(
                        symtable, statement_elif.get_block_else()
                    )
            
            # If there exists an else block
            if (self.block_else):
                return ASTUtil.evaluate_block(
                    symtable, self.block_else
                )
            return None
...
class Elif(Statement):
    ...
    def get_block_else(self):
        return self.block_else
...
class Iterate(Statement):
    ...
    def evaluate(self, symtable):
        variable_identifier = self.variable.get_identifier()
        for e in self.iterable.evaluate(symtable):
            node = Constant(e)
            symtable.insert(variable_identifier, node)
