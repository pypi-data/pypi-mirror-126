...
def __setup__ (app, mntopt):
    ...
    def permission_handler (was, perms):
        if not was.request.get_header ('authorization'):
        ...
        was.request.user = User (claims)

        if was.request.user.grp == "test":
            raise was.Error ("418 You are a teapot")
    ...
    def http_error (was, status):
        raise was.Error (status)

    @app.template_global ('get_apispecs')
    def get_apispecs (was):
    ...
    def build_routes (was, base):
        ...
        def validate_routes (bases):
            match = False
            current_uri = was.request.split_uri () [0]
            if current_uri == '/':
                match = True
            else:
                for base in bases:
