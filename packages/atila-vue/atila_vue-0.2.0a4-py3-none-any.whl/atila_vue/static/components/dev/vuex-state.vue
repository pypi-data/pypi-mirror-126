<template>
  <article v-if='$debug' class='container'>
    <div class='article'>
      <h5>Vuex State</h5>
      <ul id='vuex-state'>
        <div v-for='[k, v] in Object.entries($store.state)' :key='k'>
          <li class='mb-0' v-if="k != 'context' && k != '$apispecs'"><b>{{ k }}</b>: {{ v }}</li>
        </div>
        <li><b>$apispecs</b>
          <ul>
            <li class='mb-0' v-for='[k, v] of Object.entries($apispecs)' :key='k'>
              <b>{{ k }}</b>: {{ v }}
            </li>
          </ul>
        </li>
        <li><b>context</b>
          <ul>
            <li class='mb-0' v-for='[k, v] of Object.entries(contextOrEmpty)' :key='k'>
              <b>{{ k }}</b>: {{ v }}
            </li>
          </ul>
        </li>
