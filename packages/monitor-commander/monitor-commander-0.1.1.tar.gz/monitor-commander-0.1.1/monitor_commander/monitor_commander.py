...
def _locate_config(path: Optional[str]):
    ...
    if path and re.fullmatch("[A-Za-z0-9]+", path) is None:
        print_err("--config contains forbiden chars. Please provide an absolute path")
        sys.exit(2)
...
def _detect_monitors(config: _Config) -> List[Dict[str, _MonitorProperties]]:
    for i in range(2):
        if i != 0:
            print_err("retry...")
            sleep(1)
        proc = run([*_ddcutil(config), "detect", "--async"], stdout=PIPE, text=True, check=False)
        out = proc.stdout
...
def main():
    ...
    ).completer = FilesCompleter  # type: ignore # Missing stub

    argv = sys.argv
    if "_ARGCOMPLETE" in os.environ and "COMP_LINE" in os.environ and split_line:
        _, _, _, argv, _ = split_line(os.environ["COMP_LINE"])
    args, _ = config_parser.parse_known_args(argv[1:])
    ...
    helptxt = "Freeze one or multiple monitors. The monitor will not be set unless explicitelly specified with the --monitor arg"
    ...
    ).completer = ChoicesCompleter(  # type: ignore # Missing stub
        {preset for monitor in config.monitors for preset in monitor.presets}
    )
