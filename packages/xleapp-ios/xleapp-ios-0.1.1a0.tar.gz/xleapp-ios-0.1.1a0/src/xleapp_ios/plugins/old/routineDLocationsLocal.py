...
class RoutineDLocationsLocal(ab.Artifact):
        ...
        if version.parse(iOSversion) < version.parse("12"):
            ...
            if version.parse(iOSversion) >= version.parse("14"):  # Tested 14.1
                ...
                if usageentries > 0:
                    ...
                    report.start_artifact_report(
                        report_folder, 'RoutineD Locations Entry', description
                    )
                    report.add_script()
                    data_headers = (
                        'Timestamp',
            ...
            if version.parse(iOSversion) >= version.parse("12"):
                ...
                if usageentries > 0:
                    ...
                    report = ArtifactHtmlReport('Locations')
                    report.start_artifact_report(
                        report_folder, 'RoutineD Transtition Start', description
                    )
                    report.add_script()
                    data_headers = (
                        'Timestamp',
            ...
            if version.parse(iOSversion) >= version.parse("12"):
                ...
                if usageentries > 0:
                    ...
                    report = ArtifactHtmlReport('Locations')
                    report.start_artifact_report(
                        report_folder, 'RoutineD Transtition Stop', description
                    )
                    report.add_script()
                    data_headers = (
                        'Timestamp',
