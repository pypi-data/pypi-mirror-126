import click
import numpy as np
import pandas as pd
import starfile
import dynamotable
from eulerangles import convert_eulers
from eulerangles import euler2matrix
from eulerangles import matrix2euler

from .utils import sanitise_dynamo_table_filename, reextract_table_filename, normalize


def relion2dynamo(relion_star_file, output_dynamo_table_file):
    """
    Converts a relion STAR file into a Dynamo table file.
    """
    # Read STAR file
    relion_star = starfile.read(relion_star_file)
    
    # Remove optics table
    relion_star = pd.DataFrame.from_dict(relion_star['particles']) 
    
    # Initialise empty dict for dynamo
    dynamo_data = {}

    # Get XYZ positions and put into data
    for axis in ('x', 'y', 'z'):
        relion_heading = 'rlnCoordinate' + axis.upper()
        dynamo_data[axis] = relion_star[relion_heading]

    # Get euler angles and convert to dynamo convention (only if eulers present in STAR file)
    if 'rlnAngleRot' in relion_star.columns:
        eulers_relion = relion_star[['rlnAngleRot', 'rlnAngleTilt', 'rlnAnglePsi']].to_numpy()			       
        eulers_dynamo = convert_eulers(eulers_relion,
                                       source_meta='relion',
                                       target_meta='dynamo')
				       
        # add eulers from rlntomosubtomogram and rlnangle together
        if 'rlnTomoSubtomogramRot' in relion_star.columns:
	
            eulers_relion_subtom = relion_star[['rlnTomoSubtomogramRot', 'rlnTomoSubtomogramTilt', 'rlnTomoSubtomogramPsi']].to_numpy()
            eulers_dynamo_subtom = convert_eulers(eulers_relion_subtom,
                                           source_meta='relion',
                                           target_meta='dynamo')
	
            eulers_dynamo_matrix = euler2matrix(eulers_dynamo,
                                           axes='zyz',                      
                                           intrinsic=True,
                                           right_handed_rotation=True)
            eulers_dynamo_subtom_matrix = euler2matrix(eulers_dynamo_subtom,
                                           axes='zyz',                      
                                           intrinsic=True,
                                           right_handed_rotation=True)
				       
            eulers_dynamo_product = np.matmul(eulers_dynamo_matrix,eulers_dynamo_subtom_matrix)
	
            eulers_dynamo = matrix2euler(eulers_dynamo_product,
                                           axes='zyz', 
                                           intrinsic=True,
                                           right_handed_rotation=True)			       
	
        dynamo_data['tdrot'] = eulers_dynamo[:, 0]
        dynamo_data['tilt'] = eulers_dynamo[:, 1]
        dynamo_data['narot'] = eulers_dynamo[:, 2]
    
    # Add tomo to column
    
    dynamo_data['tomo'] = relion_star['rlnTomoName'].str.split('_').str[1]
        
    # add object number to column 21 in Dynamo table
    
    if 'rlnObjectNumber' in relion_star.columns:
        dynamo_data['reg'] = relion_star['rlnObjectNumber']
	
    #add class number to table
    
    if 'rlnClassNumber' in relion_star.columns:
        dynamo_data['class'] = relion_star['rlnClassNumber']
	            
    # Convert to DataFrame
    df = pd.DataFrame.from_dict(dynamo_data)

    # Write table file
    output_dynamo_table_file = sanitise_dynamo_table_filename(output_dynamo_table_file)
    click.echo(
        f"Writing out Dynamo table file '{output_dynamo_table_file}' and corresponding table map file with appropriate info...\n")
    dynamotable.write(df, output_dynamo_table_file)

    click.echo(f"\nDone! Converted Relion output '{relion_star_file}' into Dynamo input files")
    return


@click.command()
@click.option('--input_star_file', '-i', 'relion_star_file',
              prompt='Input relion STAR file',
              type=click.Path(exists=True),
              required=True)
@click.option('--output_table_file', '-o', 'dynamo_table_file',
              type=click.Path(exists=False),
              prompt='Output dynamo table file')
def cli(relion_star_file, dynamo_table_file):
    relion2dynamo(relion_star_file, dynamo_table_file)
