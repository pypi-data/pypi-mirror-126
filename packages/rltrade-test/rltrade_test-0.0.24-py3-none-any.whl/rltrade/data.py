...
class FeatureEngineer:
    ...
    def add_vix_fix(self,data,years):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp[f'vix_fix_{years}year'] = ((temp['close'].rolling(days).max() - temp['low'])/temp['close'].rolling(days).max()) * 100
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(temp[["tic", "date", f'vix_fix_{years}year']], on=["tic", "date"], how="left")
        return df
