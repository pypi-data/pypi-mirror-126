...
class FeatureEngineer:
    ...
    def create_data(self,df):
        ...
        if 'vix_fix' in self.additional_indicators:
            df = self.add_vix_fix(df)
    ...
    def add_sharpe(self,data,years):
        ...
        for ticker in unique_ticker:
            ...
            temp[f'sharpe_{years}year'] = temp['daily_return'].rolling(days).mean() / temp['daily_return'].rolling(days).std()
    ...
    def add_sortino(self,data,years):
        ...
        for ticker in unique_ticker:
            ...
            temp[f'sortino_{years}year'] = temp['daily_negative_return'].rolling(days).mean() / temp['daily_negative_return'].rolling(days).std()
            indicator_df = indicator_df.append(temp, ignore_index=True)
    ...
    def add_clamar(self,data,years):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp['daily_return'] = temp['close'].pct_change(1)
            temp[f'calamar_{years}year'] = temp['daily_return'].rolling(days).mean()/temp['daily_return'].diff(1).rolling(days).min()
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(indicator_df[["tic", "date", f'calamar_{years}year']], on=["tic", "date"], how="left")
        return df
    
    def add_vix_fix(self,data):
        df = data.copy()
        unique_ticker = df.tic.unique()
        indicator_df = pd.DataFrame()
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp['vix_fix'] = ((temp['close'].rolling(22).max() - temp['low'])/temp['close'].rolling(22).max()) * 100
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(temp[["tic", "date", 'vix_fix']], on=["tic", "date"], how="left")
        return df
