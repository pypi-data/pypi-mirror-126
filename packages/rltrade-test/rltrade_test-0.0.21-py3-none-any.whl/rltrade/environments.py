...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.all_metrics = ["asset", "cagr", "sharpe", "calamar", "skew", "kurtosis" ]
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            if self.target_metrics == "asset":
                self.reward = end_total_asset - begin_total_asset 
                self.rewards_memory.append(self.reward)
                self.reward *= self.reward_scaling
            
            if self.target_metrics == "cagr":
                years = self.day/365
                self.reward = ((end_total_asset/begin_total_asset) ** (1/years)) - 1
                self.rewards_memory.append(self.reward)
                # self.reward *= self.reward_scaling
            
            if self.target_metrics == "sharpe":
                ...
                self.reward = (252**0.5)*df_temp['daily_return'].mean()/(df_temp['daily_return'].std()+1e-8)
                self.reward = 0 if math.isnan(self.reward) else self.reward
                self.rewards_memory.append(self.reward)
                # self.reward *= self.reward_scaling
            
            if self.target_metrics == "calamar":
                df_temp = pd.DataFrame(self.asset_memory)
                df_temp.columns = ['account_value']
                df_temp['daily_return'] = df_temp['account_value'].pct_change(1)
                avg_return = df_temp['daily_return'].mean()
                max_drawdown = df_temp['daily_return'].diff(1).min()
                self.reward = avg_return/ (max_drawdown + 1e-8)
                self.reward = 0 if math.isnan(self.reward) else self.reward
                self.rewards_memory.append(self.reward)
                # self.reward *= self.reward_scaling
            
            if self.target_metrics == "skew":
                df_temp = pd.DataFrame(self.asset_memory)
                df_temp.columns = ['account_value']
                df_temp['daily_return'] = df_temp['account_value'].pct_change(1)
                df_temp.fillna(0,inplace=True)
                self.reward = skew(df_temp['daily_return'])
                self.rewards_memory.append(self.reward)
                # self.reward *= self.reward_scaling
            
            if self.target_metrics == "kurtosis":
                df_temp = pd.DataFrame(self.asset_memory)
                df_temp.columns = ['account_value']
                df_temp['daily_return'] = df_temp['account_value'].pct_change(1)
                df_temp.fillna(0,inplace=True)
                self.reward = kurtosis(df_temp['daily_return'])
                self.rewards_memory.append(self.reward)
                # self.reward *= self.reward_scaling
