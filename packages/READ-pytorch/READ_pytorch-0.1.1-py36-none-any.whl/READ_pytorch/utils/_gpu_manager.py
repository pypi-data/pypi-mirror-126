# -*- coding: utf-8 -*-
"""
GPUManager code gently borrowed from
...
if check_gpus():
    ...
    class GPUManager():
        ...
        def auto_choice(self,mode=0, gpu_num=1):
            '''
            mode:
            ...
            for j in range(gpu_num):
                chosen_gpu[j]['specified']=True
                index=chosen_gpu[j]['index']
                print('Using GPU {i}:\n{info}'.format(i=index,info='\n'.join([str(k)+':'+str(v) for k,v in chosen_gpu[j].items()])))
                print('<================================>')
...
if __name__ == "__main__":
    GPUManager().auto_choice(gpu_num=2)