...
def command_generator(account_feature_groups: list) -> str:
    ...
    for env in args.env:
        ...
        tags = f'--tags="{" and ".join(args.itags)} {f"".join(" and not {}".format(t) for t in args.etags)}" '
        ...
        features_list = " ".join("\"{}\"".format(feature_path) for feature_path in feature_set)
        ...
        cmd = (f'behave -D parallel="True" -D env="{env}" ' + optional_retry + optional_browser + optional_userdata +
               ...
               " ".join("\"{}\"".format(feature_path) for feature_path in feature_set))
        commands.append(cmd)
