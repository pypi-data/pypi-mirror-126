...
class Map:
    ...
    def random_line(self, width):
        ...
        """
        while True:
            x = self.random.integers(0, self.height)
            y = self.random.integers(0, self.width)

            if self.wall(x, y):
                continue

            vertical = bool(self.random.integers(2))
            signum = self.random.choice([-1, 1])
    ...
    def __init__(self, map_name, surround_with_walls, rng):
        ...
        self.map_path = "./maps/" + self.map_name
        if not os.path.exists(self.map_path):
            self.map_path = Path(__file__).parent.joinpath("maps", self.map_name)
            if not os.path.exists(self.map_path):
                logging.error(
...
class Environment:
    ...
    def __init__(self, rt_args, clone_call=False, random_number_generator=None):
        ...
        Args:
            ...
            random_number_generator (numpy.random.Generator, optional): random number generator, if None, then a new Generator is created using the seed in rt_args.seed
        """
        ...
        self.random = (
            ...
            else np.random.default_rng(rt_args.seed)
        )
        ...
        if not clone_call:
            ...
            if self.random_start:
                x = self.random.integers(low=0, high=self.map.height)
                y = self.random.integers(low=0, high=self.map.width)
                while self.map.terminal(x, y):
                    x = self.random.integers(low=0, high=self.map.height)
                    y = self.random.integers(low=0, high=self.map.width)
                self.position = np.array([x, y])
            else:
                self.position = np.array(self.random.choice(self.map.starters))

            if self.random_velocity:
                vx = self.random.integers(0, rt_args.maximal_random_velocity)
                vy = self.random.integers(0, rt_args.maximal_random_velocity)
            else:
                vx = 0
                vy = 0
    ...
    def clone(self, deep=False, clone_rng=False):
        """creates a clone of the current instance.

        Args:
            deep (bool, optional): [description]. Defaults to False.
            clone_rng (bool, optional): Controls if the random number generator is also cloned, or whether a new one is created. Defaults to False.
        ...
        oe = Environment(
            ...
            clone_call=True,
            random_number_generator=copy.deepcopy(self.random) if clone_rng else None,
        )
        if deep:
    ...
    def discretize_position(self, pos):
        ...
        """
        discrete_x = round(pos[0])
        discrete_y = round(pos[1])
        ...
        if graphical:
            ...
            if not hide_positions:
                for i, position in enumerate(self.path):
                    ...
                    x, y = self.discretize_position(position)
                    if not (
    ...
    def reward(self, old_position):
        ...
        if self.rt_args.continuous:
            old_position = self.discretize_position(old_position)
            position_discret = self.discretize_position(self.position)
    ...
    def light_step(self, action, clone_rng=False):
        ...
        """
        other_env = self.clone(clone_rng=clone_rng)
    ...
    def step(self, action):
        ...
        # NOISE!
        if self.noisy:
            if self.random.random() < self.rt_args.noise_probability:
    ...
    def reset(self):
        ...
        """
        if self.random_start:
            x = self.random.integers(0, self.map.height)
            y = self.random.integers(0, self.map.width)
            while self.map.terminal(x, y):
                x = self.random.integers(0, self.map.height)
                y = self.random.integers(0, self.map.width)
            self.position = np.array([x, y])
        ...
        if self.random_velocity:
            vx = self.random.integers(0, self.rt_args.maximal_random_velocity)
            vy = self.random.integers(0, self.rt_args.maximal_random_velocity)
        else:
            vx = 0
            vy = 0
        self.velocity = np.array((vx, vy))
    ...
    def get_state(self):
        ...
        """
        if self.rt_args.continuous:
            x, y = self.discretize_position(self.position)
        else:
            x, y = self.position
        if not self.done:
    ...
    def get_state_rep(self, pos, velo=None):
        ...
        if self.rt_args.continuous:
            x, y = self.discretize_position(pos)
        else:
            x, y = pos
