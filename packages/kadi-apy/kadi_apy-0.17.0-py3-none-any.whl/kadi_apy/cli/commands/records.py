...
def get_file(record, file_id, filepath, pattern, exclude_pattern, force, use_folder):
    """Download one file, all files or files with pattern from a record."""

    record.get_file(filepath, force, file_id, pattern, exclude_pattern, use_folder)
...
@records.command()
@apy_command
@id_identifier_options(class_type="record", helptext="to export.")
@option(
    "export-type",
    char="e",
...
def export(record, export_type, path, name, force, pipe):
    """Export the record using a specific export type."""

    record.export(export_type, path, name, force, pipe)
