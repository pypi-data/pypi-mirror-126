...
class Batch_Affine_3D(object):
    ...
    def __call__(self, images):
        ...
        for batch_index in range(bs):
            ...
            padding_tuple = (torch.where(transformed_coords == -1)[-3], torch.where(transformed_coords == -1)[-2], torch.where(transformed_coords == -1)[-1])
            if not seg:
                ...
                images[batch_index, :][padding_tuple] = self.pad_value
            else:
                images[batch_index, :, :, :] = images[batch_index, transformed_coords[0], transformed_coords[1], transformed_coords[2]]
    ...
    def gpu_calculate_coornidates(self, coords, matrices, img_size):
        ...
        coords[coords < 0 ] = -1
        coords[0][coords[0] > img_size[0] - 1] = -1
        coords[1][coords[1] > img_size[1] - 1] = -1
        coords[2][coords[2] > img_size[2] - 1] = -1
