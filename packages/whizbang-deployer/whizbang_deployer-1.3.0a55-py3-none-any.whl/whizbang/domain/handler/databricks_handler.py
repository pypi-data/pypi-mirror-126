...
class DatabricksHandler(HandlerBase, IDatabricksHandler):
    ...
    def check_job_statuses(self, databricks_url: str, solution_name: str):
        client_args = DatabricksClientArgs(host=f'https://{databricks_url}')
        databricks_state_path = path_defaults.get_databricks_state_path(app_config=self._app_config,
                                                                        solution_name=solution_name)
        ...
        for job in jobs_to_check:
            status = self.__databricks_job_manager.get_job_status(client_args=client_args,
                                                                  job_name=job["name"])
            if status == 'FAILED':
                print(f'Warning: The job {job["name"]} failed. Please check the logs in your databricks workspace.')
            elif status == 'NOT RUN':
                print(f'Warning: The job {job["name"]} was not run.')
            else:
                print(f'The job {job["name"]} Succeeded.')
