...
class BNGBase(cement.Controller):
    ...
    @cement.ex(
        ...
        arguments=[
            ...
            (
                ["-i2", "--input2"],
                {
                    "help": "Path to a second BNGL model, only to be used by the 'diff' mode",
                    "default": None,
                    "type": str,
                    "required": False,
                },
            ...
            (
                ["-m", "--mode"],
                {
                    "help": "(optional) Changes visualization mode. Options are normal and diff. "
                    + "Diff allows you to enter a second input file with -i2 or --input2 and the "
                    + "output will now be a difference graph.",
                    "default": "normal",
                    "type": str,
                },
            ),
    ...
    def visualize(self):
        ...
        if args.mode == "diff":
            assert not (
                args.input2 is None
            ), "Diff mode requires two input files using -i2 or --input2"
