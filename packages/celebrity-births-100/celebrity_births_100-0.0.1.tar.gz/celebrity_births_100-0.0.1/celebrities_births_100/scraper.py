'''
This script contains a class Scraper that gets the HTML code of a
...
class Scraper:
    '''
    This class is used to get HTML code from Wikipedia and extract the name of celebrities that
    were born on a certain date
    
    Attributes:
        ROOT (str): 'https://en.wikipedia.org/wiki/'
    '''
    ...
    def get_celebrities(self, date: str = None) -> list:
        '''
        This method returns a list of the celebrities found on Wikipedia born on the specified date.
        If the date is not specified returns a list of the celebrities found on Wikipedia born on January 1st.

        Args:
            date (str): The string representation of the date.

        Returns:
            list: A list of the celebrities born on date.
        '''
        if date is None:
            date = 'January_1'
...
if __name__ == '__main__':
    scraper = Scraper()
    celebrities = scraper.get_celebrities('February_3')
    print(celebrities)
