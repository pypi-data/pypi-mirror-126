...
from kelvin.sdk.app.utils import duration, field_info, flatten
from kelvin.sdk.datatype.message import Message
from kelvin.sdk.datatype.utils import is_protected
...
class ExtractorApp(DataApplication):
    ...
    def on_initialize(
        ...
        if not super().on_initialize(configuration, app_configuration, parameters):
            return False
        ...
        for name, info in self.interface.inputs.items():
            message_type = info.data_type

            getters = self.fields[name] = {}
            for field, T in flatten(field_info(Message.get_type(message_type))):
                column = f"{name}.{field}"
    ...
    def write_data(self, cutoff: Optional[int] = None) -> None:
        ...
        if config.retention_window is not None:
            self.clean_output_path(cutoff - int(duration(config.retention_window) * 1e9))
    ...
    def process(self) -> None:
        ...
        if last_time_of_validity >= self.last_write + int(duration(config.batch) * 1e9):
            self.write_data(last_time_of_validity - int(duration(config.delay) * 1e9))
