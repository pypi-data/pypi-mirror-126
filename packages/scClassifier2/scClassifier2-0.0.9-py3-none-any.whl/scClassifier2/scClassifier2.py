    ...
    def setup_networks(self):
        ...
        self.encoder_z = MLP(
            [self.input_size + self.output_size] + hidden_sizes + [
                [z_dim * self.output_size, z_dim * self.output_size]],
            activation=nn.Softplus,
...
def main():
    ...
    ), "sup_label_file must be provided"
    assert (
    ...
    if args.sup_data_file is not None:
        data_loaders['sup'], data_loaders['valid'] = setup_data_loader(
            SingleCellCached, args.sup_data_file, args.sup_label_file, label2class, 'sup', args.cross_validation_fold,
            args.log_transform, args.cuda, args.float64, args.batch_size
        )
        sup_num = len(data_loaders['sup'])
    if args.unsup_data_file is not None:
        data_loaders['unsup'], _ = setup_data_loader(
            SingleCellCached, args.unsup_data_file, args.unsup_label_file, label2class, 'unsup', 0, args.log_transform,
            args.cuda, args.float64, args.batch_size
        )
    ...
    try:
        ...
        for i in range(0, args.num_epochs):
            ...
            avg_epoch_losses_unsup = map(lambda v: v / unsup_num, epoch_losses_unsup) if unsup_num > 0 else [0] * len(
                epoch_losses_unsup)
