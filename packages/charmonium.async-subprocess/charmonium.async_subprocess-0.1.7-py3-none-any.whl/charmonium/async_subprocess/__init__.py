...
class CalledProcessError(Exception):
    ...
    def __init__(
        self,
        args2: Sequence[Union[str, bytes, Path]],
    ...
    def __str__(self) -> str:
        def converter(arg: Union[str, bytes, Path]) -> str:
...
command:
  {shlex.join(map(converter, self.args2))}"""
        if self.stdout:
            value = (
...
async def run(
    args: Sequence[Union[str, bytes, Path]],
    cwd: Optional[Path] = None,
    env: Optional[Union[Mapping[bytes, bytes], Mapping[str, str]]] = None,
    env_override: Optional[Union[Mapping[bytes, bytes], Mapping[str, str]]] = None,
    ...
    env = cast(Dict[str, str], dict(env if env is not None else os.environ))  # type: ignore
    if env_override:
        env.update(cast(Mapping[str, str], env_override))
    ...
    return subprocess.CompletedProcess(args, rc, cap_stdout, cap_stderr)  # type: ignore
