...
class Temperature:
	"""Temperature commands group definition. 4 total commands, 2 Sub-groups, 1 group commands"""
	...
	def operating(self):
		...
		if not hasattr(self, '_operating'):
			...
			self._operating = Operating(self._core, self._cmd_group)
	...
	def exceeded(self):
		...
		if not hasattr(self, '_exceeded'):
			from .Exceeded import Exceeded
			self._exceeded = Exceeded(self._core, self._cmd_group)
	...
	def clone(self) -> 'Temperature':
		...
		new_group = Temperature(self._core, self._cmd_group.parent)
