...
class Salignment:
	...
	def lvalid(self):
		...
		if not hasattr(self, '_lvalid'):
			from .Lvalid import Lvalid
			self._lvalid = Lvalid(self._core, self._cmd_group)
		return self._lvalid
	...
	def ulimit(self):
		...
		if not hasattr(self, '_ulimit'):
			...
			self._ulimit = Ulimit(self._core, self._cmd_group)
		return self._ulimit
	...
	def llimit(self):
		...
		if not hasattr(self, '_llimit'):
			from .Llimit import Llimit
			self._llimit = Llimit(self._core, self._cmd_group)
		return self._llimit
	...
	def xvalues(self):
		...
		if not hasattr(self, '_xvalues'):
			from .Xvalues import Xvalues
			self._xvalues = Xvalues(self._core, self._cmd_group)
	...
	def clone(self) -> 'Salignment':
		...
		new_group = Salignment(self._core, self._cmd_group.parent)
