...
class Slot:
	...
	def rxFilter(self):
		...
		if not hasattr(self, '_rxFilter'):
			from .RxFilter import RxFilter
			self._rxFilter = RxFilter(self._core, self._cmd_group)
	...
	def txFilter(self):
		...
		if not hasattr(self, '_txFilter'):
			from .TxFilter import TxFilter
			self._txFilter = TxFilter(self._core, self._cmd_group)
