...
class Spoint:
	...
	def rewait(self):
		...
		if not hasattr(self, '_rewait'):
			from .Rewait import Rewait
			self._rewait = Rewait(self._core, self._cmd_group)
		return self._rewait
	...
	def join(self):
		...
		if not hasattr(self, '_join'):
			...
			self._join = Join(self._core, self._cmd_group)
		return self._join
	...
	def clone(self) -> 'Spoint':
		...
		new_group = Spoint(self._core, self._cmd_group.parent)
