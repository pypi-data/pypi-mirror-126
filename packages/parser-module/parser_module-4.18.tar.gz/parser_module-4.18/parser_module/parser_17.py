...
URL = 'http://duma.gov.ru/duma/administration/'
...
def parser():
    ...
    if html.status_code == 200:
        soup = BeautifulSoup(html.text, 'html.parser')
        people_blocks = soup.find('ul', class_='persons-list').find_all('li')

        for person_block in people_blocks:
            person_pos = person_block.find('div', class_='person__position').find('a').get_text()
            if person_pos.lower().find('первый') != -1:
                people.append(get_person(person_block.find('h2').find('a').get('href')))
