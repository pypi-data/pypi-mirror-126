...
@Pipe
def islice(iterable: Iterable, start: int, stop: int, step: "int | None" = None) -> Iterable:
...
@Pipe
def remove(iterable: Iterable, values: Iterable) -> Iterable:
    if not hasattr(values, "__iter__"):
        values = {values}
    return (i for i in iterable if i not in values)

@Pipe
def apply(iterable: Iterable, function: "function") -> Iterable:
    return function.__call__(iterable)

if __name__ == "__main__":
    select_every_third = enumerate() >> apply(lambda iterable: (y for x, y in iterable if x % 3 == 0))
    print(range(10) | select_every_third() | as_list())