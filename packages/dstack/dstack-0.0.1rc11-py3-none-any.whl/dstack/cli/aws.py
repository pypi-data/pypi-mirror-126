...
def info_func(_: Namespace):
    try:
        ...
        if response.status_code == 200:
            response_json = response.json()
            print("AWS Access Key ID: " + (sensitive(response_json.get("aws_access_key_id")) or "None"))
            print("AWS Secret Access Key: " + (sensitive(response_json.get("aws_secret_access_key")) or "None"))
            print("Region name: " + (response_json.get("aws_region") or "None"))
            print("Artifacts S3 bucket: " + (response_json.get("artifacts_s3_bucket") or "None"))
        else:
            response.raise_for_status()
    except ConfigurationError:
        sys.exit(f"Call 'dstack login' or 'dstack register' first")
...
def register_parsers(main_subparsers):
    ...
    configure_parser.add_argument("--artifacts-s3-bucket", type=str, dest="artifacts_s3_bucket", nargs='?',
                                  const="", action='store')
    configure_parser.add_argument("--force", help="Don't ask for confirmation", action="store_true")
    ...
    clear_parser.add_argument("--force", help="Don't ask for confirmation", action="store_true")
