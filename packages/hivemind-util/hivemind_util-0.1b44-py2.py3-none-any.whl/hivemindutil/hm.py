...
def getResults(taskId, baseUrl, apiKey, incIncompleteInstances=False,
               incIterations=True, retries=1, raiseForStatus=False):
    ...
    with requests.Session() as s:
        ...
        while paginatedUrl is not None:
            ...
            while attempt >= 0:
                ...
                ## Retry server/timeout errors
                if response.status_code // 100 == 5:
                    print('Retrying API call.')
                ## Break if not errored
                elif response.status_code // 100 == 2:
                    attempt = -1
                ## Break on authentication error
...
def getInstances(taskId, baseUrl, apiKey, expandSummary=True, retries=1, raiseForStatus=False):
    authHeader = makeAuthHeader(apiKey)
    baseUrl = cleanBaseUrl(baseUrl)
    with requests.Session() as s:
        s.headers.update(authHeader)
...
def getInstancesAndResults(taskId, baseUrl, apiKey, expandSummary=True,
                           incIncompleteInstances=False, incIterations=True, retries=1):
    # Make API calls for both instances and results, and transfer data from the former to the latter.
    instances = getInstances(taskId, baseUrl, apiKey, expandSummary, retries=retries)
    results = getResults(taskId, baseUrl, apiKey, incIncompleteInstances, incIterations, retries=retries)
