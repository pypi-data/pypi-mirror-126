...
from fzq_scnu import constants
'''
！！！
...
# 进行统一操作，目的是在字典transmit中放入需要传递的所有可以用于绘制文字、图像等的参数
    ...
    def message(hardwarepos_pic_name,hardwarepos_name,io_pwm,io_pwm_num,text1,text2,page_info):
        ...
        transmit_right[dict['num_count']] = [hardwarepos_pic_name, page_info[1], hardwarepos_name,
...
class Mecanum_wheel():
    ...
    def car_hongwai(self):
        dict['Hongwai_pos']['hongwai_left_x'] = self.actor.x - self.actor.height * 6 / 11 * math.sin(
            math.radians(self.actor.angle)) - 10 * math.cos(math.radians(self.actor.angle)) - 1 / 2
        dict['Hongwai_pos']['hongwai_left_y'] = self.actor.y - self.actor.height * 6 / 11 * math.cos(
            math.radians(self.actor.angle)) + 10 * math.sin(math.radians(self.actor.angle)) - 1 / 2
        dict['Hongwai_pos']['hongwai_right_x'] = self.actor.x - self.actor.height * 6 / 11 * math.sin(
            math.radians(self.actor.angle)) + 10 * math.cos(math.radians(self.actor.angle)) - 1 / 2
        dict['Hongwai_pos']['hongwai_right_y'] = self.actor.y - self.actor.height * 6 / 11 * math.cos(
            math.radians(self.actor.angle)) - 10 * math.sin(math.radians(self.actor.angle)) - 1 / 2
