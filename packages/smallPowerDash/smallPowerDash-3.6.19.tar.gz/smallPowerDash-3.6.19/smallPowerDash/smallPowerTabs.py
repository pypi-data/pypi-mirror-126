...
class RealTimeSmallPowerMultiUnit(tabsD.RealTimeTagMultiUnit):
    def __init__(self,app,connParameters=None,baseId='rtmsp_'):
        self.cfg = cfs.ConfigFilesSmallPower_RealTime(connParameters=connParameters)
        tabsD.RealTimeTagMultiUnit.__init__(self,app,self.cfg,baseId,plugfunc=self.updatefigtraces)
        self.tabLayout = self._buildLayout(widthG=85,
                    defaultTags=self.cfg.getTagsTU('GFC_01')+self.cfg.getTagsTU('GFD_00_PT'),
                    val_window=30*1,
                    val_refresh=60,
                    min_refresh=5,
                    min_window=1,
    ...
    def updatefigtraces(self,fig):
        import plotly.graph_objects as go
        fig=go.Figure(fig)
        listBleuFonce=['Dark Blue','Egyptian Blue','Blue (Pantone)','Midnight Blue','Sapphire','Ultramarine','Resolution Blue']
        listBleuClair=['Argentinian Blue','Bleu de France','Baby blue','Uranian Blue','Ruddy Blue','Light blue','Periwinkle']
        tagBleuFonce =['GFD_01_PT' ,'GFC_01_PT', 'GFD_02_PT', 'GFC_02_PT']
        tagBleuClair = ['GFD_00_PT', 'GFC_00_PT']
        kf,kc=0,0
        for tag in fig.data:
            if len([k for k in tagBleuFonce if k in tag.name])>0:
                if not not tag.marker.color:
                    tag.marker.color = self.cfg.colorPalettes['blues']['hex'][listBleuFonce[kf]]
                if not not tag.line.color:
                    tag.line.color = self.cfg.colorPalettes['blues']['hex'][listBleuFonce[kf]]
                kf+=1
            elif len([k for k in tagBleuClair if k in tag.name])>0:
                if not not tag.marker.color:
                    tag.marker.color = self.cfg.colorPalettes['blues']['hex'][listBleuClair[kf]]
                if not not tag.line.color:
                    tag.line.color = self.cfg.colorPalettes['blues']['hex'][listBleuClair[kf]]
                kc+=1
        return fig
