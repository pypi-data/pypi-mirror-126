...
class JMeterExecutor(ScenarioExecutor):
    ...
    def __set_jvm_properties(self):
        ...
        if heap_size is not None:
            self.log.debug("bzt-tcps version:v0.1")
            self.log.debug("Setting JVM heap size -Xms -Xmx to %s, and -XX:MaxMetaspaceSize=256m", heap_size)
            self.env.add_java_param({"JVM_ARGS": "-Xms%s" % heap_size})
            self.env.add_java_param({"JVM_ARGS": "-Xmx%s" % heap_size})
            self.env.add_java_param({"JVM_ARGS": "-XX:MaxMetaspaceSize=256m"})
