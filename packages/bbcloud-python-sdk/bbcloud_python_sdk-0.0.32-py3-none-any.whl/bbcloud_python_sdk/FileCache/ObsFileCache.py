...
class ObsFileCache():
    ...
    def set(self, key, file_path, del_local=True):
        ...
        """
        if not os.path.exists(file_path) and not re.match("http", file_path) and not isinstance(file_path, str) \
                and not isinstance(file_path, dict) and not isinstance(file_path, list) and file_path is not None:
