...
def basic_authorization(credentials: list, realm: str):
    def basic_auth():
        return _basic_authorization(credentials, realm)

    return basic_auth


def _basic_authorization(credentials: list = None, realm: str = "Welcome! Please enter your username and password."):
    if not credentials:
        credentials = [{"admin": "admin"}]
    if "Authorization" not in _HEADERS:
        set_status(401)
        set_header("WWW-Authenticate", "Basic realm={}, charset=\"UTF-8\"".format(realm))
        return False
    authorization = _HEADERS["Authorization"].split("Basic ")[-1]
    u, p = b64d_and_utf8d(authorization).split(":")
