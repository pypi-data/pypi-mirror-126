...
class DynamoConversions:
    ...
    @classmethod
    def list_and_dict_to_dynamo(cls, value: Union[list, dict]):
        return json.dumps(value)

    @classmethod
    def list_and_dict_from_dynamo(cls, value: str):
        return json.loads(value)

    @classmethod
    def tuple_to_dynamo(cls, value: tuple):
        return json.dumps(list(value))

    @classmethod
    def tuple_from_dynamo(cls, value: str):
        return tuple(json.loads(value))

    @classmethod
    def string_set_to_dynamo(cls, value: set):
    ...
    @classmethod
    def bytes_set_to_dynamo(cls, value):
        for old_elem in value:
    ...
    @classmethod
    def bytes_set_from_dynamo(cls, value):
        new_set = set()
        for old_elem in value:
    ...
    @classmethod
    def bytearray_set_from_dynamo(cls, value):
        new_set = set()
        for old_elem in value:
            # boto3 uses its own binary type old_elm.value gets the binary value in the bytes python type
    ...
    @classmethod
    def number_set_to_dynamo(cls, value):
        new_set = set()
        for old_elem in value:
    ...
    @classmethod
    def int_set_from_dynamo(cls, value):
        new_set = set()
        for old_elem in value:
    ...
    @classmethod
    def float_set_from_dynamo(cls, value):
        new_set = set()
        for old_elem in value:
    ...
    @classmethod
    def gen_enum_from_dynamo_for_enum_type(cls, enum_type):
...
class DynamoCrudRepository(CrudRepository, metaclass=abc.ABCMeta):
    ...
    def find_collection(
        ...
        Args:
            ...
            keys(dict, optional): a dict {"attribute_name": "attribute_vale"} with a single key or a key pair
            ...
            limit(int, optional): the maximum items that should be returned.
            Defaults to -1 which will return the maximum items per DynamoDB query

        Returns:
            a list of instance objects and the number of objects
        """
        query_args = {}
...
class SingleTableDynamoCrudRepository(DynamoCrudRepository):
    ...
    def count(self):
        ...
        try:
            count = len(self.find_collection(key_condition_expression, index_name))
    ...
    def find_all(self):
        ...
        entity_list = self.find_collection(key_condition_expression, index_name)
        return entity_list
