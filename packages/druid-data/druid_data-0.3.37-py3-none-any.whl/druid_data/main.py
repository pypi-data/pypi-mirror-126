        ...
        if exclusive_start_key is not None:
            ...
            if limit < 1:
                ...
                logging.warning(
                    ...
                    + " with parameters"
                    + str(query_args)
                    + "\nThe limit should be specified an specified integer above 0 "
                )
                return None

            for i in range(page):  # Skip the previous pages and sets the ExclusiveStartKey
                try:
...
class SingleTableDynamoCrudRepository(DynamoCrudRepository):
    ...
    def count(self,
        ...
        try:
            count = len(self.find_collection(
                key_condition_expression,
                index_name,
                limit=limit,
