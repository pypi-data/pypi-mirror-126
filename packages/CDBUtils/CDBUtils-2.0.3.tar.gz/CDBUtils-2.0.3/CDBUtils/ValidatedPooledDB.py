...
class RepeatingTimer(Timer):
    """
    重写 Timer run方法 以实现Timer线程常驻，循环执行
    """
    def run(self):
...
class PooledDB(pool_db.PooledDB):
    ...
    def __init__(
        ...
        self.idleCheckTime = 60 if int(idle_check_time) < 0 else int(idle_check_time)
        ...
        if self.testIdle and self.idleCheckTime > 0:
            t = RepeatingTimer(self.idleCheckTime, self.start_check_idle, [self.idleCheckTime])
            t.setDaemon(True)
            t.start()
