...
from typing import Dict, Any, cast, Optional, List, Tuple, BinaryIO
...
class Connection:
    ...
    def _post_attachments(self, path: str, files: Optional[List[Tuple[str, Tuple[Any, BinaryIO]]]] = None) -> None:
        if self._request_headers.get("Content-Type"):
            self._request_headers.pop("Content-Type")
        requests.post(url=self.api_base_url + path, headers=self._request_headers, files=files)
