...
class AttMapLike(MutableMapping, metaclass=abc.ABCMeta):
    ...
    def __iter__(self):
        return iter([k for k in list(self.__dict__.keys())])
    ...
    def add_entries(self, entries):
        ...
        try:
            entries_iter = list(entries.items())
        except AttributeError:
    ...
    def to_map(self):
        ...
        """
        return self._simplify_keyvalue(list(self.items()), self._new_empty_basic_map)
    ...
    def to_dict(self):
        ...
        """
        return self._simplify_keyvalue(list(self.items()), dict)
    ...
    def _data_for_repr(self):
        ...
        """
        return [kv for kv in list(self.items()) if not self._excl_from_repr(kv[0], self.__class__)]
