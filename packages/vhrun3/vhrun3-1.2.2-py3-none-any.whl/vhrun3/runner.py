...
class HttpRunner(object):
    ...
    def __run_step_request(self, step: TStep, protocol=None) -> StepData:
        ...
        try:
            if protocol is None:
                ...
                try:
                    if step.setup_hooks:
                        setup_variables = self.__call_hooks(step.setup_hooks, step.variables, "setup request")
            ...
            try:
                if step.teardown_hooks:
                    teardown_variables = self.__call_hooks(step.teardown_hooks, step.variables, "teardown request")
            ...
            try:
                ...
                if USE_ALLURE:
                    ...
                    teststep_allure_step(step_name, request_data, response_data, validate_list, step_data.export_vars)
                if failures_string:
    ...
    def run_testcase(self, testcase: TestCase) -> "HttpRunner":
        ...
        for step in self.__teststeps:
            ...
            loop_flag = step.loop_run.get("loop_flag")
            gap_time = step.loop_run.get("gap_time")
            timeout = step.loop_run.get("timeout")
            try:
                flag = os.environ[loop_flag]
            except Exception:
                flag = "false"
