...
class CLI:
    ...
    def _create_command(name, command, clazz, show_cmd=True, help=None,
        ...
        for a in aliases + [name]:
            if len(a) <= 3:
                CLI.bash_aliases[run_cmd.__qualname__] = a
...
def create_aliases(*args):
    ...
    if args:
        aux_sub_aliases = {}
        for cmd, alias in CLI.bash_aliases.items():
            cli, _, _ = cmd.lower().partition('.')
            if args[0] == 'all' or cli in args:
                aux_sub_aliases[cli] = aux_sub_aliases.get(cli, []) + [alias]
