...
try:
    from utils_lauenn import Symmetry,Lattice,\
        HklPlane,simulatemultiplepatterns, worker_generation, chunker_list,\
            rot_mat_to_euler, read_hdf5, get_ipf_colour,predict_ubmatrix, predict,\
                predict_preprocessMP, global_plots, save_sst, worker, texttstr
except:
...
if default_initialization:
    ...
    grains_nb_simulate_global = 2000
...
class Window(QMainWindow):
    ...
    def __init__(self, winx=None, winy=None):
        ...
        self.setDisplayText("Added ORIX dependecy for HKL calculation; Polefigure and IPF plot modules are taken and modified from PYMICRO repository")
    ...
    def postprocesstrain(self, emit_dict):
        ...
        ## space group 166 
        self.crystal = Phase(space_group=self.SG, structure=strucdiffpy(lattice=latdiffpy(a, b, c, alpha, beta, gamma)))
        ...
        if self.material_ != self.material1_:
            ...
            self.crystal1 = Phase(space_group=self.SG1, structure=strucdiffpy(lattice=latdiffpy(a, b, c, alpha, beta, gamma)))
    ...
    ## If better, migrate to Orix from PYMICRO
    def run_(self, n, rules, lattice_material, symmetry, material_, crystal=None, SG=None):  

        sym_R = []
        for sym in crystal.space_group.iter_symops():
            sym_R.append(sym.R)
            
        temp_ = np.round(Miller.from_highest_indices(phase=crystal, hkl=[int(n),int(n),int(n)]).hkl)
        temp_ = CP.ApplyExtinctionrules(temp_, rules)
        classhkl_ = temp_
        # mtemp = Miller(hkl=temp_, phase=crystal)
        # classhkl_ = np.unique(_round_indices(mtemp.unique(use_symmetry=True).hkl), axis=0)
        ...
        for i in range(len(classhkl_)):
            new_hkl = _round_indices(classhkl_[i,:])
            
            another_method = False
            m1 = Miller(hkl=new_hkl, phase=crystal)
            m11, mul = m1.symmetrise(unique=True, return_multiplicity=True)
            mul_family = _round_indices(m11.hkl)
            new_hkl = _round_indices(classhkl_[-1,:])
            for sym in mul_family:
                h,k,l = sym
                i0 = -(h+k)
                sum1 = np.sum(np.abs([h,k,i0,l]))
                h1,k1,l1 = new_hkl
                i1 = -(h1+k1)
                sum2 = np.sum(np.abs([h1,k1,i1,l1]))
                if sum1 != sum2:
                    another_method = True
                    break
            if another_method:
                family = []
                for sym in sym_R:
                    n_sym = np.dot(sym, new_hkl)
                    hkl_sym = HklPlane(*n_sym, lattice=lattice_material)
                    family.append(hkl_sym)
            else:
                family = []
                for sym in mul_family:
                    hkl_sym = HklPlane(*sym, lattice=lattice_material)
                    family.append(hkl_sym)
                    
            # if symmetry == Symmetry.trigonal or symmetry == Symmetry.hexagonal:
            #     ## Orix method (for 4 indices)
            #     m1 = Miller(hkl=new_hkl, phase=crystal)
            #     m11, mul = m1.symmetrise(unique=True, return_multiplicity=True)
            #     mul_family = _round_indices(m11.hkl)
            #     family = []
            ...
            #         family.append(hkl_sym)
            # else: 
            #     ## Another method of directly multiplying the matricies with HKL 
            #     ## Not for 4 indices
            #     family = []
            #     for sym in sym_R:
            #         n_sym = np.dot(sym, new_hkl)
            ...
            classhkl_[i] = new_hkl
            norm_h, norm_k, norm_l = classhkl_[i,:]
            h_obj = HklPlane(norm_h, norm_k, norm_l, lattice=lattice_material)
            normals = np.array([np.round(ijk.normal(),6) for ijk in family])
        ...
        if len(ind_rmv) != 0:
            classhkl_ = np.delete(classhkl_, ind_rmv, axis = 0)
        classhkl = classhkl_
        ...
        for i in range(len(classhkl)): ## progress
            new_hkl = _round_indices(classhkl[i,:])
            ...
            classhkl[i] = new_hkl
            norm_h, norm_k, norm_l = classhkl[i,:]
            h_obj = HklPlane(norm_h, norm_k, norm_l, lattice=lattice_material)
            normals = np.array([np.round(ijk.normal(),6) for ijk in family])
            # normals = np.unique(normals, axis=0)
    ...
    def load_dataset(self, material_="Cu", material1_="Cu", ang_maxx=18.,step=0.1, mode=0, 
        ...
        try:
            with open(self.save_directory+"//classhkl_data_"+material_+".pickle", "rb") as input_file:
                classhkl, _, _, n, _, \
                    hkl_all_class, hkl_all, lattice_material, symmetry = cPickle.load(input_file)
                    
            if material_ != material1_:
                with open(self.save_directory+"//classhkl_data_"+material1_+".pickle", "rb") as input_file:
                    classhkl1, _, _, n, _, \
                        hkl_all_class1, hkl_all1, lattice_material1, symmetry1 = cPickle.load(input_file)
        except:
        ...
        for j in classhkl:
            normal_hkl_ = np.vstack((normal_hkl_, hkl_all_class[str(j)]["normals"]))
        normal_hkl = np.delete(normal_hkl_, 0, axis =0)
        
        if material_ != material1_:
            normal_hkl1_ = np.zeros((1,3))
            for j in classhkl1:
                normal_hkl1_ = np.vstack((normal_hkl1_, hkl_all_class1[str(j)]["normals"]))
            normal_hkl1 = np.delete(normal_hkl1_, 0, axis =0)
        
        index_hkl = [j for j,k in enumerate(classhkl) for i in range(len(hkl_all_class[str(k)]["normals"]))]
        
        if material_ != material1_:
            ind_offset = index_hkl[-1] + 1
            index_hkl1 = [ind_offset+j for j,k in enumerate(classhkl1) for i in range(len(hkl_all_class1[str(k)]["normals"]))]
    ...
    def generate_training_data(self):
        ...
        # =============================================================================
        self.run_(self.n, self.rules, self.lattice_material, self.symmetry, self.material_, self.crystal, self.SG)
        if self.material_ != self.material1_:
            self.run_(self.n1, self.rules1, self.lattice_material1, self.symmetry1, self.material1_, self.crystal1, self.SG1)
...
class AnotherWindowLivePrediction(QWidget):#QWidget QScrollArea
    ...
    def predict_single_file(self,):
        ...
        match_rate12multiorimat, mat_global12multiorimat, cnt12multiorimat,\
            files_treated12multiorimat, spots_len12multiorimat, \
                iR_pix12multiorimat, fR_pix12multiorimat, check12multiorimat, best_match12multiorimat = predict_preprocessMP(filenameSingleExp, 0, 
        ...
        match_rate12beamtime, mat_global12beamtime, cnt12beamtime,\
            files_treated12beamtime, spots_len12beamtime, \
                iR_pix12beamtime, fR_pix12beamtime, check12beamtime, best_match12beamtime = predict_preprocessMP(filenameSingleExp, 0, 
                                                   rotation_matrix,strain_matrix,strain_matrixs,
                                                   col,colx,coly,match_rate,spots_len,iR_pix,fR_pix,best_match,
                                                   mat_global,
                                                   check,self.detectorparameters,self.pixelsize,angbins,
        ...
        match_rate12fast, mat_global12fast, cnt12fast,\
            files_treated12fast, spots_len12fast, \
                iR_pix12fast, fR_pix12fast, check12fast, best_match12fast = predict_preprocessMP(filenameSingleExp, 0, 
                                                   rotation_matrix,strain_matrix,strain_matrixs,
                                                   col,colx,coly,match_rate,spots_len,iR_pix,fR_pix,best_match,
                                                   mat_global,
                                                   check,self.detectorparameters,self.pixelsize,angbins,
        ...
        match_rate12, mat_global12, cnt12,\
            files_treated12, spots_len12, \
                iR_pix12, fR_pix12, check12, best_match12 = predict_preprocessMP(filenameSingleExp, 0, 
                                                   rotation_matrix,strain_matrix,strain_matrixs,
                                                   col,colx,coly,match_rate,spots_len,iR_pix,fR_pix,best_match,
                                                   mat_global,
                                                   check,self.detectorparameters,self.pixelsize,angbins,
    ...
    def plot_pc(self):
        ...
        if self.matrix_plot_tech.currentText() == "MultiProcessing":
            ...
            for i in range(self.ncpu):
                self._worker_processes[i]= Process(target=worker, args=(self._inputs_queue, self._outputs_queue, i+1, run_flag))
            for i in range(self.ncpu):
        ...
        for files in list_of_files:
            ...
            for intmat in range(matricies):
                ...
                if len(rotation_matrix1[intmat]) == 0:
                    ...
                    col_temp = get_ipf_colour(rotation_matrix1[intmat][0], np.array([0., 0., 1.]), final_symm)
                    # col[intmat][0][cnt,:] = col_temp
                    self.col[intmat][0][cnt,:] = col_temp
                    col_tempx = get_ipf_colour(rotation_matrix1[intmat][0], np.array([1., 0., 0.]), final_symm)
                    # colx[intmat][0][cnt,:] = col_tempx
                    self.colx[intmat][0][cnt,:] = col_tempx
                    col_tempy = get_ipf_colour(rotation_matrix1[intmat][0], np.array([0., 1., 0.]), final_symm)
