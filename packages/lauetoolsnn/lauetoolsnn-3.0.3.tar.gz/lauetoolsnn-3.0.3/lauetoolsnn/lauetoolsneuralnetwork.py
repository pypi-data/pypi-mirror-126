    ...
    from utils_lauenn import Symmetry,Lattice,\
        HklPlane,simulatemultiplepatterns, worker_generation, chunker_list,\
            rot_mat_to_euler, read_hdf5, get_ipf_colour,predict_ubmatrix, predict,\
                predict_preprocessMP, global_plots, save_sst, worker, texttstr, get_material_data,\
                    write_training_testing_dataMTEX
except:
...
class Window(QMainWindow):
    ...
    def run_(self, n, rules, lattice_material, symmetry, material_, crystal=None, SG=None):  
        ...
        for j in hkl_all.keys():
            normal_hkl = np.vstack((normal_hkl, hkl_all[j]["normals"]))
        normal_hkl = np.delete(normal_hkl, 0, axis =0)
        
        index_hkl = [j for j,k in enumerate(hkl_all.keys()) for i in range(len(hkl_all[k]["normals"]))]
        ...
        for j1, i1 in enumerate(hkl_all_class.keys()):
            hkl_object = hkl_all_class[i1]["HKL_object"]
            classhkl[j1,:] = hkl_object.miller_indices()
            family = hkl_all_class[i1]["family"]
            hkl_millerindices[i1] =  np.array([ii.miller_indices() for ii in family])
    ...
    def load_dataset(self, material_="Cu", material1_="Cu", ang_maxx=18.,step=0.1, mode=0, 
        ...
        for j in hkl_all_class.keys():
            normal_hkl_ = np.vstack((normal_hkl_, hkl_all_class[j]["normals"]))
        ...
        if material_ != material1_:
            ...
            for j in hkl_all_class1.keys():
                normal_hkl1_ = np.vstack((normal_hkl1_, hkl_all_class1[j]["normals"]))
        ...
        index_hkl = [j for j,k in enumerate(hkl_all_class.keys()) for i in range(len(hkl_all_class[k]["normals"]))]
        
        if material_ != material1_:
            ind_offset = index_hkl[-1] + 1
            index_hkl1 = [ind_offset+j for j,k in enumerate(hkl_all_class1.keys()) for i in range(len(hkl_all_class1[k]["normals"]))]
