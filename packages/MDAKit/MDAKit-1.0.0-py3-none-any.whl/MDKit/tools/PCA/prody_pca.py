...
from prody.apps.prody_apps.nmaoptions import *
from prody.apps.prody_apps import nmaoptions
...
def prody_pca(coords, **kwargs):
    ...
    if os.path.isdir(coords):
        DCDFP = coords
        dcdfs = [ os.path.join(DCDFP,_) for _ in os.listdir(DCDFP) ]
        coords= dcdfs[0]
    ...
    if outall or kwargs.get("eda"):
        pd.to_pickle(pca, os.path.join(outdir, kwargs.get("eda")))
...
Perform EDA for backbone atoms:
  ...
  $ prody eda mdm2.dcd --pdb mdm2.pdb --select backbone""",
    ...
    group.add_argument('--eda', default="eda.pickl",
        help="File name to save eda pickle. default='eda.pickl'")
...
def prody_main():
    prody_parser, prody_commands = prody_args()
    if len(sys.argv) == 1:
        prody_parser.print_help()
    else:
        addCommand(prody_commands)
