...
# Custom gauge (for gauge metrics if using multiprocess add param 'multiprocess_mode="livesum"')
...
def clear_tmp_folder():
    MULTIPROC_DIR = environ["PROMETHEUS_MULTIPROC_DIR"]
    if MULTIPROC_DIR:
...
def max_workers():    
    workers_count = cpu_count() - 1 if cpu_count() - 1 >= 1 else cpu_count()
