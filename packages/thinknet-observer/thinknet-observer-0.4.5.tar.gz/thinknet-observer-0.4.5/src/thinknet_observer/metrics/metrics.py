...
from .config import _DEFAULT_BUCKETS, _DEFAULT_LABELS, NAMESPACE, SERVICENAME


class PrometheusMiddleware(metaclass=SingletonMeta):

    # NOTE: percentile for summary -> https://www.mail-archive.com/prometheus-users@googlegroups.com/msg08275.html
    # TODO: check async for fastapi
    # DONE: add readme.md and ..
    # DONE: add test for bracket
    # DONE: check read env
    # os.environ['SERVICE_NAME_PREFIX']
    # DONE: add warn seperately -> Please provide SERVICE_NAME_PREFIX and SERVICE_NAME in .env file
    ...
    METRICS_REQUEST_LATENCY_SUMMARY = Summary(
        "http_request_duration_summary_ms",
    ...
    def __init__(
        ...
        :param serviceName: can be use to override serviceName that read from .env
    ...
    def _check_env(self, namespace, serviceName):
        if namespace == "UNDEFINED" or namespace is None:
            print(
                f"{Bcolors.WARNING}WARNING: env variable : 'namespace' is not found.\nPlease provide SERVICE_NAME_PREFIX in .env file{Bcolors.ENDC}"
            )
        if serviceName == "UNDEFINED" or serviceName is None:
            print(
                f"{Bcolors.WARNING}WARNING: env variable : 'serviceName' is not found.\nPlease provide SERVICE_NAME in .env file{Bcolors.ENDC}"
            )
    ...
    def add_fastapi_middleware(self):
        ...
        # NOTE: loop through all endpoint and get path_format
