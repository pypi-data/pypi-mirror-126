...
from .exceptions import GoogleAdsAccountLostAccessException, GoogleAdsAccountNoAccessException, GoogleAdsAccountException
from .helpers import (
...
def get_google_ads_client(
    ...
    if access_token:
        ...
        if creator_email:
            login_customer_id, developer_token = get_login_customer_id_and_developer_token(mcc_credentials_path)
            ...
            return _get_user_initialized_client(
...
def check_access_account(
    ...
    account_id = remove_dash(account_id)
    query = f"""
        SELECT
...
def _check_access_account_lgq(google_ads_account_id: str, google_ads_client: GoogleAdsClient):
    ...
    query = f"""
        SELECT
          customer.manager
        FROM customer
        WHERE customer.id = '{google_ads_account_id}'"""
    search_query = google_ads_client.get_type(
        "SearchGoogleAdsRequest"
    )
    search_query.customer_id = google_ads_account_id
    search_query.query = query
    try:
        response = list(google_ads_service.search(search_query))
        if len(response) == 0:
