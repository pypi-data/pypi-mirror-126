...
from gwpopulation.cupy_utils import to_numpy, trapz, xp
...
def create_parser():
    ...
    spectra = ["mass", "orientation", "magnitude", "redshift"]
    for key in spectra:
        parser.add_argument(
            f"--{key}",
            dest=key,
...
def corner_plots(result, *args, **kwargs):
    ...
    if len(result) > 1:
        for kind, pars in zip(
            ["mass", "spin_magnitude", "spin_orientation", "redshift"],
            [
                mass_parameters,
                spin_magnitude_parameters,
...
def mass_spectrum_plot(results, limits, rate=False, observed=False, save=True):
    """
    ...
    limits: tuple
        The lower and upper percentiles to show in the uncertainty region.
    rate: bool
    ...
    for result in results:
        ...
        if os.path.isfile(filename):
            ...
            dd.io.save(filename, data=dict(lines=lines, ppd=ppd, injected=injected))
...
def spin_orientation_spectrum_plot(results, limits, rate=False, save=True):
    """
    ...
    for result in results:
        ...
        if os.path.isfile(filename):
            ...
            for ii in tqdm(range(len(samples))):
                ...
                cos_tilt_1_prob = trapz(prob, cos_tilts, axis=0)
                cos_tilt_2_prob = trapz(prob, cos_tilts, axis=1)
...
def redshift_spectrum_plot(results, limits, rate=True, save=True):
    """
    ...
    for result in results:
        if "lamb" in result.posterior:
            model = PowerLawRedshift(z_max=2.3)

            def _get_parameters(parameters):
                return dict(lamb=parameters["lamb"])

        elif "gamma" in result.posterior:
            model = MadauDickinsonRedshift(z_max=limits[1])

            def _get_parameters(parameters):
                return {key: parameters[key] for key in ["gamma", "kappa", "z_peak"]}

        else:
            raise KeyError("Cannot find redshift parameters.")

        redshifts = model.zs
        _np_redshifts = model.zs_
        ...
        if os.path.exists(filename):
            ...
            for ii in tqdm(range(len(samples))):
                parameters = dict(samples.iloc[ii])
                prob = model.psi_of_z(redshift=redshifts, **_get_parameters(parameters))
                if rate:
...
def _safe_plot(result, limits, key):
    _set_matplotlib()
    try:
        PLOT_MAP[key](result, limits, rate=True)
...
def main():
    ...
    if args.labels is not None:
        for label, result in zip(args.labels, results):
            result.label = label
            result.outdir = args.run_dir
    for key in ["mass", "magnitude", "orientation", "redshift", "corner"]:
        if getattr(args, key):
            logger.info(f"Making {key} plot")
            _safe_plot(results, [args.lower_limit, args.upper_limit], key)
    if args.samples is not None and os.path.exists(args.samples):
        samples = dd.io.load(args.samples)
