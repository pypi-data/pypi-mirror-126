...
class RsAreg:
	"""376 total commands, 17 Sub-groups, 0 group commands"""
	_driver_options = "SupportedInstrModels = AREG, SupportedIdnPatterns = AREG, SimulationIdnString = 'Rohde&Schwarz,AREG100A,100001,4.80.70.0015'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsAreg._driver_options, options, direct_session)
		self._core.driver_version = '4.80.70.0015'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsAreg':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsAreg session '{self._core.io.resource_name}'"
		else:
			return f"RsAreg with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.80.70.0015'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsAreg version failed. Current version: '4.80.70.0015', minimum required version: '{min_version}'")
	...
	@property
	def source(self):
		"""source commands group. 8 Sub-classes, 1 commands."""
	...
	@property
	def system(self):
		"""system commands group. 33 Sub-classes, 24 commands."""
	...
	@property
	def test(self):
		"""test commands group. 6 Sub-classes, 4 commands."""
		if not hasattr(self, '_test'):
			from .Implementations.Test import Test
			self._test = Test(self._core, self._cmd_group)
		return self._test

	def clone(self) -> 'RsAreg':
		"""Creates a deep copy of the RsAreg object. Also copies:
		...
		cloned = RsAreg.from_existing_session(self.get_session_handle(), self._options)
