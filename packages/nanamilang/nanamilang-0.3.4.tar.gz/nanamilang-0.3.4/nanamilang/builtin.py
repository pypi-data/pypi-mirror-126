...
class BuiltinMacro:
    ...
    def fn_macro(tree_slice: list, ev_func, token_cls) -> list:
        ...
        if len(tree_slice) == 3:
            ...
            raise MacroEvaluationError('fn: invalid function definition form length, see docs')

        ASSERT_IS_INSTANCE_OF(fn_arguments_form, list, m='fn: function arguments form invalid')
    ...
    def let_macro(tree_slice: list, ev_func, token_cls) -> list:
        ...
        for [key_token, value_token_or_form] in partitioned:
            ...
            if isinstance(key_token, token_cls) and \
                ...
                raise MacroEvaluationError(f'let: can\'t assign to: {key_token}')
...
class Builtin:
    ...
    def install(fn_meta: dict, fn_callback) -> bool:
        ...
        ref_key = fn_meta.get('_ref_key_', f'{randstr()}_func')
        maybe_existing = getattr(Builtin, ref_key, None)
        if maybe_existing:
            delattr(Builtin, ref_key)

        setattr(Builtin, ref_key, fn_callback)
        function_attribute = getattr(Builtin, ref_key, None)
