...
class Fn:
    ...
    def _form_for_complex(self, dt_instance: datatypes.Base) -> list:
        ...
        return recursively(dt_instance, dt_instance.reference())

    def handle(self, args: list) -> datatypes.Base:
        """NanamiLang Fn Handler, handle function evaluation"""

        assert len(args) == len(self._fn_arg_names), (
            f'Wrong arguments passed number: {len(args)}, expected: {len(self._fn_arg_names)}'
        )
