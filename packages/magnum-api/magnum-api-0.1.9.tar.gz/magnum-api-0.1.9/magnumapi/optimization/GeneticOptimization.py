...
class ParallelRoxieGeneticOptimization(RoxieGeneticOptimization):
    ...
    def optimize(self):
        ...
        self.pool = LocalMapdlPool(self.config.ansys.n_parallel_runners,
                                   nproc=self.config.ansys.n_proc,
                                   run_location=self.config.ansys.run_dir)
        are_uploaded = [False] * self.n_pop
        ...
        for gen in range(self.n_gen):
            ...
            for index, individual in enumerate(self.pop):
                ...
                try:
                    self.update_model(individual)
                    fom_dct = self.calculate_figures_of_merit(individual, index)
                except AttributeError:
                    fom_dct = {objective_config.objective: float('nan') for objective_config in self.config.objectives}
            ...
            self.remove_ansys_output_files()

            indices = self.extract_calculated_indices(fom_dcts)
            print('indices to process: ', indices)
            root_dir = os.path.join(self.output_subdirectory_dir, self.config.ansys.root_dir)
            execute_ansys_in_parallel(self.pool, root_dir, indices, are_uploaded)

            # read figures of merit
            ansys_fom_dcts = self.read_ansys_figures_of_merit(root_dir)
            ...
            # log current computation results
            for fom_dct, score in zip(fom_dcts, scores):
                self.logger.append_to_logger(self.decode_individual(individual), fom_dct, score)
    ...
    def calculate_scores(self, fom_dcts):
        scores = []
        for fom_dct in fom_dcts:
            if all([not np.isnan(value) for value in fom_dct.values()]):
                score = self.calculate_score(fom_dct)
            else:
                score = PENALTY
    ...
    def remove_ansys_output_files(self):
        root_dir = os.path.join(self.output_subdirectory_dir, self.config.ansys.root_dir)
        for i in range(self.n_pop):
    ...
    def read_ansys_figures_of_merit(self, root_dir) -> list:
        root_dir = os.path.join(self.output_subdirectory_dir, self.config.ansys.root_dir)
        fom_dcts = []
    ...
    def calculate_figures_of_merit(self, individual: List[int], index: int) -> dict:
        """Method calculating figures of merit with scripts or notebooks (`papermill` and scrapbook packages).
...
def execute_ansys_in_parallel(pool, root_dir, indices, are_uploaded=[]):

    def mapping_function(mapdl, root_dir, index):
        mapdl.clear()
        # shared files are uploaded only once
        input_file = '15T_mech_%d.inp' % index
        if not are_uploaded[index]:
            are_uploaded[index] = True
            single_upload_files = [input_file,
            ...
            for single_upload_file in single_upload_files:
                mapdl.upload(os.path.join(root_dir, single_upload_file), progress_bar=False)

        # per-model files are uploaded always
        reupload_files = ['Model_%d.inp' % index,
                          'forces_edit_%d.vallone' % index]

        for reupload_file in reupload_files:
            mapdl.upload(os.path.join(root_dir, reupload_file), progress_bar=False)
    ...
    output = pool.map(mapping_function, inputs, progress_bar=True, wait=True)
