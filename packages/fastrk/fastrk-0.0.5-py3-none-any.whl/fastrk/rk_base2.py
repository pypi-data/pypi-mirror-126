...
def event_detector(call_event, t, s, evvals, it, counters, evargs, evout, n_evout):
    """
    ...
    :return:
    """
    values, terminals, directions, counts = evargs
...
def prop_event(t, args):
    ...
    :return:
    """
    call_event, values, idx, \
    fode, s0, t0, max_step, rtol, atol, *fargs = args

    trj = rk_prop(fode, s0, t0, t, max_step, rtol, atol, *fargs)
    return call_event(trj[-1, 0], trj[-1, 1:], values, idx)


def calc_root_brentq(xa, xb, xtol, rtol, maxiter, args):
    """
    ...
    """
    #_fode, _s0, _max_step, _rtol, _atol = rkargs
    #call_event, values, idx, _fode, _s0, _t0, _max_step, _rtol, _atol, *fargs = args

    xpre, xcur = xa, xb
    xblk, fblk = 0., 0.
    spre, scur = 0., 0.

    fpre = prop_event(xpre, args)
    fcur = prop_event(xcur, args)
    if fpre * fcur > 0:
