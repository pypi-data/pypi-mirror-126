...
SPLITTED_DIR = join(OUTPUT_DIR, 'splitted')
...
def make_mono_files():
    for name in glob.glob(join(SPLITTED_DIR,'**','*.wav'), recursive=True):
...
def detach_AUDIO_VIDEO(file):
    ...
    detach_rep = join(SPLITTED_DIR, file_basename)
...
def build_yaml_files():
    wave_files = list(glob.glob(join(SPLITTED_DIR, '**', '*.wav'), recursive=True))
...
def collect_yaml_files():
  yaml_files = []
  for name in glob.glob(join(SPLITTED_DIR, '**', '*.yml'), recursive=True):
      # print(name)
...
def mkdirs():
    pathlib.Path(SPLITTED_DIR).mkdir(parents=True, exist_ok=True)
    pathlib.Path(SYNCED_DIR).mkdir(parents=True, exist_ok=True)
