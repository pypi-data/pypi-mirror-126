...
class Ruler(Msd3663Common):
    ...
    def get_ocs_require(self):
        ...
        # 制式区分
        if any(ct in self.get_ocs_country() for ct in ['PANAMA', 'JAMAICA']):
            ret += '// special request' + '\n'
            ret += '#undef CVT_DEF_IR_TYPE'
            ret += self.get_macro_line('CVT_DEF_JPE_BRAND_CONFIG', 'CONFIG_JPE_BRAND_PANAMA_COMMON')
        elif any(ct in self.get_ocs_country() for ct in ['COLOMBIA']):
            ret += '// special request' + '\n'
            ret += self.get_macro_line('CVT_DEF_JPE_BRAND_CONFIG', 'CONFIG_JPE_BRAND_COLUMBIA_COMMON')
        else:
            ret += '// language & region' + '\n'
            ret += self.get_macro_line('CVT_DEF_JPE_LANGUAGE_GROUP', 'CONFIG_JPE_LANG_GRP_COMMON')
            ret += self.get_macro_line('CVT_DEF_LANGUAGE_SELECT', 'LANGUAGE_ENGLISH')
    ...
    def get_ocs_country(self):
        ...
        if region_name_str != '':
            ...
            # 国家补丁
            if country == None:
                if region_name_str == '阿拉伯联合酋长国' or region_name_str == '迪拜':
