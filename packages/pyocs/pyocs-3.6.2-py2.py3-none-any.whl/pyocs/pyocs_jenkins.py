...
class PyocsJenkins:
    ...
    def get_param_from_interaction(self, model_id, build_cmd_param_dict):
        ...
        print("=======================================")
        return compile_param_dict, project_name, project, branch

    """
    #传入model_id ,创建jenkins编译任务
    """
    def create_jenkins_job(self, model_id, build_cmd_param_dict):
        compile_param, job_name, project, branch = self.get_param_from_interaction(model_id,build_cmd_param_dict)
        self._create_jenkins_job(compile_param=compile_param, job=job_name, project_name=project, branch_name=branch)
    ...
    def _create_jenkins_job(self, compile_param, job, project_name, branch_name):
        '''
        使用api的方式触发编译
        ...
        '''
        account = pyocs_login.PyocsLogin().get_account_from_json_file()
        master = self.get_jenkins_master(project_name, branch_name)
