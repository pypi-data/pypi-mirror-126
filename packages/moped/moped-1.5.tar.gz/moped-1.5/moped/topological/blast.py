...
def _generate_blast_database(genome_file: pathlib.Path) -> None:
...
def _unify_inputs(
    sequences: str | pathlib.Path | Iterable[str], tempfiles: list[pathlib.Path]
) -> pathlib.Path:
...
def _split_input_files(
    file_path: pathlib.Path, n_cores: int, temporary_files: list[pathlib.Path]
) -> list[pathlib.Path]:
...
def _run_blast_process(query_file_path: str | pathlib.Path, database_name: str) -> str:
...
def blast(
    sequences: str | pathlib.Path | Iterable[str],
    ...
    """
    temporary_files: list[pathlib.Path] = []
    genome_fp = pathlib.Path(genome_file)

    _generate_blast_database(genome_file=genome_file)
    file_path = _unify_inputs(sequences, temporary_files)
    partial_blast = functools.partial(_run_blast_process, **{"database_name": genome_fp.stem})
