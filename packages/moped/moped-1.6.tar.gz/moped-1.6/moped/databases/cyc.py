...
from typing import Any, Callable, DefaultDict, Dict, Iterable, List, Set, Tuple
...
class Cyc:
    ...
    def parse(self) -> Tuple[list[Compound], list[Reaction], Dict[str, str], Dict[str, MonomerLink]]:
        """Parse the database."""
        path = self.path
        ...
        monomer_links = {
            product: MonomerLink(gene=i.id, database_links=i.database_links)
            for i in genes.values()
            if (product := i.product) is not None
        }
        return compounds, reactions, compartments, monomer_links
...
class GeneParser:
    ...
    def parse(self) -> Dict[str, ParseGene]:
