...
class WebSiteScarperWorker(WorkerAbstract):
    ...
    def tree_branch_to_csv(self, branch, html_table_idx,  pre_defined_columns):
        ...
        for key, val in pre_defined_columns.items():
            ...
            pre_def_columns_completed[key] = branch[int(regexp_res.group(1))].text if type(branch[int(regexp_res.group(1))]) != str else branch[int(regexp_res.group(1))]
...
def test():
    ...
    ws_scraper.work({'params': {'worker_driver': 'WebSitesScrapingWorker', 'title': 'Service Bus'}, 'payload': {'scrap_flow': 'scraper_flows/azure_single_action_doc_scraper.yml', 'url_to_scrap': 'https://docs.microsoft.com/en-us/rest/api/servicebus/enumeration'}})
