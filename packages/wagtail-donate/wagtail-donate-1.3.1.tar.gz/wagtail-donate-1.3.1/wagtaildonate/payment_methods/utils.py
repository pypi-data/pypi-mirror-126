...
def instantiate_payment_method(setting, klass=None) -> PaymentMethod:
    if klass is None:
        klass = import_string(setting["class"])
    return klass(setting.get("options", {}))


def get_payment_method(user_payment_method) -> PaymentMethod:
    for payment_method in donate_settings.PAYMENT_METHODS:
        ...
        if klass.code == user_payment_method:
            return instantiate_payment_method(payment_method, klass=klass)
...
def get_all_payment_methods(
    *, allowed_frequencies=None
    ...
    for payment_method in donate_settings.PAYMENT_METHODS:
        payment_method_object = instantiate_payment_method(payment_method)
