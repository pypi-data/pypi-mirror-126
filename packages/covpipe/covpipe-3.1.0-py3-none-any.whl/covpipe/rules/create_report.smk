singularity: "docker://rkibioinf/rscript:0.1--e9d0509"
...
rule createReport:
    input:
        ...
        qc = expand(os.path.join(DATAFOLDER["qc"], "{sample}", "{sample}.qc.tsv"), sample = SAMPLES.keys()),
    ...
    shell:
        ...
        r"""
            ...
            Rscript -e "rmarkdown::render('{params.template}',
                                            params=list(proj_folder='{params.p_folder}', list_folder='{params.l_folder}', run_name='{params.run_id}', tax_id='{params.tax_id}', min_cov='{params.min_cov}', voi_file='{params.voi}', version='$VERSION'),
