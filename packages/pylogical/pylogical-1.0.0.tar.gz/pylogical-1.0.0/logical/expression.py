...
class LogicExpression(object):
    ...
    expression_regex = r"[1-9][0-9]*?[A-Z]?"
    expression_sub_regex = r"[1-9][0-9]*?[A-Z]?"
    ...
    def generate(self, content):
        ...
        self.check(content)
        expression = re.sub(self.expression_regex, self.sub_func, content)
        return expression

    def sub_func(self, matched):
        letter = next(self.expression_letters)
        self.expression_dict[letter] = matched.group()
        return letter
    ...
    def middle_to_after(self, s):
        ...
        expression = []
        ops = []
        ss = [c.strip() for c in s if c.strip()]
        for item in ss:
    ...
    def calculate(self, n1, n2, op, context):
        if op == '|':
            if context.get(self.expression_dict[n1]):
                return True

            if context.get(self.expression_dict[n2]):
                return True
            else:
                return False

        if op == '&':
            if not context.get(self.expression_dict[n1]):
                return False

            if not context.get(self.expression_dict[n2]):
                return False
            else:
                return True

    def calculate_not(self, n, context):
        if context.get(self.expression_dict[n]):
            return False
        else:
            return True
