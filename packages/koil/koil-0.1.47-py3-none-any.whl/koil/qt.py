...
class FutureWrapper(QObject):
    _call = Signal(Reference, tuple, dict)
    _resolve = Signal(Reference, object)
    _reject = Signal(Reference, Exception)
    _cancel = Signal(Reference, tuple, dict)
    _cancelled = Signal(Reference)

    def connect(self, receiver, cancel_receiver=None):
        self.call_connected = True
        self._call.connect(receiver)

        if cancel_receiver:
            self.cancel_connected = True
            self._cancel.connect(cancel_receiver)

    def resolve(self, reference, value):
        self._resolve.emit(reference, value)

    def reject(self, reference, value: Exception):
        self._reject.emit(reference, value)

    def cancelled(self, reference):
        self._cancelled.emit(reference)

    def __init__(self, *args, cancel_timeout=4, pass_through=False, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._resolve.connect(self.on_resolve)
        self._reject.connect(self.on_reject)
        self._cancelled.connect(self.on_cancelled)
        self.futureMap = {}
        self.cancelMap = {}
    ...
    async def acall(self, *args, **kwargs):
        ...
        self._call.emit(reference, args, kwargs)
        ...
        try:
            ...
            self._cancel.emit(reference, args, kwargs)
            try:
