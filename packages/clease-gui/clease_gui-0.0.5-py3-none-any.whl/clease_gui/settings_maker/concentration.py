...
from clease_gui import BaseDashboard, update_statusbar, utils
...
class ConcentrationDashboard(BaseDashboard):
    def initialize(self):
        ...
        self.out_basis = widgets.Output(layout=widgets.Layout(
            border='solid 1px'))

        # Make buttons to add, remove and clear basis
        self.add_basis_b = utils.make_clickable_button(self._on_add_basis,
                                                       description='Add basis')
    ...
    def _on_add_basis(self, b):
        ...
        new_basis = [
            val.strip() for val in value.strip().split(DELIM) if val.strip()
        ]
