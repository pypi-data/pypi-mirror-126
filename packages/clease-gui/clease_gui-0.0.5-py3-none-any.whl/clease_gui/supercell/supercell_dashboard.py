...
import clease
from clease_gui import BaseDashboard, register_logger, update_statusbar, utils, bool2symbol
...
class SupercellDashboard(BaseDashboard):
    ...
    def set_supercell_species(self, species_per_basis: Sequence[Dict[str,
                                                                     int]]):
    ...
    def _make_species_output(self):
        ...
        with out:
            ...
            for ii, species_count in enumerate(species_per_basis):
                ...
                output, callback = make_sublattice_box(species_count,
                                                       sublattice_index)
...
def make_sublattice_box(
    ...
    tot_box = widgets.IntText(description='Total in basis:',
                              value=tot_in_basis,
                              disabled=True,
                              style={'description_width': 'initial'})
    # Box with the total specified number in this basis
    tot_specified = widgets.IntText(description='Total specified:',
                                    value=0,
                                    disabled=True,
                                    style={'description_width': 'initial'})
    ...
    with sublattice_out:
        for species, count in species_count.items():
            wdgt, ratio_wdgt = _make_species_boxes(species, count,
                                                   tot_in_basis)
    ...
    def get_counts() -> Dict[str, int]:
        ...
        if cur_count != tot_in_basis:
            raise ValueError(
                ('Incorrect number of elements in sublattice {}.'
                 ' Expected {}, but got {}.').format(sublattice_index,
                                                     tot_in_basis, cur_count))
        return counts
...
def _make_species_boxes(
        species: str, count: int,
        tot_in_basis: int) -> Tuple[widgets.BoundedIntText, widgets.FloatText]:
