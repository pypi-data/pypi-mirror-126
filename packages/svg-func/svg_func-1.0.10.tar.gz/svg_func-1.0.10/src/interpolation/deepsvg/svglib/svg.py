...
class SVG:
    ...
    @staticmethod
    def load_svg(file_path,rb=False):
        if rb == True:
            return SVG.from_str(file_path)
        else:
    ...
    def save_svg(self, file_path,with_viewbox,center=None):
        with open(file_path, "w") as f:

            if center is not None:
                # print('center&&&&',center)
                f.write(self.normalize2(max(with_viewbox),center).canonicalize().simplify_heuristic().to_str(fill="#000",with_viewbox="0 0 100 100"))
            else:
                f.write(self.canonicalize().simplify_heuristic().to_str(with_viewbox=with_viewbox))
    ...
    def draw(self, fill=False, file_path=None, do_display=True, return_png=False,
             ...
             with_moves=True, with_viewbox=None):
        ...
        svg_str = self.to_str(fill=fill, with_points=with_points, with_handles=with_handles, with_bboxes=with_bboxes,
                              with_markers=with_markers, color_firstlast=color_firstlast, with_moves=with_moves,with_viewbox=with_viewbox)
    ...
    def zoom2(self, factor, center: Point = None):
        ...
        self.translate(-center)
        self._apply_to_paths("scale", factor)
        self.translate(Point(50,50))

        return self
    def normalize2(self, viewbox = None,center=None):

        scale_factor = float(60 / viewbox)
        self.zoom2(scale_factor, center)
        self.viewbox = Bbox(100)

        return self

    def normalize(self, viewbox: Bbox = None,center=None):
        if viewbox is None:
            viewbox = Bbox(24)
