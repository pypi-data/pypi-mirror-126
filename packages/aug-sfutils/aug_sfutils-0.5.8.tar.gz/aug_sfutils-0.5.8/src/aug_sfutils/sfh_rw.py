...
def duplicate_sfh(fin, fout):
 
    sfo = sfread.SFREAD(sfh=fin)
    sfw = sfh2byt(sfo)
    write_sfh(sfw, fout=fout)
...
# SIGNALS list generated automatically, override input entry
    ...
    for key, val in sfo.sfh.items():
        if val.obj_type in (6, 7, 8, 13): # Include areabases too
            sigs.items += 1
    ...
    sfw_d = {}
    for sfobj in sfo.sfh.values(): # order is consistently preserved, because of [sfobj.objid]
...
class SFH_READ:
    ...
    def __init__(self, byte_str):
        ...
        val_func = { \
            ...
            15: self.modobj, 16: self.mapext, 17: self.resource, 18: self.addrlen }
...
class SFH_WRITE:
    ...
    def __init__(self, sfo):
        ...
        if sfo.obj_type in val_func.keys():
            logger.debug(sfo.objid, sfo.objnam, sfo.obj_type)
    ...
    def sig(self):
        ...
        self.val[ 8: 24] = pack('>4H', *self.sfo.index)
