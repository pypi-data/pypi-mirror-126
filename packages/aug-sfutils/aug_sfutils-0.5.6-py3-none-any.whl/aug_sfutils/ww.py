...
class WW:
    ...
    def SetSignal(self, signame, data, indices=None):
        """Stores 'data'-array of a given Signal named 'signame'
    ...
    def SetAreabase(self, areaname, data):
        ...
        if hasattr(self, '_diaref'):
            ...
            if data.ndim > 1:
                ...
                nt = 1
            logger.debug('AB %s, %d, %d, %d, %d, %d', areaname, sf_dtype, nt, sizes[0], sizes[1], sizes[2])
            error  = ct.c_int32(0)
            ...
            if getError(error):
                ...
                logger.info('Written Areabase %s, sizes %d %d %d', areaname, sizes[0], sizes[1], sizes[2])
