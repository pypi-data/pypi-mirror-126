...
def lista_notas_emitidas():
    ...
    if query is None:
        slack.registerWarn("Query de notas fiscais emitidas nao configurada!")
    else:
        try:
...
def lista_rastreio_pedidos():
    ...
    if query is None:
        slack.registerWarn("Query rastreio de pedidos nao configurada!")
    else:
        try:
            conexao = Conexao(globals.database_type)
            conn = conexao.get_conect()
...
def lista_entrega_pedidos():
    ...
    if query is None:
        slack.registerWarn("Query entrega de pedidos nao configurada!")
    else:
        try:
            conexao = Conexao(globals.database_type)
            conn = conexao.get_conect()
