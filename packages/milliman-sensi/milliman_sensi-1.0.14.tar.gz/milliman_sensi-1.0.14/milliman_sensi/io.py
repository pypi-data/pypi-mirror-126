...
def create_one_sensi_from_base(sensi_name, base_dir, sensi_path=None, logger=None):
    if sensi_path is not None:
        ...
        try:
            ...
            excluded = shutil.ignore_patterns("sensitivities", "executed", "settings_calibration.json", "settings_simulation.json", "*_outputs", "lib", "src", "*.zip", "*.log")
            ...
            if os.path.exists(base_rsrc_dir):
                shutil.copytree(base_rsrc_dir, sensi_rsrc_dir, ignore=excluded)

            if os.path.exists(sensi_rsrc_admin_dir):
                shutil.rmtree(sensi_rsrc_admin_dir)

            if os.path.exists(base_rsrc_admin_dir):
                shutil.copytree(base_rsrc_admin_dir, sensi_rsrc_admin_dir, ignore=excluded)
