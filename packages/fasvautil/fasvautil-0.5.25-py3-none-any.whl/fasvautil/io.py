...
from fasvautil.logging import error_log
...
def format_datetime(timestamp):
    ...
    if isinstance(timestamp, datetime):
        return timestamp.strftime(DATETIME_FORMAT).replace(':', '_')
    elif isinstance(timestamp, str):
        return datetime.strptime(timestamp.replace('_', ':'), DATETIME_FORMAT)
...
def rename_images(directory, destination = None, interval=None):
    ...
    @error_log
    def _copyfile(source, target, idx, num_files, queue, rename=False):
        ...
        with ThreadPoolExecutor() as pool:
            for idx, file in enumerate(sorted(files)):
                ...
                if last_time is None or interval <= (timestamp - last_time):
                    ...
                    if destination is not None:
                        source, target = os.path.join(directory, file), os.path.join(
                            destination, "{}_{}_{}".format(timestamp.isoformat(), serial, ext))

                    else:
                        # do not copy but rename
                        source, target = os.path.join(directory, file), os.path.join(directory, "{}_{}_{}".format(
                            timestamp.isoformat(), serial, ext))

                    # copy the renamed file
                    pool.submit(_copyfile, source, target, idx, num_files, rename=destination is None,
                                queue=processed_queue)

            processed_queue.put(None)

    t = Thread(target=feed_images)
    t.start()
    try:
        while True:
            img_idx, total_image = processed_queue.get()

            yield img_idx, total_image
    except TypeError:
        # arises if finished
        pass
    finally:
