...
class HomographyTransform(KeypointCorrelator):
  ...
  def computeTform(self):
    ...
    try:
      ...
      matches = matcher.match(*[d.astype('uint8') for d in descrs])
...
class SIFTKeypoints(KeypointFinder):
  hasDescriptors = True
  ...
  def __init__(self, numFeatures=5000):
    ...
    self.numFeatures = numFeatures
    self.sift = cv.SIFT_create(numFeatures)

  @needsGrayscale
  def computeFeatures(self, img: np.ndarray):
    if img.ndim > 2:
      img = img.mean(2).astype(img.dtype)
    keypoints, descrs = self.sift.detectAndCompute(img, None)
    return np.vstack([kp.pt for kp in keypoints]), descrs