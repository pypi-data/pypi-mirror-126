...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return jsKdF if e else jsKdT
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->jsKdS:
...
@click.option("--revert",is_flag=jsKdF,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:jsKdm):
...
def _cpvcs_initialized(pod_name:jsKdq)->jsKdm:
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_init(name:jsKdq):
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=jsKdF,default=jsKdT)
def cmd_cpvcs_delete(name:jsKdq,remote:jsKdm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_register_remote(name:jsKdq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=jsKdF)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=jsKdF)
def cmd_cpvcs_rename(name:jsKdq,new_name:jsKdq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_commit(message:jsKdq,name:jsKdq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=jsKdF,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_push(squash:jsKdm,message:jsKdq,name:jsKdq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=jsKdc,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=jsKdF,default=jsKdT,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_inject(version:jsKdc,reset:jsKdm,name:jsKdq):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=jsKdF,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=jsKdF,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:jsKdq,inject:jsKdm,reset:jsKdm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=jsKdF,default=jsKdT)
def cmd_cpvcs_list_pods(remote:jsKdm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_versions(name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=jsKdF,type=jsKdc)
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_version_info(version:jsKdc,name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=jsKdc,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_version_metamodel(version:jsKdc,name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=jsKdF,type=jsKdc,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=jsKdF,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=jsKdF,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=jsKdT,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_set_version(version:jsKdc,inject:jsKdm,reset:jsKdm,commit_before:jsKdm,name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_commits(version:jsKdc,name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=jsKdF)
@click.option("--commit","-c",required=jsKdF)
@click.option("-n","--name",help="Name of the cloud pod",required=jsKdF)
def cmd_cpvcs_commit_diff(version:jsKdc,commit:jsKdc,name:jsKdq):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(jsKdS)
  status.stop()
...
def print_table(columns:Dict[jsKdq,jsKdq],rows:List[Dict[jsKdq,Any]],formatters:Dict[jsKdq,Callable[[Any],jsKdq]]=jsKdS):
 from rich.table import Table
 if formatters is jsKdS:
  formatters=jsKdb()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  jsKdE(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->jsKdS:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->jsKdS:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->jsKdS:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->jsKdS:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
