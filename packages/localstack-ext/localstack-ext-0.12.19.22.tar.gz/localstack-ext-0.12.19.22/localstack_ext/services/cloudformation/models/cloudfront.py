...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or GAuWy
 @GAuWN
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return GAuWK(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @GAuWF
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if GAuWj(origins,GAuWh)else origins.get("Items")
  origins_to_create=GAuWa([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if GAuWj(aliases,GAuWh)else aliases.get("Items")
  aliases_to_create=GAuWa(aliases)
  for distr in distrs:
   origins=GAuWa([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=GAuWa(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",GAuWD)
   config["Enabled"]=GAuWr(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if GAuWj(custom_errors,GAuWp)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=GAuWr(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":GAuWD}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=GAuWr(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=GAuWy):
    parent=config if parent is GAuWy else parent
    items=parent.get(key)or[]
    if GAuWj(items,GAuWp)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":GAuWP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if GAuWj(cache_behavs,GAuWp)else cache_behavs)
