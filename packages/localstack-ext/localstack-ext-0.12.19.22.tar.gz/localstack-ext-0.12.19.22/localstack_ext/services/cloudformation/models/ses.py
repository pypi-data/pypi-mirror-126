...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[nwMjc])[0]
 def get_physical_resource_id(self,attribute=nwMjc,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if nwMjy(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return nwMjk(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @nwMjq
 def add_defaults(resource,stack_name:nwMjN):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return nwMjk(SESReceiptRule,self).get_cfn_attribute(attribute)
 @nwMjH
