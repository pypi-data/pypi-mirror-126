...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=RvKuA(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not RvKuW
   return result
  except RvKua:
   props=self.props
   ...
   if parts[0]in candidates and RvKuy(parts)>1:
