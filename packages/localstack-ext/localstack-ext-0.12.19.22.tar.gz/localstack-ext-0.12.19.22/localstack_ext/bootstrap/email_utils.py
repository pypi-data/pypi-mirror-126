...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rXoil()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rXoiO(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rXoid,from_name=rXoid,smtp_host=rXoid,smtp_user=rXoid,smtp_pass=rXoid,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rXoiy(recipients,rXoiE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rXoid):
 ...
 result["From"]=formataddr((rXoiO(Header(from_name,"utf-8")),from_email))
 return result
