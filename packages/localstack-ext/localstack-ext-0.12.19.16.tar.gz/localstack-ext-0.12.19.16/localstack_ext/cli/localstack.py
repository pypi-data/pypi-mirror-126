...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return nLmOD if e else nLmOK
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->nLmOw:
...
@click.option("--revert",is_flag=nLmOD,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:nLmOB):
...
def _cpvcs_initialized(pod_name:nLmON)->nLmOB:
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_init(name:nLmON):
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=nLmOD,default=nLmOK)
def cmd_cpvcs_delete(name:nLmON,remote:nLmOB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_register_remote(name:nLmON):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=nLmOD)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=nLmOD)
def cmd_cpvcs_rename(name:nLmON,new_name:nLmON):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_commit(message:nLmON,name:nLmON):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=nLmOD,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_push(squash:nLmOB,message:nLmON,name:nLmON):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=nLmOu,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=nLmOD,default=nLmOK,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_inject(version:nLmOu,reset:nLmOB,name:nLmON):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=nLmOD,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=nLmOD,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:nLmON,inject:nLmOB,reset:nLmOB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=nLmOD,default=nLmOK)
def cmd_cpvcs_list_pods(remote:nLmOB):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_versions(name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=nLmOD,type=nLmOu)
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_version_info(version:nLmOu,name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=nLmOu,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_version_metamodel(version:nLmOu,name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=nLmOD,type=nLmOu,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=nLmOD,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=nLmOD,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=nLmOK,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_set_version(version:nLmOu,inject:nLmOB,reset:nLmOB,commit_before:nLmOB,name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_commits(version:nLmOu,name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=nLmOD)
@click.option("--commit","-c",required=nLmOD)
@click.option("-n","--name",help="Name of the cloud pod",required=nLmOD)
def cmd_cpvcs_commit_diff(version:nLmOu,commit:nLmOu,name:nLmON):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(nLmOw)
  status.stop()
...
def print_table(columns:Dict[nLmON,nLmON],rows:List[Dict[nLmON,Any]],formatters:Dict[nLmON,Callable[[Any],nLmON]]=nLmOw):
 from rich.table import Table
 if formatters is nLmOw:
  formatters=nLmOY()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  nLmOr(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->nLmOw:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->nLmOw:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->nLmOw:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->nLmOw:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
