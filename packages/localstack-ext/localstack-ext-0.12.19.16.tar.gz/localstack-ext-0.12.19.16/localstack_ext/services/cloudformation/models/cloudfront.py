...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or qKBQz
 @qKBQu
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return qKBQM(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @qKBQy
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if qKBQg(origins,qKBQe)else origins.get("Items")
  origins_to_create=qKBQS([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if qKBQg(aliases,qKBQe)else aliases.get("Items")
  aliases_to_create=qKBQS(aliases)
  for distr in distrs:
   origins=qKBQS([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=qKBQS(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",qKBQA)
   config["Enabled"]=qKBQs(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if qKBQg(custom_errors,qKBQD)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=qKBQs(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":qKBQA}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=qKBQs(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=qKBQz):
    parent=config if parent is qKBQz else parent
    items=parent.get(key)or[]
    if qKBQg(items,qKBQD)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":qKBQr(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if qKBQg(cache_behavs,qKBQD)else cache_behavs)
