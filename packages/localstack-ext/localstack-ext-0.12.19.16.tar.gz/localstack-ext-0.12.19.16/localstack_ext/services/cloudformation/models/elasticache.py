...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=prmbN(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not prmbu
   return result
  except prmbA:
   props=self.props
   ...
   if parts[0]in candidates and prmbE(parts)>1:
