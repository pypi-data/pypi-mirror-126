...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[xKpLm])[0]
 def get_physical_resource_id(self,attribute=xKpLm,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if xKpLR(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return xKpLW(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @xKpLA
 def add_defaults(resource,stack_name:xKpLy):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return xKpLW(SESReceiptRule,self).get_cfn_attribute(attribute)
 @xKpLT
