...
SENT_EMAILS=[]
EMAIL_BLACKLIST=VqBLc()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",VqBLN(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=VqBLo,from_name=VqBLo,smtp_host=VqBLo,smtp_user=VqBLo,smtp_pass=VqBLo,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if VqBLn(recipients,VqBLK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=VqBLo):
 ...
 result["From"]=formataddr((VqBLN(Header(from_name,"utf-8")),from_email))
 return result
