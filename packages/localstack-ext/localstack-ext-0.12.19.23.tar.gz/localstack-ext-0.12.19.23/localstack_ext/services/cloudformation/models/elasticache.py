...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tdoxs(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tdoxl
   return result
  except tdoxC:
   props=self.props
   ...
   if parts[0]in candidates and tdoxR(parts)>1:
