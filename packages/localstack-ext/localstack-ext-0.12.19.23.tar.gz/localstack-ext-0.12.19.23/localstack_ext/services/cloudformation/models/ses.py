...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[SdCRX])[0]
 def get_physical_resource_id(self,attribute=SdCRX,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if SdCRj(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return SdCRu(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @SdCRE
 def add_defaults(resource,stack_name:SdCRq):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return SdCRu(SESReceiptRule,self).get_cfn_attribute(attribute)
 @SdCRp
