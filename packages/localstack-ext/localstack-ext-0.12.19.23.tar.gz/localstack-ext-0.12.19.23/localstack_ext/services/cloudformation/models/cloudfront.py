...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or OcSNs
 @OcSNo
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return OcSNb(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @OcSNe
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if OcSNU(origins,OcSNA)else origins.get("Items")
  origins_to_create=OcSNa([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if OcSNU(aliases,OcSNA)else aliases.get("Items")
  aliases_to_create=OcSNa(aliases)
  for distr in distrs:
   origins=OcSNa([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=OcSNa(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",OcSNj)
   config["Enabled"]=OcSNh(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if OcSNU(custom_errors,OcSNl)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=OcSNh(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":OcSNj}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=OcSNh(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=OcSNs):
    parent=config if parent is OcSNs else parent
    items=parent.get(key)or[]
    if OcSNU(items,OcSNl)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":OcSNI(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if OcSNU(cache_behavs,OcSNl)else cache_behavs)
