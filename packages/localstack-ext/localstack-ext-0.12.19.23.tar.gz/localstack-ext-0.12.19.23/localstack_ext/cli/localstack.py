...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return CAtJq if e else CAtJe
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->CAtJK:
...
@click.option("--revert",is_flag=CAtJq,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:CAtJY):
...
def _cpvcs_initialized(pod_name:CAtJX)->CAtJY:
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_init(name:CAtJX):
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=CAtJq,default=CAtJe)
def cmd_cpvcs_delete(name:CAtJX,remote:CAtJY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_register_remote(name:CAtJX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=CAtJq)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=CAtJq)
def cmd_cpvcs_rename(name:CAtJX,new_name:CAtJX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_commit(message:CAtJX,name:CAtJX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=CAtJq,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_push(squash:CAtJY,message:CAtJX,name:CAtJX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=CAtJn,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=CAtJq,default=CAtJe,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_inject(version:CAtJn,reset:CAtJY,name:CAtJX):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=CAtJq,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=CAtJq,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:CAtJX,inject:CAtJY,reset:CAtJY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=CAtJq,default=CAtJe)
def cmd_cpvcs_list_pods(remote:CAtJY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_versions(name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=CAtJq,type=CAtJn)
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_version_info(version:CAtJn,name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=CAtJn,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_version_metamodel(version:CAtJn,name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=CAtJq,type=CAtJn,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=CAtJq,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=CAtJq,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=CAtJe,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_set_version(version:CAtJn,inject:CAtJY,reset:CAtJY,commit_before:CAtJY,name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_commits(version:CAtJn,name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=CAtJq)
@click.option("--commit","-c",required=CAtJq)
@click.option("-n","--name",help="Name of the cloud pod",required=CAtJq)
def cmd_cpvcs_commit_diff(version:CAtJn,commit:CAtJn,name:CAtJX):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(CAtJK)
  status.stop()
...
def print_table(columns:Dict[CAtJX,CAtJX],rows:List[Dict[CAtJX,Any]],formatters:Dict[CAtJX,Callable[[Any],CAtJX]]=CAtJK):
 from rich.table import Table
 if formatters is CAtJK:
  formatters=CAtJT()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  CAtJx(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->CAtJK:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->CAtJK:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->CAtJK:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->CAtJK:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
