...
SENT_EMAILS=[]
EMAIL_BLACKLIST=zOUNL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",zOUNh(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=zOUNX,from_name=zOUNX,smtp_host=zOUNX,smtp_user=zOUNX,smtp_pass=zOUNX,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if zOUNV(recipients,zOUNb)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=zOUNX):
 ...
 result["From"]=formataddr((zOUNh(Header(from_name,"utf-8")),from_email))
 return result
