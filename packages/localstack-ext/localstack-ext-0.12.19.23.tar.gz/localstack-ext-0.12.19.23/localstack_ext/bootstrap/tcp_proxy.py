...
class AttrDict(xbfdj):
 def __init__(self,*args,**kwargs):
  xbfda(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and xbfdo(socket,"IPV6_V6ONLY"):
...
class Half(xbfdp):
 def __init__(self,opt,sock,addr,xbfdS):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.xbfdS=xbfdS
  self.name="peer" if self.xbfdS else "client"
  self.queue=[]
  self.dest=xbfdn
  self.err=xbfdn
  self.ready=xbfdY
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":xbfdY,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":xbfdY,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":xbfdY,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":xbfdY,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
