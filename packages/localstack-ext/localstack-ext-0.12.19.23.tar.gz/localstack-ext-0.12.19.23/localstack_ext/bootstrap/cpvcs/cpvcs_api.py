...
def init(pod_name="My-Pod"):
 ...
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=KdvYk,state_files=KdvYr(),version_number=0)
 ...
 with KdvYF(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with KdvYF(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with KdvYF(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:KdvYt,meta_archives:Dict[KdvYt,KdvYt],state_archives:Dict[KdvYt,KdvYt],remote_info:Dict[KdvYt,KdvYt],pod_name:KdvYt):
...
def merge_from_remote(version_space_archive:KdvYt,meta_archives:Dict[KdvYt,KdvYt],state_archives:Dict[KdvYt,KdvYt]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->KdvYj:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:KdvYt):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:KdvYt)->KdvYj:
 ...
 return KdvYN
def list_locally_available_pods(show_remote_or_local:KdvYj=KdvYN)->Set[KdvYt]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:KdvYt,file_name:KdvYt,service:KdvYt,region:KdvYt)->KdvYt:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->KdvYt:
 ...
 return key
def _get_state_file_path(key:KdvYt)->KdvYt:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=KdvYr(KdvYV(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:KdvYt)->Optional[KdvYt]:
...
def get_version_info(version_no:KdvYU)->Dict[KdvYt,KdvYt]:
...
def read_file_from_archive(archive_path:KdvYt,file_name:KdvYt):
 try:
...
def commit(message:KdvYt=KdvYk)->Revision:
 ...
 curr_expansion_point_parent_state_files=KdvYr()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=KdvYk
...
def zip_directories(zip_dest:KdvYt,directories:List[KdvYt])->KdvYt:
 ...
 return zip_dest
def create_version_space_archive()->KdvYt:
...
def get_max_version_no()->KdvYU:
 with KdvYF(config_context.get_max_ver_path())as fp:
  return KdvYU(os.path.basename(fp.readline()))
...
def push(comment:KdvYt=KdvYk)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=KdvYr(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,get_version_state_archive(new_max_version_no)),state_files=KdvYr(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 ...
 return new_version
def get_version_meta_archive(version_no:KdvYU)->KdvYt:
...
def get_version_state_archive(version_no:KdvYU)->KdvYt:
...
def get_version_metamodel(version_no:KdvYU)->KdvYt:
 meta_archive=get_version_meta_archive(version_no)
 if meta_archive:
  return read_file_from_archive(meta_archive,METAMODELS_FILE)
def _create_metadata_archive(version_node:Version,delete_reference:KdvYj=KdvYL):
 ...
 with KdvYF(metamodels_dest,"w")as fp:
  metamodels=json_safe(metamodels)
  json.dump(metamodels,fp,indent=4)
 with KdvYF(version_service_info,"w")as fp:
...
def _create_state_directory(version_number:KdvYU,state_file_refs:Set[StateFileRef],delete_files=KdvYL,archive=KdvYN):
...
def set_active_version(version_no:KdvYU,commit_before=KdvYL)->KdvYj:
 ...
 return KdvYL
def _set_active_version(key:KdvYt):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=KdvYr(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:KdvYU)->Version:
 versions=load_version_references()
 version_ref=KdvYB((version[1]for version in versions if version[0]==version_no),KdvYk)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[KdvYU,KdvYt]]:
 result={}
 with KdvYF(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with KdvYF(symlink,"r")as sp:
    result[KdvYU(os.path.basename(symlink))]=sp.readline()
 return KdvYm(result.items(),key=lambda x:x[0],reverse=KdvYN)
def list_versions()->List[KdvYt]:
 ...
 return result
def list_version_commits(version_no:KdvYU)->List[KdvYt]:
 if version_no==-1:
 ...
 return result
def get_commit_diff(version_no:KdvYU,commit_no:KdvYU)->KdvYt:
 archive_path=get_version_meta_archive(version_no)
 if not archive_path:
...
def _update_head(new_head_ver_no,new_head_key)->KdvYt:
 with KdvYF(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->KdvYt:
 with KdvYF(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->KdvYt:
 with KdvYF(config_context.get_known_ver_path(),"a")as fp:
...
def _merge_two_state_dir_func(**kwargs):
 def _is_special_case(obj)->KdvYj:
  return KdvYa(obj,(Queue,FakeBucket))
...
def _merge_expansion_point_with_max():
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=KdvYL)
...
def _infer_backend_init(clazz,sf):
 if KdvYa(clazz,KdvYw):
  backend={}
 else:
  constructor=KdvYs(clazz,"__init__",KdvYk)
  ...
  if "region" in sig_args.args:
   ...
   backend=clazz()
 return backend
def _create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef])->KdvYt:
 ...
 def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[KdvYt,StateFileRef]:
 ...
 for state_file_from in state_files_from_regular:
  ...
  if state_file_from.any_congruence(state_files_to_regular):
   ...
   diff_json=_create_diff_json(state_file_from,KdvYk)
 ...
 for state_files_to in state_files_to_lookup.values():
  ...
  diff_json=_create_diff_json(KdvYk,state_files_to)
  result_file["diff"]=diff_json
...
def _create_diff_json(sf1:StateFileRef,sf2:StateFileRef):
 ...
 if not sf1:
  backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
  clazz=KdvYD(backend2)
  backend1=_infer_backend_init(clazz,sf2)
 else:
  backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
  clazz=KdvYD(backend1)
