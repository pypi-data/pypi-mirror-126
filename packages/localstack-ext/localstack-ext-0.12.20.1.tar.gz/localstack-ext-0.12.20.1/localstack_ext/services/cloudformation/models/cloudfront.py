...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xnGKj
 @xnGKh
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xnGKu(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xnGKg
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xnGKM(origins,xnGKR)else origins.get("Items")
  origins_to_create=xnGKv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xnGKM(aliases,xnGKR)else aliases.get("Items")
  aliases_to_create=xnGKv(aliases)
  for distr in distrs:
   origins=xnGKv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xnGKv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xnGKc)
   config["Enabled"]=xnGKq(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xnGKM(custom_errors,xnGKX)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xnGKq(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xnGKc}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xnGKq(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xnGKj):
    parent=config if parent is xnGKj else parent
    items=parent.get(key)or[]
    if xnGKM(items,xnGKX)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xnGKw(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xnGKM(cache_behavs,xnGKX)else cache_behavs)
