...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=rjTsV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not rjTsP
   return result
  except rjTsX:
   props=self.props
   ...
   if parts[0]in candidates and rjTsG(parts)>1:
