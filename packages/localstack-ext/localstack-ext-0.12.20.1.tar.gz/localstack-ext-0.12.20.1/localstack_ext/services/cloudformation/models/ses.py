...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[NCfQp])[0]
 def get_physical_resource_id(self,attribute=NCfQp,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if NCfQV(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return NCfQs(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @NCfQD
 def add_defaults(resource,stack_name:NCfQv):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return NCfQs(SESReceiptRule,self).get_cfn_attribute(attribute)
 @NCfQG
