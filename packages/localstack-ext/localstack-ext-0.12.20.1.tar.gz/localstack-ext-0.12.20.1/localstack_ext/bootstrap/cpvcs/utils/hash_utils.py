...
def random_hash()->NibhT:
 return NibhB(random.getrandbits(160))
def compute_file_hash(file_path:NibhT,accum=Nibhj)->Optional[NibhT]:
 try:
  with NibhY(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:NibhG,state_archive:NibhT)->NibhT:
 ...
 m.update(NibhT(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->NibhT:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=Nibhs(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with NibhY(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except Nibhr as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if NibhS(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(NibhT(cpvcs_node.revision_number).encode("utf-8"))
 elif NibhS(cpvcs_node,Version):
  m.update(NibhT(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
