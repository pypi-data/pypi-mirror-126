...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=BJsHK
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:BJsHX)->BJsHX:
  pass
 @BJsHD
 def get(cls,requested_file_locator:BJsHX):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=BJsHK
 @BJsHD
 def get(cls,state_file_locator:BJsHX,requested_storage:BJsHX,serializers:Dict[BJsHX,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[BJsHX,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:BJsHX)->BJsHX:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:BJsHX)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:BJsHX)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:BJsHX)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:BJsHX)->BJsHX:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[BJsHX]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:BJsHX,new_key:BJsHX,referenced_by_version:BJsHX=BJsHK):
  pass
 @abc.abstractmethod
 def version_exists(self,key:BJsHX)->BJsHE:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:BJsHX,key:BJsHX):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:BJsHX,new_key:BJsHX,base_path:BJsHX)->BJsHE:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[BJsHX]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[BJsHX,BJsHX],remote_location:BJsHX=BJsHK)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[BJsHX]:
  ...
  for cpvcs_obj in args:
   if BJsHt(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif BJsHt(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[BJsHX,BJsHX],remote_location:BJsHX=BJsHK)->List[Union[Revision,Version]]:
