...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return zpdfh if e else zpdfk
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->zpdfL:
...
@click.option("--revert",is_flag=zpdfh,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:zpdfr):
...
def _cpvcs_initialized(pod_name:zpdfj)->zpdfr:
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_init(name:zpdfj):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=zpdfh,default=zpdfk)
def cmd_cpvcs_delete(name:zpdfj,remote:zpdfr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_register_remote(name:zpdfj):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=zpdfh)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=zpdfh)
def cmd_cpvcs_rename(name:zpdfj,new_name:zpdfj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_commit(message:zpdfj,name:zpdfj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=zpdfh,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_push(squash:zpdfr,message:zpdfj,name:zpdfj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=zpdfG,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=zpdfh,default=zpdfk,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_inject(version:zpdfG,reset:zpdfr,name:zpdfj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=zpdfh,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=zpdfh,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:zpdfj,inject:zpdfr,reset:zpdfr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=zpdfh,default=zpdfk)
def cmd_cpvcs_list_pods(remote:zpdfr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_versions(name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zpdfh,type=zpdfG)
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_version_info(version:zpdfG,name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=zpdfG,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_version_metamodel(version:zpdfG,name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zpdfh,type=zpdfG,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=zpdfh,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=zpdfh,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=zpdfk,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_set_version(version:zpdfG,inject:zpdfr,reset:zpdfr,commit_before:zpdfr,name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_commits(version:zpdfG,name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=zpdfh)
@click.option("--commit","-c",required=zpdfh)
@click.option("-n","--name",help="Name of the cloud pod",required=zpdfh)
def cmd_cpvcs_commit_diff(version:zpdfG,commit:zpdfG,name:zpdfj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(zpdfL)
  status.stop()
...
def print_table(columns:Dict[zpdfj,zpdfj],rows:List[Dict[zpdfj,Any]],formatters:Dict[zpdfj,Callable[[Any],zpdfj]]=zpdfL):
 from rich.table import Table
 if formatters is zpdfL:
  formatters=zpdfe()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  zpdfg(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->zpdfL:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->zpdfL:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->zpdfL:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->zpdfL:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
