...
def extract_meta_and_state_archives(meta_archives:Dict[JLEcA,JLEcA],state_archives:Dict[JLEcA,JLEcA]):
  ...
  for version_no,archive in product_space_archive.items():
   with zipfile.ZipFile(archive)as meta_zip:
    if product_space_archive==meta_archives:
     archive_dest=config_context.get_version_meta_archive_path(version_no=version_no,with_format=JLEcD)
    else:
     archive_dest=config_context.get_version_state_archive_path(version_no=version_no,with_format=JLEcD)
...
def register_remote(remote_info:Dict[JLEcA,JLEcA]):
 if config_context.is_remotly_managed():
  LOG.warning("Pod is already remotely managed")
  return
 with JLEcb(config_context.get_remote_info_path(),"w")as fp:
...
def merge_version_space(version_space_archive):
 ...
 for version_ref_file in os.listdir(remote_config_context.get_ver_refs_path()):
  ...
  with JLEcb(remote_version_ref_file_path,"r")as fp:
...
def _create_metamodel_helper(obj,width=25,visited:set=JLEcj):
 if obj is JLEcj:
  return obj
 ...
 elif JLEcY(obj,JLEcI):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if JLEcS(result)>width:
   result={"size":JLEcS(result),"items":result[:width]}
 return result
