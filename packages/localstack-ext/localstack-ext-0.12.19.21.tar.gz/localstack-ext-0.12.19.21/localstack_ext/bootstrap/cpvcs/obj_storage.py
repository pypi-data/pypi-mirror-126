...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=gAdiy
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:gAdiv)->gAdiv:
  pass
 @gAdiO
 def get(cls,requested_file_locator:gAdiv):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=gAdiy
 @gAdiO
 def get(cls,state_file_locator:gAdiv,requested_storage:gAdiv,serializers:Dict[gAdiv,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[gAdiv,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:gAdiv)->gAdiv:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:gAdiv)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:gAdiv)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:gAdiv)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:gAdiv)->gAdiv:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[gAdiv]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:gAdiv,new_key:gAdiv,referenced_by_version:gAdiv=gAdiy):
  pass
 @abc.abstractmethod
 def version_exists(self,key:gAdiv)->gAdiE:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:gAdiv,key:gAdiv):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:gAdiv,new_key:gAdiv,base_path:gAdiv)->gAdiE:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[gAdiv]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[gAdiv,gAdiv],remote_location:gAdiv=gAdiy)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[gAdiv]:
  ...
  for cpvcs_obj in args:
   if gAdiC(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif gAdiC(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[gAdiv,gAdiv],remote_location:gAdiv=gAdiy)->List[Union[Revision,Version]]:
