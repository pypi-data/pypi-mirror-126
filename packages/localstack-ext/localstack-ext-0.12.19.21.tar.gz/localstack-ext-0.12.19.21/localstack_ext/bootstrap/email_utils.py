...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DKioE()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DKion(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DKiop,from_name=DKiop,smtp_host=DKiop,smtp_user=DKiop,smtp_pass=DKiop,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DKioL(recipients,DKioe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DKiop):
 ...
 result["From"]=formataddr((DKion(Header(from_name,"utf-8")),from_email))
 return result
