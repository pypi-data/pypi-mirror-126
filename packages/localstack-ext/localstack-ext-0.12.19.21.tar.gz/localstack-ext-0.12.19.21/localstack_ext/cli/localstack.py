...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return pxhHE if e else pxhHN
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->pxhHC:
...
@click.option("--revert",is_flag=pxhHE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:pxhHv):
...
def _cpvcs_initialized(pod_name:pxhHr)->pxhHv:
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_init(name:pxhHr):
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=pxhHE,default=pxhHN)
def cmd_cpvcs_delete(name:pxhHr,remote:pxhHv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_register_remote(name:pxhHr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=pxhHE)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=pxhHE)
def cmd_cpvcs_rename(name:pxhHr,new_name:pxhHr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_commit(message:pxhHr,name:pxhHr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=pxhHE,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_push(squash:pxhHv,message:pxhHr,name:pxhHr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=pxhHg,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=pxhHE,default=pxhHN,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_inject(version:pxhHg,reset:pxhHv,name:pxhHr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=pxhHE,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=pxhHE,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:pxhHr,inject:pxhHv,reset:pxhHv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=pxhHE,default=pxhHN)
def cmd_cpvcs_list_pods(remote:pxhHv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_versions(name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=pxhHE,type=pxhHg)
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_version_info(version:pxhHg,name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=pxhHg,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_version_metamodel(version:pxhHg,name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=pxhHE,type=pxhHg,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=pxhHE,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=pxhHE,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=pxhHN,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_set_version(version:pxhHg,inject:pxhHv,reset:pxhHv,commit_before:pxhHv,name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_commits(version:pxhHg,name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=pxhHE)
@click.option("--commit","-c",required=pxhHE)
@click.option("-n","--name",help="Name of the cloud pod",required=pxhHE)
def cmd_cpvcs_commit_diff(version:pxhHg,commit:pxhHg,name:pxhHr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(pxhHC)
  status.stop()
...
def print_table(columns:Dict[pxhHr,pxhHr],rows:List[Dict[pxhHr,Any]],formatters:Dict[pxhHr,Callable[[Any],pxhHr]]=pxhHC):
 from rich.table import Table
 if formatters is pxhHC:
  formatters=pxhHX()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  pxhHt(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->pxhHC:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->pxhHC:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->pxhHC:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->pxhHC:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
