...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=kVOwp(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not kVOwn
   return result
  except kVOwh:
   props=self.props
   ...
   if parts[0]in candidates and kVOwe(parts)>1:
