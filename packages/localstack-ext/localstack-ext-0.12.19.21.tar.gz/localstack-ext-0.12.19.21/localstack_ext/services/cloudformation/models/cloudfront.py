...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or HoEjI
 @HoEjn
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return HoEjX(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @HoEjd
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if HoEjb(origins,HoEje)else origins.get("Items")
  origins_to_create=HoEjT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if HoEjb(aliases,HoEje)else aliases.get("Items")
  aliases_to_create=HoEjT(aliases)
  for distr in distrs:
   origins=HoEjT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=HoEjT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",HoEjQ)
   config["Enabled"]=HoEjf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if HoEjb(custom_errors,HoEjc)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=HoEjf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":HoEjQ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=HoEjf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=HoEjI):
    parent=config if parent is HoEjI else parent
    items=parent.get(key)or[]
    if HoEjb(items,HoEjc)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":HoEji(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if HoEjb(cache_behavs,HoEjc)else cache_behavs)
