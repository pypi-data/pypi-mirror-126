...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[dOIzK])[0]
 def get_physical_resource_id(self,attribute=dOIzK,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if dOIzF(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dOIzi(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @dOIzV
 def add_defaults(resource,stack_name:dOIzc):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dOIzi(SESReceiptRule,self).get_cfn_attribute(attribute)
 @dOIzG
