...
SENT_EMAILS=[]
EMAIL_BLACKLIST=LcMdj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",LcMdr(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=LcMdm,from_name=LcMdm,smtp_host=LcMdm,smtp_user=LcMdm,smtp_pass=LcMdm,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if LcMdi(recipients,LcMdn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=LcMdm):
 ...
 result["From"]=formataddr((LcMdr(Header(from_name,"utf-8")),from_email))
 return result
