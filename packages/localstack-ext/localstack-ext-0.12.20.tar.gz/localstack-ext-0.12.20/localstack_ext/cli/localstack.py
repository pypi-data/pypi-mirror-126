...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return cNWJC if e else cNWJB
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->cNWJI:
...
@click.option("--revert",is_flag=cNWJC,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:cNWJA):
...
def _cpvcs_initialized(pod_name:cNWJU)->cNWJA:
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_init(name:cNWJU):
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=cNWJC,default=cNWJB)
def cmd_cpvcs_delete(name:cNWJU,remote:cNWJA):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_register_remote(name:cNWJU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=cNWJC)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=cNWJC)
def cmd_cpvcs_rename(name:cNWJU,new_name:cNWJU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_commit(message:cNWJU,name:cNWJU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=cNWJC,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_push(squash:cNWJA,message:cNWJU,name:cNWJU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=cNWJO,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=cNWJC,default=cNWJB,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_inject(version:cNWJO,reset:cNWJA,name:cNWJU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=cNWJC,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=cNWJC,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:cNWJU,inject:cNWJA,reset:cNWJA):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=cNWJC,default=cNWJB)
def cmd_cpvcs_list_pods(remote:cNWJA):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_versions(name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=cNWJC,type=cNWJO)
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_version_info(version:cNWJO,name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=cNWJO,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_version_metamodel(version:cNWJO,name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=cNWJC,type=cNWJO,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=cNWJC,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=cNWJC,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=cNWJB,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_set_version(version:cNWJO,inject:cNWJA,reset:cNWJA,commit_before:cNWJA,name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_commits(version:cNWJO,name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=cNWJC)
@click.option("--commit","-c",required=cNWJC)
@click.option("-n","--name",help="Name of the cloud pod",required=cNWJC)
def cmd_cpvcs_commit_diff(version:cNWJO,commit:cNWJO,name:cNWJU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(cNWJI)
  status.stop()
...
def print_table(columns:Dict[cNWJU,cNWJU],rows:List[Dict[cNWJU,Any]],formatters:Dict[cNWJU,Callable[[Any],cNWJU]]=cNWJI):
 from rich.table import Table
 if formatters is cNWJI:
  formatters=cNWJH()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  cNWJj(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->cNWJI:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->cNWJI:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->cNWJI:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->cNWJI:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
