...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[EXdPk])[0]
 def get_physical_resource_id(self,attribute=EXdPk,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if EXdPM(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return EXdPK(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @EXdPN
 def add_defaults(resource,stack_name:EXdPi):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return EXdPK(SESReceiptRule,self).get_cfn_attribute(attribute)
 @EXdPO
