...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=toSVK(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not toSVq
   return result
  except toSVp:
   props=self.props
   ...
   if parts[0]in candidates and toSVY(parts)>1:
