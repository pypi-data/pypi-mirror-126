...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or QTUMj
 @QTUMB
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return QTUMD(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @QTUMu
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if QTUMf(origins,QTUMn)else origins.get("Items")
  origins_to_create=QTUMA([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if QTUMf(aliases,QTUMn)else aliases.get("Items")
  aliases_to_create=QTUMA(aliases)
  for distr in distrs:
   origins=QTUMA([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=QTUMA(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",QTUMX)
   config["Enabled"]=QTUMo(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if QTUMf(custom_errors,QTUMJ)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=QTUMo(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":QTUMX}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=QTUMo(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=QTUMj):
    parent=config if parent is QTUMj else parent
    items=parent.get(key)or[]
    if QTUMf(items,QTUMJ)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":QTUMq(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if QTUMf(cache_behavs,QTUMJ)else cache_behavs)
