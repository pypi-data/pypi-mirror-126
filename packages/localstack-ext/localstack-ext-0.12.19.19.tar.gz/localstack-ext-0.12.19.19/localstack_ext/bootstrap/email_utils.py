...
SENT_EMAILS=[]
EMAIL_BLACKLIST=oSnXA()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",oSnXg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=oSnXH,from_name=oSnXH,smtp_host=oSnXH,smtp_user=oSnXH,smtp_pass=oSnXH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if oSnXr(recipients,oSnXM)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=oSnXH):
 ...
 result["From"]=formataddr((oSnXg(Header(from_name,"utf-8")),from_email))
 return result
