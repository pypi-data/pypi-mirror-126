...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=VKMfe
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:VKMfh)->VKMfh:
  pass
 @VKMfl
 def get(cls,requested_file_locator:VKMfh):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=VKMfe
 @VKMfl
 def get(cls,state_file_locator:VKMfh,requested_storage:VKMfh,serializers:Dict[VKMfh,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[VKMfh,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:VKMfh)->VKMfh:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:VKMfh)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:VKMfh)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:VKMfh)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:VKMfh)->VKMfh:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[VKMfh]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:VKMfh,new_key:VKMfh,referenced_by_version:VKMfh=VKMfe):
  pass
 @abc.abstractmethod
 def version_exists(self,key:VKMfh)->VKMfA:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:VKMfh,key:VKMfh):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:VKMfh,new_key:VKMfh,base_path:VKMfh)->VKMfA:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[VKMfh]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[VKMfh,VKMfh],remote_location:VKMfh=VKMfe)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[VKMfh]:
  ...
  for cpvcs_obj in args:
   if VKMfv(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif VKMfv(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[VKMfh,VKMfh],remote_location:VKMfh=VKMfe)->List[Union[Revision,Version]]:
