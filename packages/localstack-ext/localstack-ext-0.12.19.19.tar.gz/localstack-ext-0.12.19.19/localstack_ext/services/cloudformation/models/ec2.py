...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=OYFcu,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if OYFcb(perm["IpProtocol"])!=OYFcb(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",OYFcu)
   vpc_id=OYFcu
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or OYFcu
    vpc_id=groups and groups[0].get("VpcId")or OYFcu
   dst_group_id=result.pop("DestinationSecurityGroupId",OYFcu)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=OYFcb(result.get("IpProtocol"))
   description=result.pop("Description",OYFcu)
   cidr_ipv6=result.pop("CidrIpv6",OYFcu)
