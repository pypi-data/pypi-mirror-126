...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=gMIKP(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not gMIKj
   return result
  except gMIKR:
   props=self.props
   ...
   if parts[0]in candidates and gMIKq(parts)>1:
