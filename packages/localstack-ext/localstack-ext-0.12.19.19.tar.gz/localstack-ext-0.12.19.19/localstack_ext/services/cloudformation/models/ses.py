...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[bfGIC])[0]
 def get_physical_resource_id(self,attribute=bfGIC,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if bfGIP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return bfGIJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @bfGIN
 def add_defaults(resource,stack_name:bfGIz):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return bfGIJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @bfGIB
