...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Xxfwe
 @Xxfwy
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return XxfwG(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @XxfwS
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if Xxfwu(origins,XxfwU)else origins.get("Items")
  origins_to_create=XxfwP([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if Xxfwu(aliases,XxfwU)else aliases.get("Items")
  aliases_to_create=XxfwP(aliases)
  for distr in distrs:
   origins=XxfwP([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=XxfwP(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Xxfwg)
   config["Enabled"]=Xxfwj(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if Xxfwu(custom_errors,Xxfws)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Xxfwj(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Xxfwg}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Xxfwj(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Xxfwe):
    parent=config if parent is Xxfwe else parent
    items=parent.get(key)or[]
    if Xxfwu(items,Xxfws)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Xxfwc(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if Xxfwu(cache_behavs,Xxfws)else cache_behavs)
