...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return cfbgk if e else cfbgW
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->cfbgi:
...
@click.option("--revert",is_flag=cfbgk,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:cfbgF):
...
def _cpvcs_initialized(pod_name:cfbgd)->cfbgF:
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_init(name:cfbgd):
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=cfbgk,default=cfbgW)
def cmd_cpvcs_delete(name:cfbgd,remote:cfbgF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_register_remote(name:cfbgd):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=cfbgk)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=cfbgk)
def cmd_cpvcs_rename(name:cfbgd,new_name:cfbgd):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_commit(message:cfbgd,name:cfbgd):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=cfbgk,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_push(squash:cfbgF,message:cfbgd,name:cfbgd):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=cfbgp,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=cfbgk,default=cfbgW,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_inject(version:cfbgp,reset:cfbgF,name:cfbgd):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=cfbgk,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=cfbgk,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:cfbgd,inject:cfbgF,reset:cfbgF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=cfbgk,default=cfbgW)
def cmd_cpvcs_list_pods(remote:cfbgF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_versions(name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=cfbgk,type=cfbgp)
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_version_info(version:cfbgp,name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=cfbgp,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_version_metamodel(version:cfbgp,name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=cfbgk,type=cfbgp,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=cfbgk,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=cfbgk,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=cfbgW,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_set_version(version:cfbgp,inject:cfbgF,reset:cfbgF,commit_before:cfbgF,name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_commits(version:cfbgp,name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=cfbgk)
@click.option("--commit","-c",required=cfbgk)
@click.option("-n","--name",help="Name of the cloud pod",required=cfbgk)
def cmd_cpvcs_commit_diff(version:cfbgp,commit:cfbgp,name:cfbgd):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(cfbgi)
  status.stop()
...
def print_table(columns:Dict[cfbgd,cfbgd],rows:List[Dict[cfbgd,Any]],formatters:Dict[cfbgd,Callable[[Any],cfbgd]]=cfbgi):
 from rich.table import Table
 if formatters is cfbgi:
  formatters=cfbgU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  cfbgB(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->cfbgi:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->cfbgi:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->cfbgi:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->cfbgi:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
