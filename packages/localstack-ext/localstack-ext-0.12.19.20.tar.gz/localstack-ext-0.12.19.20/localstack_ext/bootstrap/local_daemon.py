...
class FuncThread(threading.Thread):
 def __init__(self,func,params=LQBjf):
...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if LQBjF(result,LQBje)else result
  return result
 def read_content(self):
  if LQBjE(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(LQBjD(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%LQBjV(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",LQBjV(LQBjl(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=LQBjf):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 if is_linux():
  ...
  for interface in interfaces:
   try:
    ...
    pass
 raise LQBjG("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except LQBjG as e:
  error_string=LQBjV(e)
  if LQBjF(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=LQBjb))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=LQBjb))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if LQBjF(obj,LQBjV)else obj
def to_str(obj):
 return obj.decode("utf-8")if LQBjF(obj,LQBjW)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if LQBjl(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=LQBjb)
