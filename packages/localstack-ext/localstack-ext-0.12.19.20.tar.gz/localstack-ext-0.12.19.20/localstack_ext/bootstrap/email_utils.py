...
SENT_EMAILS=[]
EMAIL_BLACKLIST=HxUSe()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",HxUSY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=HxUSd,from_name=HxUSd,smtp_host=HxUSd,smtp_user=HxUSd,smtp_pass=HxUSd,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if HxUSt(recipients,HxUSu)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=HxUSd):
 ...
 result["From"]=formataddr((HxUSY(Header(from_name,"utf-8")),from_email))
 return result
