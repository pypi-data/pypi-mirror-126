...
class AttrDict(RDNLx):
 def __init__(self,*args,**kwargs):
  RDNLH(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and RDNLg(socket,"IPV6_V6ONLY"):
...
class Half(RDNLp):
 def __init__(self,opt,sock,addr,RDNLh):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.RDNLh=RDNLh
  self.name="peer" if self.RDNLh else "client"
  self.queue=[]
  self.dest=RDNLT
  self.err=RDNLT
  self.ready=RDNLv
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":RDNLv,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":RDNLv,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":RDNLv,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":RDNLv,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
