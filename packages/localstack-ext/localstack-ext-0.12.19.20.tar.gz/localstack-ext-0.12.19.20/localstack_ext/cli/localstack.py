...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return NnaJX if e else NnaJt
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->NnaJL:
...
@click.option("--revert",is_flag=NnaJX,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:NnaJv):
...
def _cpvcs_initialized(pod_name:NnaJf)->NnaJv:
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_init(name:NnaJf):
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=NnaJX,default=NnaJt)
def cmd_cpvcs_delete(name:NnaJf,remote:NnaJv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_register_remote(name:NnaJf):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=NnaJX)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=NnaJX)
def cmd_cpvcs_rename(name:NnaJf,new_name:NnaJf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_commit(message:NnaJf,name:NnaJf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=NnaJX,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_push(squash:NnaJv,message:NnaJf,name:NnaJf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=NnaJk,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=NnaJX,default=NnaJt,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_inject(version:NnaJk,reset:NnaJv,name:NnaJf):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=NnaJX,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=NnaJX,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:NnaJf,inject:NnaJv,reset:NnaJv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=NnaJX,default=NnaJt)
def cmd_cpvcs_list_pods(remote:NnaJv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_versions(name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=NnaJX,type=NnaJk)
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_version_info(version:NnaJk,name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=NnaJk,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_version_metamodel(version:NnaJk,name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=NnaJX,type=NnaJk,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=NnaJX,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=NnaJX,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=NnaJt,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_set_version(version:NnaJk,inject:NnaJv,reset:NnaJv,commit_before:NnaJv,name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_commits(version:NnaJk,name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=NnaJX)
@click.option("--commit","-c",required=NnaJX)
@click.option("-n","--name",help="Name of the cloud pod",required=NnaJX)
def cmd_cpvcs_commit_diff(version:NnaJk,commit:NnaJk,name:NnaJf):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(NnaJL)
  status.stop()
...
def print_table(columns:Dict[NnaJf,NnaJf],rows:List[Dict[NnaJf,Any]],formatters:Dict[NnaJf,Callable[[Any],NnaJf]]=NnaJL):
 from rich.table import Table
 if formatters is NnaJL:
  formatters=NnaJM()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  NnaJl(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->NnaJL:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->NnaJL:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->NnaJL:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->NnaJL:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
