...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[shbXO])[0]
 def get_physical_resource_id(self,attribute=shbXO,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if shbXK(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return shbXu(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @shbXW
 def add_defaults(resource,stack_name:shbXp):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return shbXu(SESReceiptRule,self).get_cfn_attribute(attribute)
 @shbXo
