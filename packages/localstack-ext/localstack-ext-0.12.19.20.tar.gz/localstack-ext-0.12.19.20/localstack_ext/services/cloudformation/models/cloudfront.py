...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or IYGHJ
 @IYGHx
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return IYGHN(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @IYGHF
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if IYGHh(origins,IYGHf)else origins.get("Items")
  origins_to_create=IYGHp([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if IYGHh(aliases,IYGHf)else aliases.get("Items")
  aliases_to_create=IYGHp(aliases)
  for distr in distrs:
   origins=IYGHp([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=IYGHp(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",IYGHl)
   config["Enabled"]=IYGHg(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if IYGHh(custom_errors,IYGHd)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=IYGHg(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":IYGHl}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=IYGHg(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=IYGHJ):
    parent=config if parent is IYGHJ else parent
    items=parent.get(key)or[]
    if IYGHh(items,IYGHd)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":IYGHX(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if IYGHh(cache_behavs,IYGHd)else cache_behavs)
