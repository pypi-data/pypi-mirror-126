...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=QqWfL(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not QqWfd
   return result
  except QqWfm:
   props=self.props
   ...
   if parts[0]in candidates and QqWfE(parts)>1:
