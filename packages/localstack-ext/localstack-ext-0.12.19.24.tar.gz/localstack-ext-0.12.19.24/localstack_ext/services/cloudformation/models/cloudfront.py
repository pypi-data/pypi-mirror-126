...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or cpBaK
 @cpBaC
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return cpBaN(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @cpBaj
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if cpBaW(origins,cpBaP)else origins.get("Items")
  origins_to_create=cpBad([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if cpBaW(aliases,cpBaP)else aliases.get("Items")
  aliases_to_create=cpBad(aliases)
  for distr in distrs:
   origins=cpBad([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=cpBad(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",cpBaQ)
   config["Enabled"]=cpBam(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if cpBaW(custom_errors,cpBaz)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=cpBam(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":cpBaQ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=cpBam(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=cpBaK):
    parent=config if parent is cpBaK else parent
    items=parent.get(key)or[]
    if cpBaW(items,cpBaz)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":cpBal(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if cpBaW(cache_behavs,cpBaz)else cache_behavs)
