...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=sJNcX(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not sJNcb
   return result
  except sJNcU:
   props=self.props
   ...
   if parts[0]in candidates and sJNcE(parts)>1:
