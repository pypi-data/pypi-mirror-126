...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[eQpvc])[0]
 def get_physical_resource_id(self,attribute=eQpvc,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if eQpvV(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return eQpvj(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @eQpvJ
 def add_defaults(resource,stack_name:eQpvW):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return eQpvj(SESReceiptRule,self).get_cfn_attribute(attribute)
 @eQpvn
