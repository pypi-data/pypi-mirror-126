...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return KmESq if e else KmESl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->KmESh:
...
@click.option("--revert",is_flag=KmESq,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:KmESP):
...
def _cpvcs_initialized(pod_name:KmESb)->KmESP:
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_init(name:KmESb):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=KmESq,default=KmESl)
def cmd_cpvcs_delete(name:KmESb,remote:KmESP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_register_remote(name:KmESb):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=KmESq)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=KmESq)
def cmd_cpvcs_rename(name:KmESb,new_name:KmESb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_commit(message:KmESb,name:KmESb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=KmESq,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_push(squash:KmESP,message:KmESb,name:KmESb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=KmESv,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=KmESq,default=KmESl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_inject(version:KmESv,reset:KmESP,name:KmESb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=KmESq,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=KmESq,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:KmESb,inject:KmESP,reset:KmESP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=KmESq,default=KmESl)
def cmd_cpvcs_list_pods(remote:KmESP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_versions(name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=KmESq,type=KmESv)
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_version_info(version:KmESv,name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=KmESv,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_version_metamodel(version:KmESv,name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=KmESq,type=KmESv,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=KmESq,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=KmESq,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=KmESl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_set_version(version:KmESv,inject:KmESP,reset:KmESP,commit_before:KmESP,name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_commits(version:KmESv,name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=KmESq)
@click.option("--commit","-c",required=KmESq)
@click.option("-n","--name",help="Name of the cloud pod",required=KmESq)
def cmd_cpvcs_commit_diff(version:KmESv,commit:KmESv,name:KmESb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(KmESh)
  status.stop()
...
def print_table(columns:Dict[KmESb,KmESb],rows:List[Dict[KmESb,Any]],formatters:Dict[KmESb,Callable[[Any],KmESb]]=KmESh):
 from rich.table import Table
 if formatters is KmESh:
  formatters=KmESu()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  KmESk(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->KmESh:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->KmESh:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->KmESh:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->KmESh:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
