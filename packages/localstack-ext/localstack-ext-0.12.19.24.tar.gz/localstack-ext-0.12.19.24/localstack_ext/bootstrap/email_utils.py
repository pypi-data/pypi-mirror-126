...
SENT_EMAILS=[]
EMAIL_BLACKLIST=vswWp()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",vswWr(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=vswWU,from_name=vswWU,smtp_host=vswWU,smtp_user=vswWU,smtp_pass=vswWU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if vswWh(recipients,vswWA)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=vswWU):
 ...
 result["From"]=formataddr((vswWr(Header(from_name,"utf-8")),from_email))
 return result
