...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=jmSxd(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not jmSxU
   return result
  except jmSxW:
   props=self.props
   ...
   if parts[0]in candidates and jmSxH(parts)>1:
