...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or VgJiB
 @VgJiP
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return VgJiX(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @VgJim
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if VgJiu(origins,VgJio)else origins.get("Items")
  origins_to_create=VgJiQ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if VgJiu(aliases,VgJio)else aliases.get("Items")
  aliases_to_create=VgJiQ(aliases)
  for distr in distrs:
   origins=VgJiQ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=VgJiQ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",VgJik)
   config["Enabled"]=VgJiA(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if VgJiu(custom_errors,VgJip)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=VgJiA(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":VgJik}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=VgJiA(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=VgJiB):
    parent=config if parent is VgJiB else parent
    items=parent.get(key)or[]
    if VgJiu(items,VgJip)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":VgJiS(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if VgJiu(cache_behavs,VgJip)else cache_behavs)
