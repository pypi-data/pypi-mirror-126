...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[rpIow])[0]
 def get_physical_resource_id(self,attribute=rpIow,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if rpIoL(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return rpIoR(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @rpIoY
 def add_defaults(resource,stack_name:rpIou):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return rpIoR(SESReceiptRule,self).get_cfn_attribute(attribute)
 @rpIoV
