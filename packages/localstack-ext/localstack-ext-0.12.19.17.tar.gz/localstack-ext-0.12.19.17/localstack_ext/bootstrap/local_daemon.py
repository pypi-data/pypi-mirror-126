...
class FuncThread(threading.Thread):
 def __init__(self,func,params=HSwFX):
...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if HSwFT(result,HSwFi)else result
  return result
 def read_content(self):
  if HSwFv(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(HSwFV(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%HSwFB(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",HSwFB(HSwFA(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=HSwFX):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 if is_linux():
  ...
  for interface in interfaces:
   try:
    ...
    pass
 raise HSwFu("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except HSwFu as e:
  error_string=HSwFB(e)
  if HSwFT(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=HSwFg))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=HSwFg))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if HSwFT(obj,HSwFB)else obj
def to_str(obj):
 return obj.decode("utf-8")if HSwFT(obj,HSwFb)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if HSwFA(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=HSwFg)
