...
def apply_patches():
 ...
 extended_proto_cmds.update({"SITE ADDUSER":OPKvS(perm="m",auth=OPKvF,arg=OPKvF,help="Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.")})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=OPKvx)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=OPKvF)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=OPKvF):
...
def main():
 do_start_ftp(OPKvb(sys.argv[1]),asynchronous=OPKvx)
