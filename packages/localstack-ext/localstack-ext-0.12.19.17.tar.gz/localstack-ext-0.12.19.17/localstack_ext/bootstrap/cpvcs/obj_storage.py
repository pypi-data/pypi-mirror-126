...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=lxFIv
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:lxFIr)->lxFIr:
  pass
 @lxFIV
 def get(cls,requested_file_locator:lxFIr):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=lxFIv
 @lxFIV
 def get(cls,state_file_locator:lxFIr,requested_storage:lxFIr,serializers:Dict[lxFIr,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[lxFIr,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:lxFIr)->lxFIr:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:lxFIr)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:lxFIr)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:lxFIr)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:lxFIr)->lxFIr:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[lxFIr]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:lxFIr,new_key:lxFIr,referenced_by_version:lxFIr=lxFIv):
  pass
 @abc.abstractmethod
 def version_exists(self,key:lxFIr)->lxFIz:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:lxFIr,key:lxFIr):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:lxFIr,new_key:lxFIr,base_path:lxFIr)->lxFIz:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[lxFIr]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[lxFIr,lxFIr],remote_location:lxFIr=lxFIv)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[lxFIr]:
  ...
  for cpvcs_obj in args:
   if lxFIU(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif lxFIU(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[lxFIr,lxFIr],remote_location:lxFIr=lxFIv)->List[Union[Revision,Version]]:
