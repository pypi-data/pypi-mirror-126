...
def random_hash()->gSXlq:
 return gSXlt(random.getrandbits(160))
def compute_file_hash(file_path:gSXlq,accum=gSXlT)->Optional[gSXlq]:
 try:
  with gSXln(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:gSXlV,state_archive:gSXlq)->gSXlq:
 ...
 m.update(gSXlq(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->gSXlq:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=gSXlv(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with gSXln(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except gSXlM as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if gSXlH(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(gSXlq(cpvcs_node.revision_number).encode("utf-8"))
 elif gSXlH(cpvcs_node,Version):
  m.update(gSXlq(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
