...
SENT_EMAILS=[]
EMAIL_BLACKLIST=EMmcp()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",EMmcH(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=EMmcL,from_name=EMmcL,smtp_host=EMmcL,smtp_user=EMmcL,smtp_pass=EMmcL,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if EMmch(recipients,EMmcs)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=EMmcL):
 ...
 result["From"]=formataddr((EMmcH(Header(from_name,"utf-8")),from_email))
 return result
