...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return KLyIr if e else KLyIh
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->KLyIU:
...
@click.option("--revert",is_flag=KLyIr,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:KLyIF):
...
def _cpvcs_initialized(pod_name:KLyIA)->KLyIF:
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_init(name:KLyIA):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=KLyIr,default=KLyIh)
def cmd_cpvcs_delete(name:KLyIA,remote:KLyIF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_register_remote(name:KLyIA):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=KLyIr)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=KLyIr)
def cmd_cpvcs_rename(name:KLyIA,new_name:KLyIA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_commit(message:KLyIA,name:KLyIA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=KLyIr,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_push(squash:KLyIF,message:KLyIA,name:KLyIA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=KLyIB,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=KLyIr,default=KLyIh,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_inject(version:KLyIB,reset:KLyIF,name:KLyIA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=KLyIr,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=KLyIr,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:KLyIA,inject:KLyIF,reset:KLyIF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=KLyIr,default=KLyIh)
def cmd_cpvcs_list_pods(remote:KLyIF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_versions(name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=KLyIr,type=KLyIB)
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_version_info(version:KLyIB,name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=KLyIB,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_version_metamodel(version:KLyIB,name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=KLyIr,type=KLyIB,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=KLyIr,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=KLyIr,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=KLyIh,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_set_version(version:KLyIB,inject:KLyIF,reset:KLyIF,commit_before:KLyIF,name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_commits(version:KLyIB,name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=KLyIr)
@click.option("--commit","-c",required=KLyIr)
@click.option("-n","--name",help="Name of the cloud pod",required=KLyIr)
def cmd_cpvcs_commit_diff(version:KLyIB,commit:KLyIB,name:KLyIA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(KLyIU)
  status.stop()
...
def print_table(columns:Dict[KLyIA,KLyIA],rows:List[Dict[KLyIA,Any]],formatters:Dict[KLyIA,Callable[[Any],KLyIA]]=KLyIU):
 from rich.table import Table
 if formatters is KLyIU:
  formatters=KLyIc()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  KLyIH(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->KLyIU:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->KLyIU:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->KLyIU:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->KLyIU:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
