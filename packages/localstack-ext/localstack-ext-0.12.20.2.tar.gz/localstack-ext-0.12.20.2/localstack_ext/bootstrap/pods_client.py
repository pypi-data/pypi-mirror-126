...
class PodInfo:
 def __init__(self,name=rEgQP,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(rEgQz):
 BACKEND="_none_"
 def __init__(self,pod_name=rEgQP,config=rEgQP):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise rEgQv("Not implemented")
 def delete(self,remote:rEgQl)->rEgQl:
  raise rEgQv("Not implemented")
 def push(self,comment:rEgQj=rEgQP)->PodInfo:
  raise rEgQv("Not implemented")
 def pull(self,inject_version_state:rEgQl=rEgQp,reset_state_before:rEgQl=rEgQp,fetch_all:rEgQl=rEgQP):
  raise rEgQv("Not implemented")
 def commit(self,message:rEgQj=rEgQP):
  raise rEgQv("Not implemented")
 def inject(self,version:rEgQk,reset_state:rEgQl):
  raise rEgQv("Not implemented")
 def list_versions(self)->List[rEgQj]:
  raise rEgQv("Not implemented")
 def version_info(self,version:rEgQk):
  raise rEgQv("Not implemented")
 def version_metamodel(self,version:rEgQk):
  raise rEgQv("Not implemented")
 def set_version(self,version:rEgQk,inject_version_state:rEgQl,reset_state:rEgQl,commit_before:rEgQl):
  raise rEgQv("Not implemented")
 def list_version_commits(self,version:rEgQk)->List[rEgQj]:
  raise rEgQv("Not implemented")
 def get_commit_diff(self,version:rEgQk,commit:rEgQk)->rEgQj:
  raise rEgQv("Not implemented")
 def register_remote(self,pod_name:rEgQj)->rEgQl:
  raise rEgQv("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->rEgQl:
  raise rEgQv("Not implemented")
 def list_pods(self,fetch_remote:rEgQl)->Set[rEgQj]:
  raise rEgQv("Not implemented")
 ...
 def get(cls,pod_name,pre_config=rEgQP):
  ...
  raise rEgQv('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=rEgQp
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=rEgQV)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=rEgQV
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise rEgQv("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @rEgQC
 def get_state_zip_from_instance(get_content=rEgQp):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise rEgQv("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:rEgQj=rEgQP):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @rEgQC
 def parse_pod_name_from_qualifying_name(qualifying_name:rEgQj)->rEgQj:
 ...
 @rEgQC
 def _get_max_version_for_pod_from_platform(pod_name:rEgQj,auth_headers):
  ...
  remote_max_ver=rEgQk(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with rEgQM(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with rEgQM(meta_archive,"rb")as meta_archive_content:
   ...
   with rEgQM(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=rEgQP)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:rEgQj):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:rEgQj=rEgQP)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:rEgQl=rEgQp,reset_state_before:rEgQl=rEgQp,fetch_all:rEgQl=rEgQV):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=rEgQp):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(rEgQI(lambda ver:rEgQj(ver),rEgQJ(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:rEgQk,reset_state:rEgQl):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=rEgQV,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:rEgQl)->Set[rEgQj]:
  ...
  return result
 @rEgQx
 def upload_content(cls,presigned_url:rEgQj,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise rEgQv("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @rEgQC
 def create_platform_url(request:rEgQj)->rEgQj:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:rEgQj=rEgQP)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=rEgQV)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(rEgQL(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=rEgQL(zip_data_content)
  return result
 def pull(self,inject_version_state:rEgQl=rEgQp,reset_state_before:rEgQl=rEgQp,fetch_all:rEgQl=rEgQP):
  ...
  rm_rf(zip_path)
 @rEgQC
 def presigned_url(pod_name:rEgQj,mode:rEgQj)->rEgQj:
  ...
  if response.status_code>=400:
   raise rEgQv("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise rEgQv("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:rEgQj=rEgQP):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=rEgQV)
 ...
 def pull(self,inject_version_state:rEgQl=rEgQp,reset_state_before:rEgQl=rEgQp,fetch_all:rEgQl=rEgQP):
  ...
  if remote_ref not in remote_refs:
   raise rEgQv('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=rEgQc(self,"clone_dir",rEgQP)
...
class PodConfigManagerMeta(rEgQT):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=rEgQP
   for manager in cls.CHAIN:
    try:
     tmp=rEgQc(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif rEgQG(tmp,rEgQF)and rEgQG(result,rEgQF):
       result.extend(tmp)
    except rEgQv:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not rEgQP:
    return result
   raise rEgQv('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(rEgQz,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @rEgQx
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise rEgQv('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(rEgQz):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=rEgQV).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=rEgQY([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=rEgQp):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except rEgQv:
   if safe:
    return{}
   raise rEgQv('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(rEgQz):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise rEgQv("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise rEgQv("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:rEgQj,remote:rEgQl,pre_config:Dict[rEgQj,rEgQj])->rEgQl:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs)->rEgQl:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:rEgQj,new_pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:rEgQl,pre_config:Dict[rEgQj,rEgQj],**kwargs)->List[rEgQj]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],message:rEgQj=rEgQP,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:rEgQj,version:rEgQk,reset_state:rEgQl,pre_config:Dict[rEgQj,rEgQj],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs)->List[rEgQj]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:rEgQk,pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:rEgQk,pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:rEgQk,inject_version_state:rEgQl,reset_state:rEgQl,commit_before:rEgQl,pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj],**kwargs)->rEgQl:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:rEgQk,pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj])->List[rEgQj]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:rEgQk,commit:rEgQk,pod_name:rEgQj,pre_config:Dict[rEgQj,rEgQj])->rEgQj:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=rEgQP,squash_commits=rEgQp,comment=rEgQP,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=rEgQp,reset_state_before=rEgQp,**kwargs):
 pre_config=kwargs.get("pre_config",rEgQP)
 if not pod_name:
  raise rEgQv("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=rEgQp,exclude_from_reset:List[rEgQj]=rEgQP):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise rEgQv("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->rEgQj:
 try:
...
def get_persisted_resource_names(data_dir)->List[rEgQj]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and rEgQL(os.listdir(entry.path))>0:
    names.append(entry.name)
