...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qhirt()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qhirC(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qhirg,from_name=qhirg,smtp_host=qhirg,smtp_user=qhirg,smtp_pass=qhirg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qhiru(recipients,qhirD)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qhirg):
 ...
 result["From"]=formataddr((qhirC(Header(from_name,"utf-8")),from_email))
 return result
