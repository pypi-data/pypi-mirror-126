...
def random_hash()->TKQwe:
 return TKQwG(random.getrandbits(160))
def compute_file_hash(file_path:TKQwe,accum=TKQwA)->Optional[TKQwe]:
 try:
  with TKQwH(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:TKQwg,state_archive:TKQwe)->TKQwe:
 ...
 m.update(TKQwe(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->TKQwe:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=TKQwU(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with TKQwH(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except TKQwz as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if TKQwV(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(TKQwe(cpvcs_node.revision_number).encode("utf-8"))
 elif TKQwV(cpvcs_node,Version):
  m.update(TKQwe(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
