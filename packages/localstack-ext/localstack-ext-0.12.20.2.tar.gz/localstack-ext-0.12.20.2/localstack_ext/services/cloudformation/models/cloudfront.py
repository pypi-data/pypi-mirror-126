...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or EsVNi
 @EsVNU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return EsVNd(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @EsVNp
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if EsVNe(origins,EsVNx)else origins.get("Items")
  origins_to_create=EsVNc([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if EsVNe(aliases,EsVNx)else aliases.get("Items")
  aliases_to_create=EsVNc(aliases)
  for distr in distrs:
   origins=EsVNc([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=EsVNc(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",EsVNl)
   config["Enabled"]=EsVNo(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if EsVNe(custom_errors,EsVNW)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=EsVNo(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":EsVNl}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=EsVNo(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=EsVNi):
    parent=config if parent is EsVNi else parent
    items=parent.get(key)or[]
    if EsVNe(items,EsVNW)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":EsVNY(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if EsVNe(cache_behavs,EsVNW)else cache_behavs)
