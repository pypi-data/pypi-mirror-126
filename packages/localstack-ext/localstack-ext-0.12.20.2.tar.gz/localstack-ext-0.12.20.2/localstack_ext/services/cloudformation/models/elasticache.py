...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=eAInl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not eAInR
   return result
  except eAInE:
   props=self.props
   ...
   if parts[0]in candidates and eAInU(parts)>1:
