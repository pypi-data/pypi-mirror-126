...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[LoRCA])[0]
 def get_physical_resource_id(self,attribute=LoRCA,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if LoRCh(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return LoRCD(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @LoRCz
 def add_defaults(resource,stack_name:LoRCN):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return LoRCD(SESReceiptRule,self).get_cfn_attribute(attribute)
 @LoRCb
