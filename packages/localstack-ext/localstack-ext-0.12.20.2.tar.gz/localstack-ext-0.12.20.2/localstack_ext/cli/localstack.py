...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return AUrNY if e else AUrNH
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->AUrNS:
...
@click.option("--revert",is_flag=AUrNY,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:AUrNq):
...
def _cpvcs_initialized(pod_name:AUrNb)->AUrNq:
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_init(name:AUrNb):
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=AUrNY,default=AUrNH)
def cmd_cpvcs_delete(name:AUrNb,remote:AUrNq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_register_remote(name:AUrNb):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=AUrNY)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=AUrNY)
def cmd_cpvcs_rename(name:AUrNb,new_name:AUrNb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_commit(message:AUrNb,name:AUrNb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=AUrNY,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_push(squash:AUrNq,message:AUrNb,name:AUrNb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=AUrND,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=AUrNY,default=AUrNH,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_inject(version:AUrND,reset:AUrNq,name:AUrNb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=AUrNY,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=AUrNY,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:AUrNb,inject:AUrNq,reset:AUrNq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=AUrNY,default=AUrNH)
def cmd_cpvcs_list_pods(remote:AUrNq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_versions(name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=AUrNY,type=AUrND)
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_version_info(version:AUrND,name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=AUrND,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_version_metamodel(version:AUrND,name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=AUrNY,type=AUrND,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=AUrNY,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=AUrNY,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=AUrNH,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_set_version(version:AUrND,inject:AUrNq,reset:AUrNq,commit_before:AUrNq,name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_commits(version:AUrND,name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=AUrNY)
@click.option("--commit","-c",required=AUrNY)
@click.option("-n","--name",help="Name of the cloud pod",required=AUrNY)
def cmd_cpvcs_commit_diff(version:AUrND,commit:AUrND,name:AUrNb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(AUrNS)
  status.stop()
...
def print_table(columns:Dict[AUrNb,AUrNb],rows:List[Dict[AUrNb,Any]],formatters:Dict[AUrNb,Callable[[Any],AUrNb]]=AUrNS):
 from rich.table import Table
 if formatters is AUrNS:
  formatters=AUrNF()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  AUrNv(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->AUrNS:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->AUrNS:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->AUrNS:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->AUrNS:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
