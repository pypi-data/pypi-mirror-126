...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return SEYtz if e else SEYtx
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->SEYtd:
...
@click.option("--revert",is_flag=SEYtz,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:SEYti):
...
def _cpvcs_initialized(pod_name:SEYtK)->SEYti:
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_init(name:SEYtK):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=SEYtz,default=SEYtx)
def cmd_cpvcs_delete(name:SEYtK,remote:SEYti):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_register_remote(name:SEYtK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=SEYtz)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=SEYtz)
def cmd_cpvcs_rename(name:SEYtK,new_name:SEYtK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_commit(message:SEYtK,name:SEYtK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=SEYtz,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_push(squash:SEYti,message:SEYtK,name:SEYtK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=SEYtm,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=SEYtz,default=SEYtx,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_inject(version:SEYtm,reset:SEYti,name:SEYtK):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=SEYtz,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=SEYtz,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:SEYtK,inject:SEYti,reset:SEYti):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=SEYtz,default=SEYtx)
def cmd_cpvcs_list_pods(remote:SEYti):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_versions(name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SEYtz,type=SEYtm)
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_version_info(version:SEYtm,name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=SEYtm,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_version_metamodel(version:SEYtm,name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SEYtz,type=SEYtm,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=SEYtz,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=SEYtz,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=SEYtx,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_set_version(version:SEYtm,inject:SEYti,reset:SEYti,commit_before:SEYti,name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_commits(version:SEYtm,name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=SEYtz)
@click.option("--commit","-c",required=SEYtz)
@click.option("-n","--name",help="Name of the cloud pod",required=SEYtz)
def cmd_cpvcs_commit_diff(version:SEYtm,commit:SEYtm,name:SEYtK):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(SEYtd)
  status.stop()
...
def print_table(columns:Dict[SEYtK,SEYtK],rows:List[Dict[SEYtK,Any]],formatters:Dict[SEYtK,Callable[[Any],SEYtK]]=SEYtd):
 from rich.table import Table
 if formatters is SEYtd:
  formatters=SEYtU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  SEYtT(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->SEYtd:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->SEYtd:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->SEYtd:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->SEYtd:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
