...
class DecryptionHandler:
 decryption_key:zneUi
 def __init__(self,decryption_key:zneUi):
  self.decryption_key=decryption_key
 def decrypt(self,content)->zneUi:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=zneUu):
  if path and not zneUv(path,zneUJ):
   path=zneUJ(zneUr(path,"_path",[]))
  if not path:
   return zneUu
...
def patch_traceback_lines():
 if zneUr(traceback.FrameSummary,"_ls_patch_applied",zneUu):
  return
 @zneUC
 def line(self):
  try:
 ...
 zneUc(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=zneUo
