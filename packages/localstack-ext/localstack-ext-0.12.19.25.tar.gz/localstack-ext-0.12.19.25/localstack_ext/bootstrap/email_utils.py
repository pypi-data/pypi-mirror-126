...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SyzeT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SyzeP(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Syzek,from_name=Syzek,smtp_host=Syzek,smtp_user=Syzek,smtp_pass=Syzek,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Syzeg(recipients,Syzep)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Syzek):
 ...
 result["From"]=formataddr((SyzeP(Header(from_name,"utf-8")),from_email))
 return result
