...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or GdbBw
 @GdbBF
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return GdbBQ(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @GdbBi
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if GdbBs(origins,GdbBI)else origins.get("Items")
  origins_to_create=GdbBR([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if GdbBs(aliases,GdbBI)else aliases.get("Items")
  aliases_to_create=GdbBR(aliases)
  for distr in distrs:
   origins=GdbBR([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=GdbBR(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",GdbBM)
   config["Enabled"]=GdbBu(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if GdbBs(custom_errors,GdbBo)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=GdbBu(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":GdbBM}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=GdbBu(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=GdbBw):
    parent=config if parent is GdbBw else parent
    items=parent.get(key)or[]
    if GdbBs(items,GdbBo)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":GdbBl(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if GdbBs(cache_behavs,GdbBo)else cache_behavs)
