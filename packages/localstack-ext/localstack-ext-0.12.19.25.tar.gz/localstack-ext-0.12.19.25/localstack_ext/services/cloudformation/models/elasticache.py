...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=gmUhY(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not gmUhM
   return result
  except gmUhW:
   props=self.props
   ...
   if parts[0]in candidates and gmUhu(parts)>1:
