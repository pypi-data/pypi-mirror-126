...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[cQgdy])[0]
 def get_physical_resource_id(self,attribute=cQgdy,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if cQgdn(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return cQgdM(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @cQgdv
 def add_defaults(resource,stack_name:cQgdp):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return cQgdM(SESReceiptRule,self).get_cfn_attribute(attribute)
 @cQgdj
