...
SENT_EMAILS=[]
EMAIL_BLACKLIST=goaSr()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",goaSc(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=goaSw,from_name=goaSw,smtp_host=goaSw,smtp_user=goaSw,smtp_pass=goaSw,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if goaSW(recipients,goaSU)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=goaSw):
 ...
 result["From"]=formataddr((goaSc(Header(from_name,"utf-8")),from_email))
 return result
