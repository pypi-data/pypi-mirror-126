...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=GsEPc
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:GsEPO)->GsEPO:
  pass
 @GsEPp
 def get(cls,requested_file_locator:GsEPO):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=GsEPc
 @GsEPp
 def get(cls,state_file_locator:GsEPO,requested_storage:GsEPO,serializers:Dict[GsEPO,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[GsEPO,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:GsEPO)->GsEPO:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:GsEPO)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:GsEPO)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:GsEPO)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:GsEPO)->GsEPO:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[GsEPO]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:GsEPO,new_key:GsEPO,referenced_by_version:GsEPO=GsEPc):
  pass
 @abc.abstractmethod
 def version_exists(self,key:GsEPO)->GsEPw:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:GsEPO,key:GsEPO):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:GsEPO,new_key:GsEPO,base_path:GsEPO)->GsEPw:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[GsEPO]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[GsEPO,GsEPO],remote_location:GsEPO=GsEPc)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[GsEPO]:
  ...
  for cpvcs_obj in args:
   if GsEPz(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif GsEPz(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[GsEPO,GsEPO],remote_location:GsEPO=GsEPc)->List[Union[Revision,Version]]:
