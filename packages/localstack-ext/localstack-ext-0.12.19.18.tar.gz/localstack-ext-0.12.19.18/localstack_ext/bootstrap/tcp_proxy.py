...
class AttrDict(wEfIv):
 def __init__(self,*args,**kwargs):
  wEfIT(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and wEfIH(socket,"IPV6_V6ONLY"):
...
class Half(wEfIC):
 def __init__(self,opt,sock,addr,wEfIp):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.wEfIp=wEfIp
  self.name="peer" if self.wEfIp else "client"
  self.queue=[]
  self.dest=wEfIW
  self.err=wEfIW
  self.ready=wEfIJ
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":wEfIJ,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":wEfIJ,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":wEfIJ,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":wEfIJ,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
