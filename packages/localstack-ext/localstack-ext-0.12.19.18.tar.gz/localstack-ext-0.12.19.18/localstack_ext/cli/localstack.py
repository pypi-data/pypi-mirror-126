...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ckpTQ if e else ckpTv
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ckpTS:
...
@click.option("--revert",is_flag=ckpTQ,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ckpTJ):
...
def _cpvcs_initialized(pod_name:ckpTN)->ckpTJ:
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_init(name:ckpTN):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=ckpTQ,default=ckpTv)
def cmd_cpvcs_delete(name:ckpTN,remote:ckpTJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_register_remote(name:ckpTN):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=ckpTQ)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_rename(name:ckpTN,new_name:ckpTN):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_commit(message:ckpTN,name:ckpTN):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=ckpTQ,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_push(squash:ckpTJ,message:ckpTN,name:ckpTN):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=ckpTE,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=ckpTQ,default=ckpTv,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_inject(version:ckpTE,reset:ckpTJ,name:ckpTN):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=ckpTQ,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=ckpTQ,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:ckpTN,inject:ckpTJ,reset:ckpTJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=ckpTQ,default=ckpTv)
def cmd_cpvcs_list_pods(remote:ckpTJ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_versions(name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ckpTQ,type=ckpTE)
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_version_info(version:ckpTE,name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=ckpTE,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_version_metamodel(version:ckpTE,name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=ckpTQ,type=ckpTE,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=ckpTQ,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=ckpTQ,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=ckpTv,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_set_version(version:ckpTE,inject:ckpTJ,reset:ckpTJ,commit_before:ckpTJ,name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_commits(version:ckpTE,name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=ckpTQ)
@click.option("--commit","-c",required=ckpTQ)
@click.option("-n","--name",help="Name of the cloud pod",required=ckpTQ)
def cmd_cpvcs_commit_diff(version:ckpTE,commit:ckpTE,name:ckpTN):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(ckpTS)
  status.stop()
...
def print_table(columns:Dict[ckpTN,ckpTN],rows:List[Dict[ckpTN,Any]],formatters:Dict[ckpTN,Callable[[Any],ckpTN]]=ckpTS):
 from rich.table import Table
 if formatters is ckpTS:
  formatters=ckpTI()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ckpTU(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ckpTS:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ckpTS:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ckpTS:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ckpTS:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
