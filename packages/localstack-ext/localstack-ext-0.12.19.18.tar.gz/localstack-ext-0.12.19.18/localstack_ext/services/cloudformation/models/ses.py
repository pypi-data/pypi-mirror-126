...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[dVhSF])[0]
 def get_physical_resource_id(self,attribute=dVhSF,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if dVhSO(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dVhSA(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @dVhSc
 def add_defaults(resource,stack_name:dVhST):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return dVhSA(SESReceiptRule,self).get_cfn_attribute(attribute)
 @dVhSN
