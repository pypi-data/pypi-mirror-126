...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xGXtM(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xGXtA
   return result
  except xGXtK:
   props=self.props
   ...
   if parts[0]in candidates and xGXtu(parts)>1:
