...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or cOgFR
 @cOgFw
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return cOgFd(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @cOgFv
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if cOgFH(origins,cOgFa)else origins.get("Items")
  origins_to_create=cOgFj([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if cOgFH(aliases,cOgFa)else aliases.get("Items")
  aliases_to_create=cOgFj(aliases)
  for distr in distrs:
   origins=cOgFj([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=cOgFj(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",cOgFS)
   config["Enabled"]=cOgFn(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if cOgFH(custom_errors,cOgFI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=cOgFn(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":cOgFS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=cOgFn(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=cOgFR):
    parent=config if parent is cOgFR else parent
    items=parent.get(key)or[]
    if cOgFH(items,cOgFI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":cOgFe(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if cOgFH(cache_behavs,cOgFI)else cache_behavs)
