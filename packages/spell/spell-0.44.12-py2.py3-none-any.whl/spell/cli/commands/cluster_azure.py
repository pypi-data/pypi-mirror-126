...
def assign_acr_roles(
    ...
    click.echo(
        f"Successfully assigned `ACR Push and ACR Pull` Role to Service Principal `{sp_object_id}`"
    )
...
def add_az_registry(repo_name, cluster):
    ...
    resource_group_name = click.prompt(
        "Enter the name of the Resource Group you registry falls under"
    )
    ...
    if resource_group_name == cluster["networking"]["azure"]["resource_group_name"]:
        click.echo(
            "This is the same resource group for your Spell resources, permissions already granted!"
        )
        return
...
def delete_az_registry(repo_name, cluster):
    ...
    """
    resource_group_name = click.prompt(
        "Enter the the name of the Resource Group you registry falls under"
    )
    # If registry is in the Spell RG, we automatically have permissions
    if resource_group_name == cluster["networking"]["azure"]["resource_group_name"]:
        return
