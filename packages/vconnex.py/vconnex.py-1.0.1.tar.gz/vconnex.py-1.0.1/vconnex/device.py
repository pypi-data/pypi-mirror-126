...
class VconnexDeviceManager(threading.Thread):
    ...
    def __update_device_list(self):
        while self.is_initialized():
            ...
            current_device_map = self.device_map
                ...
                if device_id in new_device_id_list:
                    ...
                    try:
                        if (
                            ...
                            or current_device.modifiedTimeStr != new_device.modifiedTimeStr
                        ):
                            current_device_data = current_device.data
                            old_device = copy.deepcopy(current_device)
                            for attr in vars(new_device):
                                setattr(current_device, getattr(new_device, attr))
                            current_device.data = current_device_data
                            modified_device_tuple_list.append((current_device, old_device))
                    except Exception:
                        logger.exception("Oops, something went wrong!")
