...
class NeuronCells:
    ...
    def inference(self, vector: np.ndarray):
        """
        inference(classify)
    ...
    def inference_category(self, vector: np.ndarray):
        """
        inference(classify)
        ...
        min_distance = np.min(distances)
        closest_cell_idx = distances.index(min_distance)
        category = self.cells[closest_cell_idx].target

        return category

    def inference_distances(self, vector: np.ndarray):
        """
        inference(classify)
