...
directory_base = os.path.basename("test_pydev") + "/"

global construct
construct = ["AAA", "AAT", "AAC", "AAG",
            "ATA", "ATT", "ATC", "ATG",
            "ACA", "ACT", "ACC", "ACG",
            "AGA", "AGT", "AGC", "AGG",
            ...
            "GCA", "GCT", "GCC", "GCG",
            "GGA", "GGT", "GGC", "GGG"]
construct = dict(zip(construct, np.arange(len(construct))))
...
def access_read_ranges(selected, threads):
	ranges = []
	post_data = []
	for source in selected:
		file = directory_base + "MetaPop/02.Filtered_Samples/" + source + "_preprocessed.bam"
...
#Ref, snp, rs, and sr are the counts of observed all ref, all snp, reference first + snp second, snp first + ref second, resp.
def calc_phi(ref, snp, rs, sr):
	top = (ref*snp)-(sr*rs)
	botA = np.sqrt(ref+rs)
	botB = np.sqrt(ref+sr)
	botC = np.sqrt(snp+rs)
...
def do_mine_reads(output_directory, threads):
	...
	res = access_read_ranges(selections_to_read, threads)
